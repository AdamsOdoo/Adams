...
## Consolidated candidate — source frozen, publication pending
...
- Settled local gate: 467 dependency-free tests pass in 18.736 seconds; Python/bash syntax, static policy, change-size, whitespace and both marketplace package checks pass. After freezing, all 467 tests pass again in 17.571 seconds; baseline and catalog provenance checks pass. Six generated artifacts reproduce byte-identically. Local GraphQL validation is blocked by missing graphql-core; the CI policy gate installs its pinned dependency.
...
- Prepared source `c7543af0864490e4e5c545cbeb634c59bee3d831` has tree `cb55145f5c6e016b57ba7148108a397dec5cca9b`, verified identical to local `60894d9`. All 461 local tests pass in 21.180 seconds; changed Python syntax and whitespace checks pass. Publication head and native rerun still require verification.
...
- Cancellation SQL flush, settings preparation boundary, source inventories and API-pin corrections pass their selected native classes. Four residuals remain: one invalid duplicate variant fixture; one cleanup fixture missing expected callback digest; two real inventory-observation paths rejected by the reader actor check. Do not label these corrections qualified before the next native result.
- Read-only tracing confirms installed root cron can claim other-company jobs, but handler invocation retains ambient company and root lacks connector role membership. A bounded internal worker authorization and target-company correction is under development; broad sudo/user-role relaxation is prohibited.
