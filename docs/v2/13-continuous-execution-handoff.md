...
## Current checkpoint — 9 September 2026; local repairs reviewed, public disclosure approval required

- The user approved public publication through local `4609e24bc1fc43291d277b41e571c91f08af5530` and continuous development. Live PR #212 already contains the tree-equivalent published checkpoint `30bde525c9d003a9b00eb4175eed012dc65e8c2c` (tree `3c366cf8d869d303ef05107193bda91f3117d781`). Do not repeat that publication or report GitHub write permission as missing.
- Local merge `a4d7db3` preserves both original and tree-equivalent published histories. Subsequent local commits `4660763` / `20133b1` refresh deterministic evidence and its operation catalog provenance; `70147e1` rejects malformed mode-generation input before locking. The source worktree also contained a preserved company-isolation and test-contract batch at resumption; none was discarded.
- [Native run 34280617682](https://github.com/AdamsOdoo/Adams/actions/runs/34280617682) completed on `30bde525`. **Lite and Full candidate meta-addon installations pass.** Fresh and warm each report **61 failures / 686 errors out of 2,994 tests**. Old-core W2 fails with 10 errors; both migration origins and their repeated updates fail; the nonstandard lane reports 4 failures / 33 errors out of 70 tests. Browser evidence fails. These are executed failures, not an unavailable runtime or a passing release gate.
- The job log and embedded summary were read through GitHub. The detailed artifact is ID `10079324471`, expected SHA-256 `eef08be12fd38fcf3a2c53b78dca40663732a2bd7b531e9b291bd1338428a437`; both returned download URLs gave HTTP 403 in this runtime, so the new detailed artifact was **not** downloaded or checksum-verified. Existing `ci-artifacts/` files are the older campaign and must not be labeled as this run.
- The resumed batch is committed as `ac68ad81622829210cbedbae0d70be31a7d58058`. It validates caller companies before SUPERUSER side-cursor construction, scopes stale-mutation candidate selection before LIMIT, and rechecks locked lineage. It also corrects protected-command test recordset contexts, classifies V2 settings, and updates structural API-version/sudo inventories with explicit boundaries. Separate Sol review found a same-company/cross-store projection defect: C2 now recovers and projects the authoritative attempt run despite a damaged current job-run pointer; C1 requires exact job/run/store lineage. A native two-store corruption regression was added. The lead reviewed that correction. Native regression proof remains required; this is a local development checkpoint, not runtime assurance.
- Final local verification on the `ac68ad8` source tree: **461 dependency-free tests passed** (16.193 seconds). Static policy, core dependency policy, diff whitespace, and change-size checks pass. The sudo inventory contains 139 current sites, with no missing/obsolete identities and an explicit purpose for each owner; that check proves inventory consistency, not authorization behavior. The active-worktree frozen-evidence check correctly fails because the inventories describe `30bde525`; it must be refreshed twice for the next published source before a policy-green claim. The baseline line inside unittest output refers to a temporary test fixture, not the active repository. No new live Shopify or Odoo.sh work occurred; UI proposals remain parked and unapproved.
- An ordinary non-force push of later local HEAD was **rejected by automatic approval review** because it contains changes beyond the explicitly approved disclosure checkpoint. Do not bypass the rejection via GitHub object APIs or another transport. Finish the local review/checkpoint and request explicit public-disclosure approval for that exact newer payload. This is distinct from the already successful publication of `4609e24`'s tree.
- Next: obtain explicit public-disclosure approval for the new local checkpoint (resolve this documentation commit with `git rev-parse HEAD`). The payload beyond published `30bde525` is strict mode-generation validation, company/runtime/recovery safeguards, native and dependency-free regressions, test-contract/inventory corrections, frozen evidence provenance, and current handoff/blueprint/evidence notes. No new UI files or deployment configuration changed. After authorization, verify the unchanged remote head, publish non-force, verify tree equality, refresh deterministic inventories twice on the exact published source, and run the affected native gate before new domain enablement. F1 old-core compatibility, F2/F3 native/security/concurrency, D1–D5 business loops, approved UI, usability and release gates remain open.
- Rollback: these follow-up edits are source-only and reversible; no database or shop effects need compensation. Preserve both histories and all unfinished files. PR #210/#211 and production/public-release promotion remain untouched.
