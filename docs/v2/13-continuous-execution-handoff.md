...
## Active correction checkpoint — 9 September 2026

- Last published `7c4b89bce6c1d6599b898e7e57719c4b41240d4f`, source `96f139c1b0beb4d20e0a77eb2f3d4eabbf9ffbb1`. [Policy 34332567179](https://github.com/AdamsOdoo/Adams/actions/runs/34332567179) passes. [Native 34332566803](https://github.com/AdamsOdoo/Adams/actions/runs/34332566803): 553 tests, one failure, one error, no selected skips/missing classes; full campaign did not execute. Exact-source artifact/checksum and summary are recorded in document 18.
- Prepared production corrections: cancellation flushes its exact stored SQL filter fields before selecting/locking jobs; read-only review traced the missing-flush defect. All typed gateways/transport/read contracts now share the canonical API pin, whose value remains unchanged. No new permission bypass or remote operation is introduced.
- Prepared tests: settings model-boundary test gets the same temporary test-only create ACL as SEC-2; exact export/fulfillment gateway inventories; owning webhook/inventory query assertions; authorized real reader fixtures; canonical generated product domains; valid attribute-backed export variants; line-wrap-insensitive safety copy. The UI inventory explicitly keeps the preserved P16 proposal unmanifested. No production UI changes.
- Qualification: 461 dependency-free tests pass (16.662 seconds); static/dependency/change-size, changed Python compilation and whitespace checks pass. A static execution of the changed UI inventory check passes. Sol's focused transport/gateway/policy checks pass. These do not qualify native behavior.
- Next prerequisite selects 29 complete classes, retaining the existing 21 and adding the corrected domain/source-guard/reader classes. Independent-cursor and full fresh/warm/upgrade/browser campaigns remain unchanged downstream. Actual scheduled-worker admission through the typed reader remains an end-to-end qualification item; isolated reader mocks do not prove it.
- Prepared source is local `fbc91a5`, tree-identical to GitHub object `c195f82b3f181caf8371a5f4411af3bfe115436d`. Six frozen artifacts regenerated twice identically; publication/CI must be verified.
- First next action: freeze/publish this coherent correction with regenerated exact-source evidence, inspect its 29-class native result, then the full campaign if admitted. Do not publish another source over a running campaign. UI remains parked/unapproved; no direct Shopify/Odoo.sh session/production actions performed. Normal authorized dev publication may trigger Odoo.sh builds whose state has not been inspected.

## Current checkpoint — 9 September 2026; expanded security passes, residual guards under correction

- Published candidate `e087d961c8cf24091d313b9331a150f12cb539dc` contains source `783e818b1c2443e8e8f971244fa9984c6b6b4686` and exact-source frozen evidence. [Policy 34330476419](https://github.com/AdamsOdoo/Adams/actions/runs/34330476419) passes. [Native 34330476348](https://github.com/AdamsOdoo/Adams/actions/runs/34330476348): **490 tests, six failures, zero errors, no selected skips/missing classes**, 232 seconds. Full campaign did not execute. Verified artifact/checksum and summary are recorded in document 18 and evidence.
- All six added SEC-3/runtime/command-result classes pass, including historic same-company/cross-store quarantine. Both secondary-store C2 checks pass. The remaining recovery failure is the copied foreign C1 job starting in draft; the next fixture explicitly seeds running state/timestamp/owner and checks that state before sweeping.
- Five known mutation-source guard failures remain. Bounded correction maps extracted gateway documents to exact guarded consumers, recognizes actual mutation-attempt model ownership, and validates the one query-only reconciliation sender. No production transport/permission weakening or broad path exemption is authorized; negative guard checks are required before publication.
- Additional original-campaign fixtures are corrected: Odoo 19 user `group_ids` and public `default_get` for the zero configuration default. Cancellation fixtures also carry their owning run/configuration identity. The next prerequisite includes these full classes plus SEC-2 and sales company isolation (21 total). Native proof of these local corrections remains pending.
- Onboarding correction: sale previously injected `order_company_id` before the core structural-create validator, rejecting legitimate nonroot setup. A private preparation hook now derives addon-owned values after unchanged original-input admission. Two sales native regressions cover ambient company A/store B, supplied-company rejection, input immutability, trusted mixed-company batches and mismatched-company refusal. Separate read-only review found no issue; native proof is pending.
- Local verification for this prepared batch: 461 dependency-free tests pass (20.278 seconds); changed compilation, static/dependency/change-size and whitespace checks pass. Final source-guard AST sweep validates all 14 bindings with zero mutation/send/attempt-write violations; wrong-import, query/lease, inherited-write and retention negatives pass. These static checks do not establish native correctness. The new native candidate remains pending.
- Prepared source is local `c6b65d0`, tree-identical to GitHub source object `96f139c1b0beb4d20e0a77eb2f3d4eabbf9ffbb1`. Six frozen artifacts regenerated twice identically against that source; publication and matching native result must be verified.
- First next action: finish and review the source-guard correction, execute focused local checks, publish the coherent residual fixture/guard candidate with regenerated exact-source evidence, then inspect its native result. Preserve the full downstream campaign and all broader qualification gates. Do not describe the prior 490-test result as applying to changed source.
- UI remains parked/unapproved. Old-core W2, genuine upgrades, full domain/browser/live/performance/UAT/canary/release gates remain open. No direct Odoo.sh/session or live Shopify action occurred; normal authorized publication may trigger development builds whose state is not verified here. PR #210/#211 remain untouched. Production/public release requires separate authorization.

## Previous checkpoint — 9 September 2026; first focused native result and residual correction
...
- Diagnostic source is local `cd3bee01880858209378a085edb0589966579047`, tree-equivalent to GitHub object `6ca91e42d16861ef95b52ab300acb8314006709d`. Frozen evidence was generated twice identically on that object. Branch update/next native result must be verified before claiming publication or qualification.
...
- [Document 18](18-native-failure-triage.md) and its machine-readable index account for all 743 fresh error/failure headers, including 31 subtests. **667** stop at the operational activation gate. This is a setup cascade, not 667 independently diagnosed production defects. The remaining 76 headers include actor, DTO, structural, security, data and downstream workflow failures.
- Consolidated test fixtures across core, product/export, inventory, sale, fulfillment and webhook domains now explicitly seed active operational stores. Intentionally blocked/setup scenarios retain their states. Dedicated-cursor fixture cleanup includes the lifecycle audit records created by normal activation. Genuine barriers and production admission checks are retained. The earlier `d7000202` fixture corrections are included.
- Production correction: store-admin lifecycle actions are serialized before entering the strict JSON-shaped immutable DTO mapping. The native credential-safe projection test now also checks JSON encoding and consistency with top-level actions. This is backend response repair, not a UI redesign.
- Two inventory contract tests now inspect the imported typed gateway documents instead of their former service-file location, retaining nested inventory lookup and exactly-one idempotency directive per mutation. Other structural/security inventory failures remain open; no blanket allowlist was introduced.
- Added `tools/run_connector_focus.py` and `.ci/connector-native-focus.json`. CI runs nine exact native backend classes with the existing module closure, exact source/Odoo pin and disposable PostgreSQL DB before the unchanged full runner. Failures/errors, zero execution, missing class starts or selected skips fail the prerequisite. Its artifact explicitly records diagnostic scope, not full qualification; the real process-death test is enabled.
- Verification so far: 461 dependency-free tests pass; static, dependency-direction and change-size policies pass. Focus selectors and result/skip parsing were checked against the actual failed native log. New fixture and DTO assertions remain native-unqualified. Frozen inventories must be regenerated twice against the exact published source object before moving the branch.
- Source is locally committed as `c6327432f6bb7fc30cd75f9133fc6fc6137867f8`; GitHub source object `ab3611a9f8e95f6bc24123933b41ec4ed0763f8b` has the identical tree `d6a54dbc62f2cee685ab0519c8811aea746f5a69`. The branch ref has not yet been moved at preparation time. Six frozen artifacts were regenerated twice against that exact GitHub source object and matched byte-for-byte; unchanged GraphQL contracts carry matching provenance.
- First next action: publish this prepared source plus evidence to PR #212 without force, then verify the resulting ref. Inspect its focused native artifact and repair the first residual root; allow the full campaign only after this prerequisite passes. Do not repeat a full campaign on known failed foundations.
- Old-core W2 compatibility, SEC-3 coverage/lineage, remaining structural/provider fixtures, full integrated/native/browser/live/security/performance gates and canaries remain open. No live Shopify or production changes occurred. Rollback is source-only; preserve both local and published histories.

## Previous checkpoint — 9 September 2026; published policy green, native running, fixture follow-up local

- The user explicitly approved checkpoint `07041662f7e13c57e8edd815360429bb7bef4a5c`. It is now published on draft PR #212 as `51c20c11647d6eb4ff944b81a4704178a4039d34`, tree `3541c511e67105a139a59793ff12db45a257ed93`. All five intermediate trees matched their local commits. The non-force branch update was verified; no publication permission is outstanding for this payload.
- Shell git lacked HTTPS credentials; the authorized GitHub connection published the trees and commits. The published commit objects were reconstructed locally only after their exact SHA-1 identities matched; a tree-identical local merge preserves both histories without resetting, rebasing or discarding work. The local merge is a history checkpoint, not a remote source change.
- Published repairs: strict mode-generation input validation; caller company validation before privileged side cursors; bounded company-scoped stale recovery; exact C1 lineage and authoritative C2 attempt-run recovery/projection. Separate Sol review found and corrected the same-company/cross-store run projection defect, and the lead inspected the correction. Native regressions are included for forged companies, company-scoped recovery, and same-company run drift. Test recordset-context and structural settings/API/sudo inventories were also corrected.
- Published source verification: **461 dependency-free tests pass** locally and the corresponding remote fast-test step passed. Static policy, dependency direction and file-size gates pass remotely. Inventory equality (139 sudo sites) is structural evidence only, not proof of authorization behavior. Native membership, recovery and concurrency proof remains required.
- Evidence follow-up is published as `695aa801fbc04ea78376bf675f81451207577ef8` (tree `8dc602e47db9f8de1be9ebf81ff29dce0af1c9e9`), identical to local evidence commit `44beec95`. [Policy run 34320216952](https://github.com/AdamsOdoo/Adams/actions/runs/34320216952) **passes in full**, including GraphQL and frozen inventory checks. [Native run 34320216972](https://github.com/AdamsOdoo/Adams/actions/runs/34320216972) is running on this evidence head. PR #212 body contains these identities and links; PR #210/#211 heads were reverified unchanged.
- [Policy run 34319890550](https://github.com/AdamsOdoo/Adams/actions/runs/34319890550) stopped at the old frozen-evidence source. Six inventories have now been regenerated twice on exact source `51c20c1`, with byte-identical outputs; the operation catalog's unchanged documents retain their contracts with matching source provenance. The active repository baseline check and focused catalog tests pass. This documentation/evidence follow-up changes no addon source.
- [Native run 34319890549](https://github.com/AdamsOdoo/Adams/actions/runs/34319890549) was running at this checkpoint; no native success is inferred. Publication of this evidence-only follow-up may supersede it through existing CI concurrency rules; inspect the latest exact-head run rather than launching duplicates.
- Previous native source `30bde525`, [run 34280617682](https://github.com/AdamsOdoo/Adams/actions/runs/34280617682): Lite/Full candidate installations pass; fresh/warm each 61 failures and 686 errors of 2,994 tests; old-core W2 10 errors; nonstandard 4 failures and 33 errors of 70 tests; migration/repeated-update and browser gates fail. Its job summary was read, but detailed artifact `10079324471` downloads returned HTTP 403. Older local `ci-artifacts/` logs must not be relabeled as the new campaign.
- Local-only fixture follow-up after `695aa801`: corrected `TestBusinessAdmission` operational/other/no-credential stores in `test_disconnect_quiescence.py`, `TestInventoryPushMechanics.setUpClass`, `TestProductMatchDecision._make_store` and `TestProductImportMatching._import_job`. Each now calls the owning `_p15_set_activation('active')` service at explicit fixture preparation; no production admission, state transition or assertion was relaxed. Four test files changed, with eight inserted lines and one replaced docstring line. No new fixture is a live store.
- Provenance for that follow-up: old `ci-artifacts/fresh.log` is source `1c75c10`, not the current running campaign. All four test files were unchanged from that source to `51c20c1` before these edits. Traces stop at the still-current P15 active-store gate, before the intended assertions. Sol independently diagnosed the three domain families; the lead reviewed the raw setup paths and used the normal activation service. Historical counts identify scope, not a claim of newly passing tests.
- The four-file follow-up passes Python compilation and patch-whitespace checks only; Odoo-native execution remains pending. The prior 461-test source result and green remote policy belong to the published source. Do not describe the new native fixture corrections as qualified from those results. They remain locally committed while the coherent native run executes, avoiding another cancellation/restart.
- First next action: inspect native run `34320216972` to completion, retrieve its raw artifact if accessible, and reconcile the next first failure with these four local fixture changes. Run the affected native classes on the eventual combined correction checkpoint and batch any further root fixes before publication; refresh source evidence for that new candidate. F1–F3 and D1–D5 remain open; UI remains parked and unapproved; UAT/security/performance/live/canary/release gates remain required.

- No Shopify, Odoo.sh, staging or production changes occurred in this checkpoint. PR #210/#211 remain untouched. Public release and production promotion still require separate authority. Source rollback is the published tree above; evidence/docs-only follow-up needs no database or shop compensation.

Everything below is historical unless explicitly referenced by this checkpoint.
