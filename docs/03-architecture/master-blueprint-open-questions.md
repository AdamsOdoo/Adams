...
| MBQ-45 | **Partially resolved by DEC-013 acceptance (2026-07-03):** the proposed role hierarchy is accepted (Admin ⊃ Operator/Reviewer ⊃ Auditor). **Partially resolved by DEC-018 acceptance (2026-07-04):** the roles→groups mapping and surface-split residual are now also resolved — the four accepted roles map **1:1** to four Odoo security groups in Phase 1 (no finer-grained composition); the connector uses **one shared, role-gated application surface**, not a forked admin-app/functional-app pair. | DEC-012 §10; setup-ux-principles P10; Part A §J.1/§F.5; DEC-013; DEC-018 | Fixes group design before CSVs are written | Resolved (hierarchy + mapping/surface split) — ChatGPT via DEC-013 and DEC-018; exact `ir.model.access.csv` rows/record rules (MBQ-44) and group XML IDs (MBQ-01/02/03) remain Implementation planning | Yes for exact CSVs/XML IDs only; the mapping/surface-split question itself no longer blocks |
