...
**Accepted as the central register through DEC-014**, latest acceptance
...
level**; notably **MBQ-04, MBQ-08, MBQ-24, MBQ-27, MBQ-28, MBQ-53,
MBQ-54, MBQ-55, MBQ-56, MBQ-57, and MBQ-58 remain open**. Registering (or
...
Sprint C note immediately below, current as of this proposal.
...
> **Master Blueprint Sprint C note (2026-07-03) — Proposed for ChatGPT
> review via DEC-015; NOT yet accepted.** Sprint C
...
> proposes updates to the inventory/fulfillment rows below (MBQ-32
> through MBQ-43) and adds two new rows, MBQ-60 and MBQ-61. **Pending
> ChatGPT review and acceptance of DEC-015**, this sprint proposes:
> MBQ-37 and MBQ-39 as **Proposed resolved** (official-doc verification
> rows, resolvable by the sprint itself, not a ChatGPT-decision-owner row
> — but still pending DEC-015 acceptance before treated as final);
> MBQ-32, MBQ-36, MBQ-38, MBQ-40, MBQ-42, and MBQ-43 as **proposed
> partially resolved**; and MBQ-33, MBQ-34, and MBQ-41 remain **carried forward,
> open**, with a **recommendation** proposed for each, pending ChatGPT's
> direct decision (these three are explicitly ChatGPT-decision-owner
> rows and are not self-resolved by this sprint). MBQ-35 is **carried
> forward, open, unchanged**. **The register's accepted-through-DEC-014
> status above is otherwise unaffected — this sprint's inventory/
> fulfillment row updates are proposed, not yet part of the accepted
> register**, pending DEC-015's review. **MBQ-04, MBQ-08, MBQ-24,
> MBQ-27, MBQ-28, MBQ-53, MBQ-54, MBQ-55, MBQ-56, MBQ-57, and MBQ-58
...
| ID | Open question | Source | Why it matters | Decision owner | Blocks implementation |
| --- | --- | --- | --- | --- | --- |
| MBQ-32 | Exact **Odoo ORM source/field/formula behind "Free to Use"** (and whether a configurable Forecast/On-Hand/Free-to-Use default is offered). **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance, official-doc verification:** `product.product.free_qty` (compute `qty_available − reserved_quantity − expired_unreserved_qty`), equivalently per-location `stock.quant.available_quantity` (`quantity − reserved_quantity`), both cited against official Odoo 19.0 source (`master-blueprint-inventory-fulfillment.md` §A.4, accessed 2026-07-03). Whether the connector reads `free_qty` via location context or aggregates `stock.quant.available_quantity` directly, and whether a configurable default is offered, remain implementation planning. This row stays **open** until DEC-015 is accepted. | DEC-010 (semantic concept decided; source unverified) | Pushing the wrong Odoo quantity to Shopify `available` over/under-sells live stock | Official-doc verification (Odoo 19 source) — **Proposed partially resolved, Sprint C, pending DEC-015 acceptance** | Yes for the residual aggregation-mechanism/configurable-default detail; the source-field finding itself is proposed, pending acceptance |
| MBQ-33 | Exact **granularity of "first"** for the first-push guard (per-store / per-binding / per-variant-location), no coarser than per-store. **Carried forward, open — Sprint C proposes a recommendation** (2026-07-03): guard fires per **mapped Odoo-location ↔ Shopify-Location pair** (`master-blueprint-inventory-fulfillment.md` §A.5) — a recommendation for ChatGPT's direct decision, not self-accepted. | DEC-007 §4; DEC-010 | Fixes where the guard/confirmation record attaches | ChatGPT (Sprint C) | Yes |
| MBQ-34 | **Ongoing apply-mode** — auto-apply vs review-then-apply for post-first-push writes (C-INV-04). **Carried forward, open — Sprint C proposes a recommendation** (2026-07-03): review-then-apply as the Phase 1 default, consistent with DEC-003's "auto-apply not accepted as default" (`master-blueprint-inventory-fulfillment.md` §A.7/§G) — a recommendation for ChatGPT's direct decision, not self-accepted. | DEC-003; DEC-010 | Auto-apply was explicitly not accepted as default MVP behaviour; must be decided, not assumed | ChatGPT (Sprint C) | Yes |
...
| MBQ-36 | Exact **mutation choice per trigger type** (`inventorySetQuantities` preferred default vs `inventoryAdjustQuantities` for deltas). **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance:** direction proposed — `inventorySetQuantities` (compare-and-set) as the default for all trigger types; `inventoryAdjustQuantities` a candidate for narrower single-delta event-driven pushes only (`master-blueprint-inventory-fulfillment.md` §A.13/§G). Exact per-trigger choice, batching, and error handling remain open. This row stays **open** until DEC-015 is accepted. | DEC-010 | Compare-and-set vs delta semantics differ under concurrency | Implementation planning (Sprint C) — proposed direction, pending DEC-015 | Yes |
| MBQ-37 | **Shopify inventory webhook topic string(s)** — unverified in repo docs. **Proposed resolved by Sprint C (2026-07-03), pending DEC-015 acceptance, official-doc verification:** `INVENTORY_LEVELS_UPDATE` (plus `INVENTORY_LEVELS_CONNECT`/`INVENTORY_LEVELS_DISCONNECT`), confirmed against the official Shopify `WebhookSubscriptionTopic` enum (`master-blueprint-inventory-fulfillment.md` §A.9, accessed 2026-07-03). The underlying fact is verified; this row stays **open** until DEC-015 is accepted. | DEC-010; ar007-ar008-evidence-refresh | Webhook-driven import can't be built on an unverified topic; layered sync stands regardless | Official-doc verification (Sprint C) — **Proposed resolved, pending DEC-015 acceptance** | No pending fact verification; row formally closes on DEC-015 acceptance |
| MBQ-38 | Exact **first-push confirmation record schema** (what is persisted: preview snapshot, confirmer, source-of-truth, scope). **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance:** blueprint-level concept fixed — extends the Part A guard/audit record shape with a preview snapshot, confirming operator + timestamp, recorded source-of-truth, and scope (`master-blueprint-inventory-fulfillment.md` §A.5). Exact field names/schema remain implementation planning. This row stays **open** until DEC-015 is accepted. | DEC-010 | The guard's audit/idempotency anchor (DEC-009 layer) | Implementation planning (Sprint C) — proposed concept, pending DEC-015 | Yes |

## 6. Fulfillment (routed to Sprint C)

| ID | Open question | Source | Why it matters | Decision owner | Blocks implementation |
| --- | --- | --- | --- | --- | --- |
| MBQ-39 | Exact **Odoo tracking-reference field source** (carrier/tracking fields on `stock.picking`/delivery). **Proposed resolved by Sprint C (2026-07-03), pending DEC-015 acceptance, official-doc verification:** `stock.picking.carrier_tracking_ref` (Char), `carrier_tracking_url` (computed Char, via `carrier_id.get_tracking_link(picking)`), and `carrier_id` (Many2one to `delivery.carrier`), all defined in Odoo 19.0's `stock_delivery` module, cited against official Odoo source (`master-blueprint-inventory-fulfillment.md` §B.5, accessed 2026-07-03). **Surfaces new open question MBQ-60** (whether `stock_delivery`/`delivery` is a required Odoo dependency). This row stays **open** until DEC-015 is accepted. | DEC-011; ar007-ar008-evidence-refresh | Tracking write-back needs a verified source field | Official-doc verification (Odoo 19) — **Proposed resolved, Sprint C, pending DEC-015 acceptance** | No pending fact verification; row formally closes on DEC-015 acceptance (see MBQ-60 for the new module-dependency question) |
| MBQ-40 | Exact **backorder-to-picking linkage** fields/rules for sequential partial fulfillments. **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance:** `stock.picking.backorder_id` (Many2one, "Back Order of") and reverse `backorder_ids` (One2many) cited against official Odoo 19.0 source (`master-blueprint-inventory-fulfillment.md` §B.7, accessed 2026-07-03). The delivery-specific backorder-wizard UX/copy nuance flagged by `ar007-ar008-evidence-refresh.md` was not independently re-verified this sprint and remains open. This row stays **open** until DEC-015 is accepted. | DEC-011 | Each backorder picking is its own fulfillment event; linkage must be exact | Official-doc verification + Implementation planning (Sprint C) — proposed finding, pending DEC-015 | Yes, for the residual wizard-UX/copy detail and pending acceptance |
| MBQ-41 | Exact **notification-UI granularity** (global/per-store minimum decided; per-order override open). **Carried forward, open — Sprint C proposes a recommendation** (2026-07-03): a global/per-store default is sufficient for Phase 1 MVP; per-order override deferred to a later phase (`master-blueprint-inventory-fulfillment.md` §B.6) — a recommendation for ChatGPT's direct decision, not self-accepted. | DEC-007 §5; DEC-011 | Operator control surface for the notification guard | ChatGPT (Sprint C) | Yes (notification UI beyond the per-store default) |
| MBQ-42 | Exact **fulfillment location-confirmation mechanism** (core Shopify Location reference vs live FulfillmentOrder `assignedLocation` read, or both; live read treated as authoritative unless proven otherwise). **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance:** mechanism proposed — a live `assignedLocation` read is authoritative for a specific operation; the core Location reference is used only for naming/display and mismatch-detection, never as an override authority; a mismatch routes to the existing `ambiguous match` class (`master-blueprint-inventory-fulfillment.md` §B.8). Proposed, pending ChatGPT acceptance of the mechanism itself via DEC-015. | DEC-010/DEC-011 | Prevents fulfilling from a mismatched location without depending on inventory's mapping | ChatGPT (Sprint C) + Implementation planning | Yes |
| MBQ-43 | **Core Location reference cache policy** — stale-cache handling, refresh cadence, precedence vs live reads. **Proposed partially resolved by Sprint C (2026-07-03), pending DEC-015 acceptance:** precedence rule proposed — a live read always wins over the cache for a specific operation, cache refreshed on setup-readiness checks and the shared reconciliation cadence (`master-blueprint-inventory-fulfillment.md` §B.8). Exact refresh cadence/mechanism remains open. This row stays **open** until DEC-015 is accepted. | DEC-010/DEC-011; Part A §B.4 | A stale cache must never override live Shopify state for a specific operation | Implementation planning (Sprint C) — proposed rule, pending DEC-015 | Yes (fulfillment/inventory location checks) |
