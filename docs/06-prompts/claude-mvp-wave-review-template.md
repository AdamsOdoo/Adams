...
Record `Accept` only after all Tier 1/Tier 2 blockers are closed and required evidence is genuine. Tier 3 polish does not independently block merge. Post the complete decision and finding set verbatim to the PR, stating the exact reviewed SHA — never let the implementing session rewrite, summarize, or suppress it. Ready-marking and merge must be performed by a **separate top-level closure session** (or another explicitly authorized independent actor) — never the session/thread that implemented or reviewed the PR — after it independently re-verifies the exact accepted SHA and that the required evidence still corresponds to it. Merge with a normal merge commit into `mvp/program-integration` unless a separate accepted governance record says otherwise. Update `mvp-program-state.md` immediately after merge.
