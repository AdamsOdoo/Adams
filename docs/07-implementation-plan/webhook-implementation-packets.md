...
| --- | --- | --- | --- |
| **W1 — Core receiver + subscription management** | One HTTPS controller in core (`/shopify/webhook/<store>`), raw-body HMAC-SHA256 verify (401 on mismatch), fast-ack 200 within the 1 s/5 s budget (enqueue-only), `X-Shopify-Webhook-Id` dedup table, `X-Shopify-Event-Id` correlation logging, topic-registration seam (domains declare topic→job_type), shop-specific subscriptions via `webhookSubscriptionCreate` (branch A has no TOML pipeline — app-specific subscriptions become the Phase-2+ B-1 mechanism), subscription-state readiness check — W1 replaces (via `_inherit`) the core `webhook_hmac` readiness slot, which **Task CORE-R1 (D-R1-3, ownership moved from Area-6 D-A6-7 on 2026-07-11)** turns into a not-applicable pass while no webhook intake exists, with the real HMAC-configuration + subscription-state verification, delivery log with PII redaction (§3) | Yes (MVP tail) | Area 6 + U1 merged; W1 gate act |
