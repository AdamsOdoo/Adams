...
| Wave 5 | U1 Gate-A PR #194 is open/draft/unmerged at `b38e687` and must be refreshed after Wave 4 because it was derived from an earlier candidate. SEC-2 and accepted Wave 4 block U1 implementation. No Wave-5 implementation is authorized here. |
| Release dependencies | SEC-3 before external UAT/RC; inventory concurrency fixture-residue cleanup; PERF-0; Shopify dev-store provisioning; CI/first continuous full-suite run; final lifecycle/configuration docs; product-owner release sign-off. |
