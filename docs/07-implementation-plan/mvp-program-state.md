...
| #197 SEC-3 — 2026-07-27 delta | `[Corrected 2026-07-27]` **Still open.** Draft PR #204 extends implementation coverage over its own new surface — 2 models, 17 stored fields, 1 computed relation, 20 public/RPC methods, 14 elevated seams — inventoried in [`../03-architecture/sec3-company-isolation-audit.md`](../03-architecture/sec3-company-isolation-audit.md) §8, with a local two-company/two-role negative matrix over every one. One genuine cross-company defect was found and fixed doing it: a non-stored computed field whose contents record rules filter is cached once per record for the whole transaction unless it declares `depends_context`, so the first reader's result was served to the second in either direction. **Implementation coverage is not acceptance:** #197's independent Tier-1 security review and exact-SHA runtime evidence do not exist for that head. |
