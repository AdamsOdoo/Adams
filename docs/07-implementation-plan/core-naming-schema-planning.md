...
for MBQ-04 specifically, resolvable by an explicit descope rather than a
resolution. This document is that session.
...
- Serialization guard mechanism.
- Retry constants.
- Access/group XML ID planning.
- `odoo_event` trigger-origin mechanics.
- Credential persistence posture for slice 1 only (descope vs. resolution).
...
- Security CSV creation (row *shapes* are planned in §10; no CSV file is
  created).
- XML views.
- Real credential persistence, unless officially evidenced (it is not — see
  §11).
...
| Sync job | `shopify.connector.job` | Concrete | `shopify_connector_core` | The job/log/error/retry substrate (Part A §D); one record per logical sync operation, carrying current state, error class, retry counters, and the idempotency key (§8) | MBQ-01, MBQ-02, MBQ-19, MBQ-20, MBQ-21, MBQ-62 |
...
| Idempotency / operation guard | **Not a separate model** — fields + a uniqueness constraint on `shopify.connector.job` | — | — | The idempotency key and the serialization guard both operate on the same "is there already an unresolved job for this operation/target" question the job table already answers; a separate guard model would just be a shadow index of the job table (see §8) | MBQ-20, MBQ-21 |
...
| Credential lifecycle metadata | `shopify.connector.store.credential` | Concrete | `shopify_connector_core` | Kept as its own model (not fields on `store`) so field-level `groups` restriction and one-active-record-per-store rotation history (Part A §B.2) do not have to coexist with the busier, less security-sensitive store record; **the actual secret/token value has no proposed field** — see §11 | MBQ-04 (descope only — see §11) |

**Reading this table:** seven models are proposed (`store`, `store.settings`,
`location`, `binding.mixin`, `job`, `job.log`, `store.credential`); three
candidate "extra" models named in the audit's own checklist (error/
manual-review, idempotency/operation guard, audit/evidence payload) are
explicitly proposed **not** to exist as separate models, each with its
folding target stated. This is deliberately conservative — see §13 for why
this bound matters for later migration safety.
...
| `credential_ids` | One2many → `shopify.connector.store.credential` | No | — | — | Yes | Convenience reverse relation | MBQ-04 |
| `settings_id` | One2many → `shopify.connector.store.settings` (one active row) | No | — | — | Yes | Convenience reverse relation to the store's settings record | MBQ-07 |
...
| Field | Type | Required? | Index? | Unique constraint? | Readonly? | Purpose | Related MBQ |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `store_id` | Many2one → `store` | Yes | Yes | Yes (one settings record per store) | Yes after create | Store scoping (DEC-006) | MBQ-01/02/07 |
...
`_inherit` when that domain's own naming/schema pass runs. Field-level change
history for this model is proposed to rely on Odoo's native `mail.thread`
tracking (`tracking=True` on the relevant fields) rather than a bespoke
settings-history model, consistent with §13's anti-fragmentation goal.

### 4.3 `shopify.connector.store.credential`

| Field | Type | Required? | Index? | Unique constraint? | Readonly? | Purpose | Related MBQ |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `store_id` | Many2one → `store` | Yes | Yes | No (see constraint note) | Yes after create | Store scoping | MBQ-04 |
| `state` | Selection (`not_configured`/`present`/`invalid`/`revoked`) | Yes, default `not_configured` | No | No | Yes (system-written) | Credential lifecycle status (Part A §B.2) | MBQ-04 |
| `token_variant` | Selection (`non_expiring`/`expiring_rotating`) | No | No | No | Yes | Which DEC-004 token variant this credential uses, once MBQ-05 is decided | MBQ-04 |
| `granted_scopes_snapshot` | Text | No | No | No | Yes | Last-verified granted-scopes snapshot (not the secret itself) | MBQ-04 |
| `last_validated_at` | Datetime | No | No | No | Yes | Last successful validation timestamp | MBQ-04 |
| `active` | Boolean | No, default `True` | Yes | Partial unique (`store_id` where `active`) | Yes (system-written) | Marks the current credential; rotation creates a new row and sets the prior row `active=False` rather than editing history in place (Part A §B.2) | MBQ-04 |

**No field for the actual secret/token value is named, typed, or created by
this proposal.** See §11 — this is the explicit MBQ-04 descope. If ChatGPT
later resolves MBQ-04 for real credential persistence, the resolved field
(name, type, and storage/encryption mechanism) is added to this model by that
future decision, not invented here.

### 4.4 `shopify.connector.location`
...
### 4.5 `shopify.connector.binding.mixin` (abstract)
...
(§4.6) — a job needs to reference an arbitrary source record generically,
...
| `job_type` | Selection (extensible via `selection_add`; core registers none itself) | Yes | Yes | No | Yes after create | Domain-registered job-type/handler-routing value (Part A §A.5.2); also doubles as the idempotency key's "operation type" component (§8) — no separate `operation_type` field is proposed | MBQ-01/02, MBQ-19, MBQ-20 |
...
| `idempotency_key` | Char, computed + stored | Yes | Yes | Yes, with `store_id` (composite) | Yes | Composed from `store_id` + `job_type` + `res_model`/`res_id` + `shopify_target_gid` + `payload_hash` (§8) | MBQ-20 |
...
### 4.7 `shopify.connector.job.log`

| Field | Type | Required? | Index? | Unique constraint? | Readonly? | Purpose | Related MBQ |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `job_id` | Many2one → `job`, `ondelete='cascade'` | Yes | Yes | No | Yes after create | Parent job (one2many from job) | MBQ-19 |
...
- **Settings live on a separate model, not the store model** — see §3's
  rationale (cleaner extension seam, keeps the security-adjacent store
  record smaller).
...
- **Retry-eligibility fields:** `retry_count` and `next_retry_at` on `job`
  (§4.6); which error classes are auto-retryable at all is **not** a schema
...
- **Exact `job_source` selection values:** `webhook`, `manual_sync`,
  ...
  `shopify.connector.job.job_source` (§4.6), matching DEC-009's original six
...
- **Retry-origin event identity/timestamp:** `trigger_origin_event_ref` and
  `trigger_origin_event_at` (§4.6) satisfy DEC-019's audit requirement that
...
Resolving MBQ-20 and MBQ-21 at planning-proposal level:

- **Exact operation-level idempotency key components:** `store_id` +
  `job_type` (doubling as "operation type" — no separate field) +
  `res_model`/`res_id` (source record) + `shopify_target_gid` (Shopify
  target, where known) + `payload_hash` (payload version/hash) — matching
  Part A §D.6's conceptual tuple
  `(store, operation type, source record, Shopify target ID where known,
  payload version/hash)` field-for-field.
- **Field names/types:** all on `shopify.connector.job` (§4.6):
  `job_type` (Selection), `res_model` (Char), `res_id` (Integer),
  `shopify_target_gid` (Char, nullable), `payload_hash` (Char, nullable),
  `idempotency_key` (Char, computed + stored from the preceding five).
- **Uniqueness scope:** `(store_id, idempotency_key)` — a job for the same
  operation, same target, same payload version, in the same store, is the
  same logical operation; a genuinely different payload (different
  `payload_hash`) naturally produces a different key rather than colliding.
...
- **Serialization guard mechanism:** a query-time guard owned by
  `shopify.connector.job` itself — before enqueuing a new job, check for an
  existing job with the same `(store_id, res_model, res_id, shopify_target_gid)`
  tuple in a **non-terminal** state (`draft`/`queued`/`running`/
  `retry_waiting`/`failed_retryable`/`blocked_manual_review`); if found, the
  new operation is either **blocked** (queued behind the existing job, per
  Part A §D.7) or **superseded** via the existing `superseded_by_job_id`
  pointer (§D.9), never dispatched in parallel with the unresolved prior
  operation.
- **How unresolved ambiguous/manual-review operations prevent conflicting
  dispatch:** because the guard checks job **state**, not just existence, a
  job sitting in `blocked_manual_review` or `retry_waiting` for a given
  target continues to block a new operation against that same target until
  it reaches a terminal state or is explicitly superseded — satisfying Part
  A §D.7's "serialized while a prior operation against that target is
  unresolved" rule without a separate lock table.
- **Which model owns the guard:** `shopify.connector.job` — no separate
  "operation guard" model is introduced (§3).
- **Connector-side idempotency is permanent, not assumed platform-covered:**
  this key layer prevents *connector-side* duplicate processing only; it
  does not, by itself, make a non-`@idempotent` Shopify mutation safe to
  resend after an ambiguous outcome — the ambiguous-outcome rule (Part A
  §D.7, unchanged, already accepted) still requires a verification read or a
...
- **Which groups can read/write core store/config/job/log/error records**
  ...
  | `shopify.connector.store.credential` (metadata fields only — see §11) | Read (masked) | Read (masked) | Read (masked) | Read + Write (masked; no read-back of any future secret field, per Part A §J.2) |
  | `shopify.connector.location` | Read | Read | Read | Read — no manual Create/Write/Unlink for any group; the cache is system-maintained (§4.4) |
  ...
  **No group is granted Unlink on any core model** — jobs, logs, bindings,
  and audit records are never user-deletable, matching §12's "no silent
...
- **Option A (descope) is adopted.** The first core slice creates
  `shopify.connector.store.credential` (§4.3) as a schema home for
  credential **lifecycle metadata** — status, token variant, granted-scopes
  snapshot, last-validated timestamp, active/superseded rotation history —
  but **no field name, type, or storage/encryption mechanism is proposed or
  created for the actual secret/token value.** No real Shopify token is
  accepted, stored, or transmitted by this model as proposed.
- **What is, and is not, resolved:**
  - **Proposed resolved by descope only:** the schema *shape* for
    credential lifecycle metadata that does not require holding a real
    secret — `store.credential`'s six metadata fields (§4.3) can exist,
    be created, and be exercised (state transitions, rotation history) with
    zero real tokens involved.
  - **Still open, not resolved:** the exact field name, type, and
    storage/encryption mechanism for the actual secret/token value. Real
    token storage remains blocked until either (a) ChatGPT reviews official
    Odoo documentation/source evidence on field-level encryption-at-rest and
    decides a mechanism, or (b) some other explicit ChatGPT decision resolves
    MBQ-04 for real persistence.
...
- **Uniqueness:**
  ...
  - `shopify.connector.store.settings.store_id` — unique (one settings row
    per store).
  - `shopify.connector.store.credential` — partial unique on `store_id`
    where `active = True` (one active credential per store; history
    preserved via `active = False` rows, never deleted).
...
- **Foreign keys:** every non-`store` model above carries a required
  `store_id` Many2one; `shopify.connector.job.log.job_id` is
  `ondelete='cascade'` (log rows are meaningless without their parent job);
  every other `store_id` foreign key is `ondelete='restrict'` (a store
  record is never deletable while jobs/logs/bindings/credentials reference
  it — disconnect uses `state`, not deletion, per MBQ-08).
...
- **Binding uniqueness:** deferred to each concrete domain binding model
  (out of scope), but the abstract mixin's own fields (§4.5) are designed so
  every concrete model can enforce the composite uniqueness without adding
  new mixin fields later.
- **Job duplicate prevention:** the serialization guard (§8) plus the
  idempotency-key uniqueness constraint together prevent both duplicate
  *connector-side processing* (same key) and *conflicting concurrent
  dispatch* (same target, unresolved prior job).
- **Selection constraints:** `trigger_origin` required iff `job_source =
  'odoo_event'` (§7); `manual_review_subreason` required iff `state =
  'blocked_manual_review'` (§4.6).
- **No silent destructive history deletion:** no `unlink` access is granted
  to any role on `job`, `job.log`, or `location` (§10); disconnect and
  domain-disable both use state fields, never deletion, matching Part A
  §I.4/§B.1's already-accepted "disabling/disconnecting must not delete
  history" rules.
...
- **No one giant module, no over-fragmentation.** Seven models cover the
  full core substrate the audit named; three candidate extra models
  (error/manual-review, idempotency/operation-guard, audit-payload) are
  explicitly folded rather than added, keeping the schema from
  over-fragmenting (RA-012 pattern) while still giving every domain module a
  single, shared substrate to build on (RA-013 pattern, avoiding duplicated
  job/log/error abstractions per domain).
...
- **MBQ-01:** *Proposed resolved pending ChatGPT acceptance* — exact Odoo
  ...
  `.store.credential`, `.location`, `.binding.mixin`, `.job`, `.job.log`)
  proposed in §3. Domain-specific binding model names (MBQ-55) and
  view/menu XML IDs (MBQ-03) remain out of scope and unresolved by this
  pass.
- **MBQ-02:** *Proposed resolved pending ChatGPT acceptance* — field
  names/types for the seven models above proposed in §4, including
  constraint/index design (§12).
- **MBQ-04:** *Proposed partially resolved pending ChatGPT acceptance, by
  explicit slice-1 descope only (Option A) — not resolved for real
  credential persistence.* A `shopify.connector.store.credential` model is
  proposed for lifecycle metadata only (§4.3); no field, type, or
  storage/encryption mechanism is proposed for the actual secret/token
  value. Real credential storage remains open (§11), pending official Odoo
  evidence and a separate ChatGPT decision.
...
- **MBQ-21:** *Proposed resolved pending ChatGPT acceptance* — the
  serialization guard is proposed as a query-time guard owned by
  `shopify.connector.job` (§8) — checking for an existing non-terminal job
  against the same `(store, res_model, res_id, shopify_target_gid)` tuple —
  not a separate model or a queue-level lock.
...
- **All first code** — no Odoo module, model file, view, controller,
  ...
  this pass.
- **Real credential persistence** — MBQ-04's actual secret-storage
  mechanism remains open (§11).
...
**Recommend: accept as proposed**, with three specific judgment calls flagged
for particular scrutiny, since each is a genuine design choice this document
made rather than a mechanical restatement of already-accepted content:
...
1. **Job+log split** (§6, MBQ-19) — chosen over a single mutable job model.
   ...
   with no separate `job.log`), that is an **accept with changes** on §3/§4.6/§4.7
   specifically, not a rejection of the rest of this document.
2. **`enqueue_decisions` as a generic JSON field** (§4.6, §13) rather than
   ...
   changes** on §4.6 only.
3. **A `store.credential` model created now, ahead of MBQ-04's real
   resolution** (§3, §4.3, §11) — chosen so credential *lifecycle* schema
   (state, rotation history) does not have to wait for the secret-storage
   mechanism itself. If ChatGPT prefers deferring even the metadata model
   until MBQ-04 resolves in full, that is a narrow **accept with changes**
   removing `store.credential` from this pass's model list until a later
   session.
