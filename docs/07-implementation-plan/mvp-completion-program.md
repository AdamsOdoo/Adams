...
**Program-specific stop condition (11):** SRR-03 itself remains OPEN (not merely: the "CLOSED" vs. "OPEN" documentation contradiction — DEC-033 is Accepted and resolves that wording conflict, but does not itself close the risk) and Wave 2 is about to merge, be enabled, or receive live Shopify validation before Wave 1's SRR-03 closure evidence (§4 Wave 1) is accepted. Sol must escalate rather than treat the wording reconciliation as equivalent to risk closure.
...
2. **SRR-03:** evidence-backed status is OPEN; DEC-033 (Accepted) records the formal reconciliation, Task 012 read-replay classification, and Wave 1 closure sub-gate. Pending: later runtime closure evidence (Wave 1's own deliverable); this is separate from mutation-domain Layer 2.
