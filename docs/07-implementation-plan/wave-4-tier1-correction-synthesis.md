...
- **Decision-lock update (2026-07-23):** the exact central-vs-caller transaction architecture (savepoint ownership, caught-exception tuple, re-search/recovery contract, caller-side narrowing) is now frozen in [`wave-4-tier1-decision-lock.md`](wave-4-tier1-decision-lock.md) Decision D — savepoint/catch/re-verify is owned once, centrally, inside `_enqueue_once`, not duplicated at each of the 8 call sites.
...
**Decision-lock update (2026-07-23):** confirmed and made explicit — [`wave-4-tier1-decision-lock.md`](wave-4-tier1-decision-lock.md) Decision A: does not block PR #189 merge; blocks external UAT and MVP release-candidate acceptance; owner is a new dedicated task (candidate "SEC-3"), not SEC-2 (SEC-2's own allowed-files/forbidden-files scope, verified by direct read, excludes job/dispatcher/DEC-031-Layer-2 code); frozen target architecture is an `ir_attachment.py`-style `_check_access`/`_search` delegation override, not a stored `company_id` field (no single ORM-traversable relation exists for the polymorphic `res_model`/`res_id` shape).
...
- **Decision-lock update (2026-07-23):** the exact value is now frozen — [`wave-4-tier1-decision-lock.md`](wave-4-tier1-decision-lock.md) Decision C — `external_fulfillment_observed` / "External Fulfillment Observed"; no product-owner naming placeholder remains. `review_reason` count moves 20 → 21.
...
- **`W4-R-P2-007` (do not code-fix the permanent mechanism in this pass):** would-be files are `_c8_location`, `_quantity_compatible_pickings`, `_select_deterministic_picking` in `shopify_connector_fulfillment_mode2.py` — but the fix must **not** read `shopify.connector.location.mapping` (violates DEC-008/DEC-011/DEC-038 Q3-RULED) and must **not** add Odoo-location storage to `shopify.connector.location` (violates its documented invariant). Escalate to the architecture-review log / a DEC amendment closing DEC-011's still-open cross-check-mechanism item.

**Decision-lock update (2026-07-23):** [`wave-4-tier1-decision-lock.md`](wave-4-tier1-decision-lock.md) Decision B freezes the permanent architecture (a new core `shopify.connector.location._resolve_odoo_location()` seam overridden by `shopify_connector_inventory`, called by name only — still a separate future task, not this pass) **and** authorizes a narrow interim correction inside this PR's own scope: `_c8_location` must always fail closed to `location_unmapped` for the warehouse-cross-check dimension (no real evidence exists yet) rather than silently succeeding once the Shopify location merely resolves. The interim item is now part of this correction batch (see the updated locked prompt).
...
See [`wave-4-tier1-correction-locked-candidate.md`](../06-prompts/wave-4-tier1-correction-locked-candidate.md), now normalized against the four decisions frozen in [`wave-4-tier1-decision-lock.md`](wave-4-tier1-decision-lock.md) (2026-07-23). **Use of that prompt is not authorized until the control room accepts this synthesis and the decision-lock document.**
