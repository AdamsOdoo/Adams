...
roles/access, and data retention — and where a Connector User can
...
| (d) Roles & access | Two role cards with capability summaries and cannot-do lines; the single-dropdown user-form mock ("Shopify Connector: [User ▾]" — one selection, options empty/User/Administrator); a raw-PII access note (both roles read the raw operational PII their role permits — **no PII masking, no unmask toggle, no separate PII tier in the MVP**) | [`connector-roles-and-permissions.md`](../../02-product/connector-roles-and-permissions.md) §1.1/§1.2/§3/§4.3 [Proposed] |
| (e) Retention & privacy | A documented data-retention & erasure policy (retention window, erasure-request handling), a redaction reminder (logs/audit/errors/headers redacted at write time — mandatory), and the audit note (who/when/old→new; logs append-only + redacted at write). **PII masking is not an MVP capability (post-MVP privacy enhancement).** | Roles doc §3 (PCD minimization posture); DEC-009 audit posture |
...
| Assign roles | No | Yes (every change logged) |
| Retention & privacy policy | No | Yes (changes audited; no PII-masking action in the MVP) |
...
5. Tab (d) Administrator — role cards, user-form dropdown, raw-PII access note.
6. Tab (e) Administrator — documented retention & erasure policy.
...
- Two-role split, single privilege dropdown on the user form, and raw
  operational PII readable by both roles per permitted operations — **no PII
  masking, no unmask toggle, no separate PII tier in the MVP** (product-owner
  ruling 2026-07-16; SEC-2 two-role & PII-simplification packet): roles doc
  §1/§3/§4.3 [Proposed]. Log/audit/error/header redaction of PII stays
  mandatory [Fact — repo]; PCD minimization posture [Inference, roles doc §3].
