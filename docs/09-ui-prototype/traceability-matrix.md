...
| `matching-center/` | **S6/S8** (DEC-006; DEC-012 §6; Part D §11) | Recovery (flow 3) | DS §2 (selective Owl), §11 | binding → **exact normalized email** → manual review; **one active exact-email → auto-bind; valid email + no active/archived → auto-create; ambiguity/archived-only/missing-email/conflict → manual review** (merged Task 011); `blocked_manual_review → danger` | Match-key order; **the automatic bind/create outcomes** (Task 011); evidence-first; audit | Confidence-in-words; **binding-key/advisory evidence marks (no fuzzy)**; **success/audit presentation of the two automatic outcomes (`is-linked` card, green outcome chips, navigation-only actions)**; ambiguity distinguished from technical error **within the danger family** by icon/owner/copy | U3 (Owl) |
...
| `odoo-native-exemplar/` | **S4 + job form** (SPEC screens 8–10; DEC-012 §4) | Daily check / recovery | DS §2 (PD-7) | 5 states; row-level exceptions; **`blocked_manual_review` rows = danger** (review `4950432754` §2) | S4 list; fixed vocabularies as text; smart buttons; **stays Odoo-native** (PD-7) | Restrained token-layer look on standard views | U1 |
...
| P6 | Confidence-in-words + evidence-table (**binding-key vs advisory** marks, no fuzzy verdict); **success/audit presentation of the two automatic Task-011 outcomes** (auto-bind / auto-create) as non-decision screens, manual review reserved for ambiguity | matching | Presentation (aligns to merged Task 011; review `4950432754` §1) |
| P7 | 4-column diff layout with inline source-of-truth + protected-field treatment; **blocked state asserts no authority / computes no result** | product-diff | Presentation |
