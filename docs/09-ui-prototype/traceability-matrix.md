...
| `matching-center/` | **S6/S8** (DEC-006; DEC-012 §6; Part D §11) | Recovery (flow 3) | DS §2 (selective Owl), §11 | binding → **exact normalized email** → manual review (email sole auto key; name/phone advisory); `blocked_manual_review → danger` | Match-key order; evidence-first; blocking preview; audit | Confidence-in-words; **binding-key/advisory evidence marks (no fuzzy)**; 4-decision set; ambiguity distinguished from technical error **within the danger family** by icon/owner/copy | U3 (Owl) |
...
| P6 | Confidence-in-words + evidence-table (**binding-key vs advisory** marks, no fuzzy verdict) + 4-decision matching layout | matching | Presentation |
...
| P12 | **Manual review = danger family** (follows the §6 token map `blocked_manual_review → danger`) with the **§11 “warning family” wording corrected** to match | dashboard, matching | Semantic-map alignment (review `4950255482` §5) |
