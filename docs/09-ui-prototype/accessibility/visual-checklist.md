...
| **V-4** | State never by color alone; all five states designed per surface | **Pass** | Every state carries a **word** (band sentence, status/owner chip) + icon; color only reinforces. Manual review (`blocked_manual_review`) is the **danger** family on **every** surface that shows it — `dashboard-manual-review-1366.png`, `matching-ambiguous-1366.png`, **and** the `native-list-{1366,768,375}` Products “Needs review” + Customers “Waiting on a decision” rows (review `4950432754` §2) — yet distinguished from technical error **without color** (hand icon + reviewer owner + “not a system failure” vs triangle + system owner + “try again”: vs `dashboard-error-1366.png` & `matching-error-1366.png`). The two **automatic** matching outcomes carry their state in words via green “Linked/Created automatically” chips (`matching-single/none-1366.png`). Dashboard ships all five states; every other spec defines all five. | — |
...
| **V-6** | One primary action per screen; destructive styling + preview | **Pass** | Exactly one filled-accent primary per state; secondaries outlined; high-impact changes never pre-selected — `product-diff-blocked-1366.png` leads with the **fixer** (“Set price source of truth”), shows **no computed price and no Confirm**. The **automatic** matching outcomes present **no decision buttons** — navigation only (Open contact / View audit trail / Back); **manual review** has a single primary “Link” that stays `aria-disabled` until one candidate is chosen. | — |
