...
1. **Dashboard hierarchy** — accept/revise the ranked §9 implementation
   (lead band → ≤3 exceptions → quiet chips → activity) replacing the
   nine-equal-card grid.
...
5. **Matching-center composition** — accept/revise confidence-in-words, the
   evidence-table layout, the four-decision set, and the ambiguous-vs-technical-
   error visual split.
6. **Product diff/preview composition** — accept/revise the 4-column diff,
   inline source-of-truth, protected-field treatment, and blocked-state posture.
7. **Odoo-native token treatment** — accept/revise how restrained the connector
   token layer is on standard Odoo list/form views.
8. **Responsive & RTL behavior** — accept/revise the mobile reflow (stacked
...
| CSS syntax | brace-balance check on `prototype.css` | **Balanced** (266 `{` / 266 `}`). |
...
| PNG evidence | dims + sizes over 34 files | **34 PNGs, 8.13 MB.** CSS widths 1366 (×24), 768 (×5), 375 (×5); rendered at 2× (2732 / 1536 / 750 physical). |
| Contrast | WCAG luminance formula, 32 pairs | **All 32 pass** their SC 1.4.3 / 1.4.11 threshold (contrast-table.md). |
| Viewport rendering | headless Chromium at 1366/768/375 + RTL | All screens render without horizontal page scroll; mobile tables reflow; RTL mirrors. |
| Handoff structure | append one top entry only | One new top entry added; older entries untouched. |
