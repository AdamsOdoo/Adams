...
- **Branch / PR:** current session branch
  (`claude/task-003-static-validation-cszl88`); PR → opened as **draft** into
  `Shopify-connector`, not merged, not marked ready. Docs-only session — no
  code, test, manifest, security, XML, or CSV change of any kind. **No Task
  004 work of any kind in this session. No OAuth implementation. No
  setup-wizard change.**
- **Files changed:** `docs/05-qa/task-003-static-validation-sweep.md` (new),
  ...
  (this entry; corrected the entry below's stale "draft PR, not merged"
  wording for PR #108, which this session confirmed is merged, merge commit
  `76cd74e7cf585e1925c3ac280bd6ca97c55df7ab`).
- **What changed / residue fixed:** This session ran a static/offline-only
  ...
  that do not require a valid Shopify Admin API token, explicitly to run in
  parallel with (and without touching) a separate, concurrently-running
  session performing the Fable/manual-OAuth-token-acquisition experiment.
  Confirmed by direct repo/source inspection: **VAL-A4** — Task 003 (PR #101,
  merge commit `e27f10e55f3504d1a9b8871a207b3d9762a3c783`) introduced no XML
  file, menu, action, wizard, controller, route, cron, or server/scheduled
  action of any kind; the module's only XML file
...
- **Items deferred:** Everything requiring a live Odoo instance, a live
  ...
  experiment (tracked in a different, concurrently-running session) is
...
- **What remains blocked:** Task 003 manual validation **remains
  incomplete**. Task 004 **remains blocked**. VAL-B2 and the OAuth/
  token-acquisition open question are unchanged by this session and are
  being handled by a separate, concurrently-running session — not duplicated
...
- **Next recommended session:** ChatGPT reviews this static/offline sweep
  alongside the separate OAuth-experiment session's findings; a future live
  session (with an actual Odoo 19 instance, PostgreSQL, live Shopify dev
  store, and server-log access) is still required to close VAL-A1, VAL-A4's
  DB half, VAL-B2, VAL-B4–B7, VAL-C1's server-log half, VAL-D1/D2's live
  halves, VAL-E1, and VAL-G1–G4 before Task 003 can be marked complete.
- **Stop condition:** stopped at the scoped boundary — docs-only static/
  ...
  OAuth work performed or claimed.

### Shopify Token-Acquisition Research — compact handoff (2026-07-07)

- **Branch / PR:** `claude/shopify-token-acquisition-research-lpi7jt` → **PR
  #108**, merged into `Shopify-connector` (merge commit
  `76cd74e7cf585e1925c3ac280bd6ca97c55df7ab`). Docs-only session — no code,
  test, manifest, or security-file change. **No Task 004 work of any kind in
  this session.**
