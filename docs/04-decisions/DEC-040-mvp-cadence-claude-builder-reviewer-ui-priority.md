...
2. **Independent-review mechanism (this is how "Claude builds and reviews" stays safe).** DEC-039's no-self-acceptance rule is unchanged and is satisfied by either:
   - a distinct top-level Claude Code session (a different conversation) reviewing the PR from scratch; or
   - within one session, a **fresh subagent invocation** (via the `Agent` tool) that receives only the PR diff, the wave's acceptance criteria, and `claude-mvp-wave-review-template.md` — with no memory of why the implementing thread made its choices — explicitly instructed to adversarially re-verify, not summarize or rubber-stamp.
...
5. **Non-negotiable safeguards for large-batch delivery — the explicit answer to "don't blow up development":**
   - Every batch ships with full automated test coverage for what it adds/changes, **plus genuine Odoo.sh runtime evidence** (build ID, fresh-install result, focused-class results — never a static-only claim), before the independent review even begins.
