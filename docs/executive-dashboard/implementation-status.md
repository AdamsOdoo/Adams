...
**BLOCKED ON DEVELOPMENT BUILD — Finance/Inventory visual gate NOT PASSED.** This section supersedes the historical candidate identities below. Recovered PR HEAD was `91bbf98877b63892a4d6483c63ab9611a15f1b0d`; the initial checkout had no unpublished changes. Existing later fixes were preserved. PR #214 remains draft, open and unmerged; production/main were not changed.
...
- Current application source **`638af7eabd0186612856b7df8d9b59fd0c3d7506`** / development build **38534410** also failed with Odoo.sh **Platform error**. It uses module versions **19.0.1.7.0**, full signed Finance amounts without unnecessary zero decimals, approved bar-chart markup, and source-backed margin/budget notes. It requires a successful native run and paired review. Local frontend checks remain65/65, XML parsing and Python compilation pass; these do not establish rendered correctness. The previous tested application build38533011 does not contain this later presentation change.
...
The immutable [approved HTML and adjustment record](reference/README.md) are now checked in outside addon assets. `scripts/dashboard-visual-compare.py` generates independent pairs, overlays and diffs and rejects mismatched capture metadata. Its synthetic checks passed for identical pixels, a visible change, and state-mismatch rejection. **The comparison tool has not yet established Finance/Inventory parity.** The native capture test is an initial diagnostic for the populated Finance region only, not a completed state matrix; remaining reference-state normalization, drawers and Inventory capture are unresolved. The native test fixture is transaction-scoped, uses actual Owl/Odoo assets and is absent from normal application requests. Running a second native Chromium test process alongside the live development server exceeded that container's process limit; those ad-hoc test processes ended and the temporary test file/import were removed. Use the supported build test process.
