...
Native build38534772 (`b1d92f77`) produced the first populated Finance pair but failed one browser test: a print assertion reused the new card title instead of the report label. Build38535653 (`08c8d486`) produced corrected reference crops and exposed the equivalent PDF assertion. Both assertions are corrected in `b9a9cbb076700c4210b2143696c0336a216974a7`; its build38536432 completed with Odoo.sh done/Warning, not Failed. The later application candidate is `ea4d1ab4c33cb24f3e394ae23014c17c6ebb9575`, development build38536975; that native run is still pending at this checkpoint. Earlier platform errors are historical, not the current blocker.

The captured Finance comparison exposed heading line-height, source-button padding, margin-note and context-card differences. The branch now explicitly scopes heading/button inheritance to Finance, ports the approved context-card structure, and restores the currency/partial-month chart caption. These latest changes require native and rendered verification. Local frontend checks:65/65; XML parse and Python compile passed. No visual pass is claimed.

The native Finance capture is diagnostic: matching warning/source states, complete Finance drawers/lower groups and Inventory coverage remain unresolved. Current pairs have been inspected privately, but no complete matched-state overlay/diff matrix is qualified. `scripts/dashboard-visual-compare.py` preserves independent baselines, rejects mismatched metadata and masks, and retains original dimensions. Geometry differences generate padded diagnostic canvases and force review; no resizing or percentage threshold can waive them.
