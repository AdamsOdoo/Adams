...
### Native RTL closure candidate after 81e76b2

The approved margin-note LTR isolation is preserved. Native Odoo uses `body.o_rtl` without a DOM `dir` attribute, so dashboard action-arrow selectors now recognize that standard shell class; keyboard tabs read their rendered direction. Finance's Arabic subtitle uses the approved prototype wording. The reference's UI27 content adapter now isolates localized dates with Unicode LRI/PDI, preserving chronological ranges in surrounding Arabic (no CSS, sizing, or baseline image replacement). It permits the unchanged proper report name “Executive Summary” and the untranslated authoritative bucket label “Not overdue” when translating otherwise covered compound captions. Reference date isolation is explicit and independently generated; it is not a pixel-difference waiver.

The 81e76b2 narrow header and working-capital overlays were inspected: the previous extra mobile-menu slot, wrapped card headers and wrapped report links are resolved. RTL arrow/caption corrections and Inventory control typography still require the next native capture. Finance/Inventory gate remains open.

### Focused regression matrix

The functional browser journey retains all seven existing widths and both languages/appearances in eight representative cases, rather than 28 repeated combinations. English Light covers320,1366,1440,1920; English Dark768; Arabic Dark390,1440; Arabic Light1024. All journey assertions remain; the special1440px English/Arabic export and report-return paths remain exercised. The five independent reference-comparison cases remain unchanged. This follows the owner's representative-coverage instruction and does not replace screenshot comparison with test counts.

### Measured remaining control corrections (dc78359/38564209)

Rendered manifest inspection confirms the Inventory date input is36px high in Odoo versus37.5px in the reference, while its font, line-height, padding, width and position match. Native stylesheets explicitly reset WebKit date-segment padding/display. The candidate restores those browser defaults only within Inventory, rather than adding an arbitrary height. The reference uses tabular digits while Odoo form controls reset to normal; reference-surface controls now inherit the dashboard's tabular digits. Apply/Clear use the approved flex alignment; the sort select radius is6px rather than Bootstrap's4px. Finance period captions preserve the reference's explicit LTR isolation; source drawer dates are explicitly LTR and aging labels preserve the approved contiguous text order. These corrections require fresh comparison; no differences are waived.

### Navigation coverage and representative-run correction

1069161/38565264 produced the five controlled cases. Its RTL overlays show the measured input/button/tabular-digit corrections and Finance date/aging ordering resolved. The build is markedFailed: the reduced functional matrix still had a legacy assertion requiring four PDFs despite selecting two1440px PDF cases. The assertion now checks the exact filenames for every selected1440px case; both languages remain covered, along with the existing content/page checks. Native qualification must rerun; the failed build is not accepted.

Finance/Inventory's sidebar navigation and opened More menu are added to paired capture coverage. The approved department/search/info icons are copied directly from the immutable SVG definitions, and More uses the approved five actions/order; restoration remains inside Saved views. Sidebar review-only footer content is excluded from the navigation-region pair, with standard company identity retained. These scoped changes await capture and review before shared propagation.

## Navigation and empty-state closure — 23 September

Development fe949be52c439cf301202f45b94e802b3300e18d /38565860 completed with Odoo.sh Warning; exact final native aggregate remains to be retrieved. Independent1440px sidebar/menu captures identified missing department SVG geometry and muted menu text. The SVG extraction incorrectly selected labels outside the immutable ICON map; extraction is now bounded to that map. The reference navigation crop now includes the Quick access group (the prior last-of-type selector selected only the department nav). This corrects evidence coverage, not baseline styling.

The1920px Inventory empty-state pair also exposed a15px width shift when the native scrollbar disappeared and an unstyled Clear button. Scoped stable scrollbar space and the exact approved empty-state button style correct these defects. Stock action icons use the approved18px size. Native rendering of these corrections is pending; the Finance/Inventory gate remains OPEN.69 focused frontend tests pass. No new source calculation or access-control change.
