...
- Owner follow-up: the app icon is the original Adams Executive Dashboard icon again (same file as `adams_executive_dashboard`); an account opens the General Ledger filtered on it (the ledger's search set to the account code and the account unfolded), from Bank & Cash, receivables/payables and the profit panels. Verify on Odoo.sh that only that account is listed with its entries.
