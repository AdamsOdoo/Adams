...
| 12 | Fulfillment/tracking updates | Task 014 (revised) + operating modes + status model implementation | Wave 4 (Mode 2: Wave 5/stretch) | Odoo.sh green; [`../05-qa/fulfillment-mode-uat-matrix.md`](../05-qa/fulfillment-mode-uat-matrix.md) executed; correct merchant-managed scopes evidenced |
...
| 22 | **Dev-store UAT evidence (VAL-B2)** | **Dev-store credentials/access not provisioned — this is a hard stop: zero live Shopify calls have ever been made; every UAT matrix in `../05-qa/` is blocked on it.** DEC-028 posture also required before any real PII | Wave 6 (provisioning is a product-owner action, needed earlier for per-wave dev-store proofs from Wave 2 on) | VAL-B2 executed; per-domain UAT scenarios ([`cod-uat-matrix.md`](../05-qa/cod-uat-matrix.md), [`fulfillment-mode-uat-matrix.md`](../05-qa/fulfillment-mode-uat-matrix.md), [`reconnect-backfill-uat-matrix.md`](../05-qa/reconnect-backfill-uat-matrix.md)) executed; no token/PII leakage in the post-run scan |
