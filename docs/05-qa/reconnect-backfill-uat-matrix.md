...
| UAT-RB-2.6 | Inventory — no blind push | Pre-disconnect pending push targets exist; remote quantities changed while disconnected | First post-reconnect inventory action is a reconciliation **read** of all mapped pairs (store-identity check first, DEC-036 D18); pushes resume only with fresh `changeFromQuantity` bases [corrected 2026-07-19, Gate B — field renamed from `compareQuantity`, see DEC-037]; no pre-disconnect computed quantity is pushed; unexplained drift found during the read routes to review, never a blind push-over (DEC-037 §1 item C6) |
