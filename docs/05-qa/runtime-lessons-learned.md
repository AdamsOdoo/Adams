...
| LL-006 | Independent worker cursors deadlock under Odoo tests. | The main `TransactionCase` lock was not released. | Generic Python threading review omitted the Odoo 19 test-lock contract. | Every threaded independent-cursor test uses one `release_test_lock()` runner. | `TestMutationConcurrency._run_workers` and its AST guard. | Task 013 and every concurrent wave. | enforced |
| LL-007 | Worker exits silently, remains alive, or contaminates later tests. | Shared lists and unbounded coordination hid outcomes. | Tests asserted aggregate results without one report per worker. | Named workers use dedicated channels, bounded joins, exact outcomes, and zero-residue cleanup. | Central runner, worker-channel assertions, cleanup verification. | Task 013 onward. | enforced |
...
- [ ] Route every independent-cursor thread through `release_test_lock()`.
- [ ] Require one named result per worker, bounded termination, named retry
