...
| token waste (#17) / unclear handoff (#16) | 2 | **2nd (unclear handoff, DP-002 + DP-007) — checklist/rule updated.** Phase-exit criteria + a documentation-maintenance rule were added to `quality-feedback-loop.md` §10–§11 (`[Recommendation]`) and pointed to from `CLAUDE.md`. |
...
| DP-007 | 2026-07-02 | Control-Room Reset Sprint 1 | Multiple product/research docs kept asserting superseded "current truth" (e.g. "No MVP scope is finalized," TeqStars "403-blocked") **after** a later session (DEC-003 acceptance; the Sprint C2 TeqStars rebaseline) had already superseded them; `research-backlog.md` marked ~13 completed items `Not started`; two folder READMEs claimed to be "Empty" despite containing accepted/framed content; DEC-003's own Option C rejection was never mirrored into `rejected-approaches-log.md` | unclear handoff (#16) | Each sprint updated its **own** output file(s) but did not sweep **other, older** docs whose status lines were implicitly contradicted by the new decision; the rolling handoff grew ~2,900 lines append-only, making a full-repo consistency check costly enough that it kept being deferred | Future sessions reading a stale doc in isolation (not the full rolling handoff) would inherit wrong context — e.g. treating MVP scope as still open, or re-deep-diving an already-complete competitor | A decision/acceptance session must, in the same session, update the specific Status/Governance line(s) it supersedes in *other* already-existing docs it knows contradict it — not just its own output file; if it cannot find/verify all of them, it should say so as an open item rather than silently leaving stale text | The new **phase-exit criteria** and **documentation-maintenance rule** (`quality-feedback-loop.md` §10–§11) plus this residue sweep's own checklist (`documentation-residue-sweep.md`) — reuse the sweep methodology (grep for status/governance phrases + verify against the actual current decision) for future convergence sprints | **Mitigated** (this sprint) | `documentation-residue-sweep.md`; `research-backlog.md`; `mvp-scope.md`; `feature-taxonomy.md`; `product-vision.md`; `setup-ux-principles.md`; `04-decisions/README.md`; `03-architecture/README.md`; `rejected-approaches-log.md` |

_**DP-007 addendum — PR #59 ChatGPT review (2026-07-02):** the first residue-sweep pass
under-covered its own pattern — `feature-taxonomy.md` and `capability-evidence-map.md` still
carried TQ-403/claim-only wording (evidence-weighting note + the "Capabilities with weak or
blocked evidence" section + two `C-DOCS-*` table cells), `feature-taxonomy.md` still called
single- vs multi-store "not decided" in three places despite DEC-003, `setup-ux-principles.md`
still called Odoo Online custom-module compatibility "open" despite RB-14 Part 2 (PR #58), and
`rejected-approaches-log.md`'s own historical notes still read as if RA-001 did not exist.
**Not logged as a new row** — same category (unclear handoff #16), same root cause (a status
line not swept when superseded), caught and fixed within the same review cycle rather than a
fresh, independent occurrence. Reinforces DP-007's own prevention rule: a residue sweep must
grep for the pattern **across every allowed file**, not stop once the first hit per file is
fixed. No counter change (still 2nd occurrence of unclear handoff #16, per the table above)._
