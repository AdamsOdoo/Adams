...
| unsupported assumption (#3) / weak research (#1) | 2 | **2nd (DP-003, DP-004)** — at the update-rule threshold. Prevention rule added: configuration fields ≠ demonstrated support, and a market-promise ≠ demonstrated bidirectionality; feature-matrix review must justify ✅ by a demonstrated workflow, not a visible field/generic claim. **Sprint D note:** DP-004's rule was **applied, not re-triggered** (WK multi-company ➖, SH multi-company not-found, EC export not-found, TQ claim-only) — no new occurrence. |
| premature architecture (#4) | 1 | No (1st — **DP-005**, logged + prevention rule). NB: an earlier Sprint A "one giant module" bias was captured in `architecture-review-log.md` AR-001, not as a DP row; DP-005 is the first formal defect-pattern row for this category. |
