...
| AR-022 | 2026-07-05 | MBQ-04 credential persistence research and decision proposal | Proposes, via [`../01-research/odoo-credential-storage-official-notes.md`](../01-research/odoo-credential-storage-official-notes.md) and [`../03-architecture/mbq-04-credential-persistence-decision-proposal.md`](../03-architecture/mbq-04-credential-persistence-decision-proposal.md), the official-evidence answer AR-019/AR-020 identified as missing before MBQ-04 could move past a full descope: adversarially-verified research against official Odoo 19.0 docs and source establishes that `password=True`/the view-arch `password` attribute is UI-display masking only (not an ORM read-restriction, not encryption); `ir.config_parameter` is plain, unencrypted key-value storage under a single `group_system`-only ACL, with no official documentation calling it secure secret storage; field-level `groups`/`ir.model.access`/`ir.rule` are access control only and are explicitly bypassed by `sudo()`/superuser mode even at the field-`groups` level; five real official Odoo credential-field examples (`ir.mail_server.smtp_pass`, Stripe/Adyen/Authorize.Net payment-provider keys, `iap.account.account_token`) all use the identical plain-`Char` + `groups='base.group_system'` pattern with no encryption; and the only official encryption-at-rest claim found (`odoo.com/security`, AES-256) is infrastructure-level, scoped explicitly to hosted Odoo Cloud, not to on-premise or to the ORM/schema layer. The decision proposal evaluates five options (A full descope, B standard Odoo field/`groups`, C `ir.config_parameter`, D external secret manager, E hybrid metadata-in-Odoo/secret-outside) and **recommends (proposed only) Option B** — the pattern every real official Odoo secret field already uses, consistent with DEC-004's already-accepted "masked storage/field-level `groups`/least-privilege" posture with no DEC-004 amendment needed — while naming Options D/E as a possible stronger-posture follow-up requiring their own separate evidence pass. **Docs-only; no implementation. Does not itself update MBQ-04's status in `master-blueprint-open-questions.md` (outside this session's allowed-files scope) — proposes classifying it Partially resolved, for ChatGPT to apply via a future acceptance patch.** | **Proposed for ChatGPT review** | Synthesizes already-accepted DEC-004 (auth/token-model posture) and the AR-019/AR-020 record of MBQ-04's exact evidence gap; five parallel official-source research passes plus an independent adversarial verification pass on every risk-flagged claim (encryption/security/password-related), all performed and cited this session (access date 2026-07-05) — see the research notes' "Sources searched" section for the full list; no fact asserted without a direct official-doc or official-source-code citation, and every unconfirmed point is logged as an open question rather than inferred | Checked against `rejected-approaches-log.md` — no existing rejected approach is reintroduced; the proposal explicitly declines to assert `password=True` or `ir.config_parameter` provide encryption/security beyond access control, per this task's hard rules, and does not claim Option B constitutes "encryption at rest" anywhere in its wording | A masking/access-control mechanism being mistaken for encryption (mitigated: both documents state repeatedly and explicitly that UI masking and `groups=` access control are not encryption, and that `sudo()`/superuser bypasses field-level `groups`); this proposal being mistaken for a decision or for DEC-004 having been amended (mitigated: Status sections in both documents state "Proposed," "Not accepted," "Not implemented," and note Option B requires no DEC-004 amendment while Options D/E would); MBQ-04 being marked resolved/accepted prematurely (mitigated: this row and the proposal's own classification section both state "Partially resolved," not "Resolved," pending ChatGPT's acceptance of a specific option) | Pending ChatGPT review of the decision proposal; if accepted, a future acceptance patch would apply the proposal's register-impact wording to `master-blueprint-open-questions.md` (MBQ-04) — not performed by this entry, and not implementation | **Proposed** |
