...
| AR-035 | 2026-07-09 | Task 010 product import final prompt and product-domain gate-opening proposal | After PR #136 merged into `Shopify-connector` (merge commit `c171d8f9b404f0b9bc066ee6fbef811086f5d0fc`), drafts the file-exact Task 010 final implementation prompt via [`../07-implementation-plan/task-010-product-import-final-implementation-prompt.md`](../07-implementation-plan/task-010-product-import-final-implementation-prompt.md) (marked "DO NOT USE THIS PROMPT UNTIL CHATGPT ACCEPTS THE TASK 010 GATE-OPENING PROPOSAL AND EXPLICITLY ISSUES THIS PROMPT IN CHAT" — not issued), converting the accepted MBQ-55 names into exact allowed/forbidden files, exact schema, an exact MVP dedup-threshold conversion of the already-accepted DEC-014 point H two-tier gate, and an exact job/sync-engine usage recommendation (one `product_import_sync` job type via the existing, already-proven `selection_add`/`_get_handlers()` extension seams, zero core edits); and a product-domain gate-opening proposal via [`../07-implementation-plan/task-010-product-import-gate-opening-proposal.md`](../07-implementation-plan/task-010-product-import-gate-opening-proposal.md), auditing all twelve gate criteria and presenting evidence that criteria 3, 4, 5, 9, and 12 (previously not yet satisfied) are now satisfied by this session's own drafting act, with an explicit, non-overclaiming reconciliation of the checkpoint/resume-ownership blocker (readiness-checkpoint Class A, resolved here per its own named in-task-decision resolution path, not asserted as having already been non-blocking) | **Proposed / Under review only — NOT Accepted.** Neither document authorizes Task 010, opens the Task 010 gate, or opens the product-domain implementation gate; both remain closed until ChatGPT explicitly accepts the gate-opening proposal as the distinct act [`product-domain-gate-criteria-proposal.md`](../07-implementation-plan/product-domain-gate-criteria-proposal.md) §4 requires | This session's own governing instructions required stopping before drafting a gate-opening proposal if any of criteria 3/4/5/9/12 proved unsatisfiable by documentation work alone; none did — see the gate-opening proposal's own §3 evidence table | ChatGPT review of [`../07-implementation-plan/task-010-product-import-gate-opening-proposal.md`](../07-implementation-plan/task-010-product-import-gate-opening-proposal.md) against [`../07-implementation-plan/product-domain-gate-criteria-proposal.md`](../07-implementation-plan/product-domain-gate-criteria-proposal.md) §3, and of the final prompt against the accepted MBQ-55 schema | Premature use of the final prompt before ChatGPT issues it; the drafted MVP dedup thresholds or job-type recommendation being mistaken for a new architecture decision rather than a narrow, in-task conversion of already-accepted DEC-014/core-seam facts; VAL-B2, MBQ-05, TD-002, the fulfillment API model, Lite/Full packaging, and the multi-server concurrency proof requirement being mistaken as resolved (none is; all remain exactly as open as their own cited sources state, per the gate-opening proposal's own §7) | This entry does not authorize implementation. Next step: ChatGPT review of the gate-opening proposal; if accepted, ChatGPT separately and explicitly issues the final prompt in a new Claude Code session; the resulting implementation PR must remain draft until ChatGPT reviews it | **Proposed / Under review** |
