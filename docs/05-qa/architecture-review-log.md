...
| AR-037 | 2026-07-09 | MBQ-55 customer-binding naming/schema proposal, customer-domain gate criteria proposal, and Task 011 readiness assessment | After PR #138 (Task 010 implementation) and PR #139 (Task 010 post-merge closure docs) both merged into `Shopify-connector` (latest merge commit `297f398da96d11d1f8f9e25a9d15570a66aed80a`), proposes closure for the customer-binding portion of MBQ-55 via [`../07-implementation-plan/mbq-55-customer-binding-naming-schema-proposal.md`](../07-implementation-plan/mbq-55-customer-binding-naming-schema-proposal.md) — exact proposed model name (`shopify.connector.customer.binding`), future module/file/class names, a full field proposal (inherited-mixin, new relational, imported-snapshot, out-of-scope, and deferred buckets), and a proposed store-settings home (`customer_fallback_partner_id`) for the MBQ-29-resolved single fallback-partner-per-store concept, grounded directly in the merged core code and the already-accepted DEC-006/DEC-008/DEC-013/DEC-014 architecture, mirroring the AR-034/PR #136 pattern used for the product portion; also proposes customer-domain (Task-011-scoped only, explicitly not order-import) implementation-gate opening criteria via [`../07-implementation-plan/customer-domain-gate-criteria-proposal.md`](../07-implementation-plan/customer-domain-gate-criteria-proposal.md); adds a non-rewriting scope/readiness refresh, [`../07-implementation-plan/task-011-customer-import-proposed.md`](../07-implementation-plan/task-011-customer-import-proposed.md), citing rather than editing the existing [`task-011-customer-import-matching-proposed.md`](../07-implementation-plan/task-011-customer-import-matching-proposed.md); and a full Task-011-scoped blocker-classification readiness assessment mirroring `master-implementation-readiness-checkpoint.md`, via [`../07-implementation-plan/task-011-customer-import-gate-readiness.md`](../07-implementation-plan/task-011-customer-import-gate-readiness.md), concluding **not ready** — the customer-domain gate has never been opened and its trigger conditions were undefined until this session's own proposal, and MBQ-55's customer-binding portion remains open pending this session's proposal's own acceptance. This session also confirms, by direct read of the open-questions register, that **MBQ-29 (default-customer fallback) is now Resolved** (not merely partially resolved as DEC-014 alone stated) via the Final MBQ closure pass (AR-020, 2026-07-05) — a genuine narrowing recorded in all four documents above, correcting an unreconciled discrepancy the original Task 011 draft had flagged | **REVISE** (ChatGPT control-room review, GitHub comment ID `4928244425`) — not accepted, PR #140 kept **draft**, not merged. ChatGPT found the naming proposal's ambiguous-customer-match behavior unsafe and structurally inconsistent: it proposed creating a `shopify.connector.customer.binding` row in `status = 'review'` with no `match_key` for an ambiguous match, but `partner_id` is `required=True` — an ambiguous match has no single confirmed candidate, so creating a row would force an automatic guess among several `res.partner` candidates, contradicting DEC-006/RA-006 and the Task 010 precedent (ambiguous/blind product matches never create a binding row). ChatGPT also required an explicit Task 011/Task 012 boundary for the fallback-partner concept, so it does not drag order-import behavior into Task 011. **Revision applied in the same PR:** `mbq-55-customer-binding-naming-schema-proposal.md` §3/§7.1.A/§9/§10/§11 revised so an ambiguous match creates no binding row — the job routes to `blocked_manual_review` with candidate `res.partner` detail stored at job/log level only, never in a binding row; `status = 'review'` is now explicitly scoped to lifecycle review of an already-real binding only. §7.3/§9/§10 revised to adopt **Posture A**: Task 011 proposes only the `customer_fallback_partner_id` store-settings field as inert config substrate, with zero order-resolution behavior and zero consumption within Task 011's own flow — order-level routing/audit-marker logic remains entirely Task 012's future scope. `customer-domain-gate-criteria-proposal.md` gained a new criterion 15 (ambiguous-match handling) and a revised criterion 13 (fallback-partner boundary); `task-011-customer-import-gate-readiness.md` §5/§9 and `task-011-customer-import-proposed.md` §1/§2 updated for consistency. **Conclusion unchanged: Task 011 still not ready to gate; no implementation authorized** **Acceptance (2026-07-09, ChatGPT control-room review, GitHub comment ID `4928377625`):** ChatGPT accepts the revised content of PR #140 in full: (a) the customer-binding naming/schema proposal, closing **only the customer-binding portion of MBQ-55** — the order-binding portion remains fully open, requiring its own future, separate naming pass before Task 012; (b) the customer-domain gate criteria, **accepted as criteria only** — the customer-domain gate itself remains **closed**, opening only via a distinct, future, explicit ChatGPT gate-opening act once every criterion in `customer-domain-gate-criteria-proposal.md` §3 is confirmed satisfied; (c) the corrected ambiguous-match posture — no `shopify.connector.customer.binding` row is ever created for an ambiguous match, `partner_id` remains required, the job routes to `blocked_manual_review` with candidate `res.partner` detail stored at the job/log level only, and `status = 'review'` is reserved for lifecycle review of an already-real binding; (d) the Posture A fallback-partner boundary — Task 011 proposes only the `customer_fallback_partner_id` store-settings config field as inert substrate, with zero order-resolution behavior and zero consumption within Task 011's own flow. **This acceptance does not open the customer-domain gate, does not authorize Task 011 implementation, and does not authorize Task 012/order import.** PR #140 remains **draft, not merged** — merge is a separate, not-yet-authorized act | This is a correction to a naming/schema proposal after control-room review, not a new architecture decision; the corrected posture reuses only already-accepted DEC-006/RA-006 (no automatic guessing) and the already-established Task 010 precedent (ambiguous/blind matches never create a binding row), extending both to the customer domain rather than inventing new rules | ChatGPT review of comment `4928244425` against the original naming proposal, followed by ChatGPT's acceptance review of the revision via comment `4928377625`; this session's own self-audit re-confirms `partner_id` remains required, no placeholder binding row is created for an ambiguous match, candidate detail is stored at job/log level only, and the fallback-partner Posture A boundary is stated consistently across all four Task-011 documents | **Kept open, unaffected by this acceptance:** the exact job/log field(s) used to store ambiguous-match candidate detail (named as criterion 15/blocker row for a future final implementation prompt to fix); the order-binding portion of MBQ-55 (fully open, untouched); MBQ-05; VAL-B2; TD-002; the fulfillment API model decision; Lite/Full packaging; the multi-server concurrency proof requirement (SRR-03/04/09); address handling; company/person (`is_company`) classification; the final Task 011 implementation prompt (not drafted); and the future customer-domain gate-opening act (not performed by this acceptance). This entry, and the four documents it records, must not be read as authorizing Task 011, Task 012, or any other domain-sync task, opening the Task 011 gate, opening the customer-domain gate, or authorizing any code, module, UI/wizard/webhook/OAuth file | Recommended next: ChatGPT's **final merge review of PR #140** — the customer-binding naming proposal and the customer-domain gate criteria are now accepted (this entry), but the customer-domain gate itself remains closed; a future session may draft Task 011's file-exact final implementation prompt and a customer-domain gate-opening proposal only after a distinct, future, explicit ChatGPT gate-opening act confirms every criterion in `customer-domain-gate-criteria-proposal.md` §3 satisfied — not performed by this acceptance | **Accepted** |
| AR-038 | 2026-07-09 | Project readiness master audit, open-points closure register, implementation readiness map, final pre-implementation roadmap, UAT readiness gap analysis, and next-gate readiness roadmap (post-PR #140 pre-implementation planning-closure package) | After PR #140 merged into `Shopify-connector` (merge commit `0e138d9ad8e14a9d0766122f30beea2e19df549a` — see the AR-037 Merge-Record Note below), a high-power audit session (explicitly authorized as such) proposes a docs-only planning-closure package: [`../08-release-readiness/project-readiness-master-audit.md`](../08-release-readiness/project-readiness-master-audit.md) (full PR/decision/task/gate status reconstruction from git/GitHub/repo evidence + read-only code inspection + 2026-07-09 official-source verification), [`../08-release-readiness/open-points-closure-register.md`](../08-release-readiness/open-points-closure-register.md) (every open point classified: blocks-Task-011 / blocks-later-MVP / tracked / needs-live-access / needs-ChatGPT-decision / needs-implementation / deferred-non-MVP, with six evidence-cited docs-only closures CL-1–CL-6), [`../08-release-readiness/implementation-readiness-map.md`](../08-release-readiness/implementation-readiness-map.md) (per-task gate map; customer-domain criteria live table: 7/15 satisfied, all 8 remaining resolvable by one final-prompt session + gate act), [`../08-release-readiness/final-pre-implementation-roadmap.md`](../08-release-readiness/final-pre-implementation-roadmap.md) (sequenced path: audit-PR review → Task 011 final-prompt/gate-opening-proposal session → explicit gate act → one Task 011 implementation session → per-domain pre-pass pattern for Tasks 012–015; parallel tracks MBQ-05 branch B / VAL-B2 / concurrency plan / docs-maintenance / Lite-Full framing), [`../08-release-readiness/uat-readiness-gap-analysis.md`](../08-release-readiness/uat-readiness-gap-analysis.md) (0/15 scenarios executable; four cross-cutting blockers U-1–U-4; two-wave UAT path), and [`next-gate-readiness-roadmap.md`](../07-implementation-plan/next-gate-readiness-roadmap.md) (the exact next session's required outputs and the eight ChatGPT decisions D1–D8 with official-source decision inputs). Status-refresh patches applied to allowed registers only: AR-037 merge-record note (this log), TD-002 clarification note (DEC-011's accepted FulfillmentOrder-only write model vs the genuinely-open exact scope set — TD-002 stays Open), `sync-engine-open-questions.md` revision note 3 (Q5/Q28/Q29/Q37-product statuses vs merged evidence), and a new handoff entry with the exact next-session prompt | **Accepted by ChatGPT** (2026-07-10 — see acceptance note below) | Consolidates the post-PR #140 state so the control room can issue the Task 011 final-prompt/gate-opening-proposal session with zero ambiguity; closes every docs-only-closable status gap found; classifies everything else with owner and next action | Repo/git/GitHub evidence as cited per file; official Shopify (protected-customer-data, Customer, MailingAddress, versioning, access-scopes) and Odoo 19 source (`res_partner.py`, `sale_order.py`) pages, all fetched 2026-07-09 | Risks unchanged and restated, not resolved: VAL-B2; MBQ-05 branch B; MBQ-55 order portion; TD-002; SRR-03/04/09; Lite/Full framing; UI/OAuth/webhook chain; register-staleness items routed as OP-24/OP-25 (files outside this session's allowed list) | ChatGPT reviews the audit PR (draft, docs-only, into `Shopify-connector`); if accepted and merged, issue the Task 011 final-prompt/gate-opening-proposal session per the handoff's next-session prompt — merging the audit PR itself authorizes nothing, opens no gate, and issues no prompt | **Accepted** |
| AR-039 | 2026-07-10 | Task 011 final gate-readiness package: decision-closure brief (D1–D8), final implementation prompt (not usable), customer-domain gate-opening proposal, MBQ-05 branch B decision brief, pre-implementation readiness signoff, and stale-register cleanup | After PR #143 (AR-038, Accepted) merged into `Shopify-connector` (merge commit `4a45f3ea1e6e92acd0621fdc6e2a435b29170221`, verified via GitHub `pull_request_read` 2026-07-10), the explicitly-authorized high-power gate-readiness session (Fable as strategic auditor/planning finisher, per the issued session prompt) produces the complete Task 011 pre-implementation closure package, docs-only: [`../07-implementation-plan/task-011-decision-closure-brief.md`](../07-implementation-plan/task-011-decision-closure-brief.md) (all eight roadmap §4 decisions D1–D8 closed at **recommendation level** with 2026-07-10 official-source re-verification — every load-bearing Shopify/Odoo fact re-fetched and adversarially confirmed, excerpts captured under [`../00-source-materials/shopify-customer-odoo19-partner-task-011-captures.md`](../00-source-materials/shopify-customer-odoo19-partner-task-011-captures.md) per the OP-44 routing); [`../07-implementation-plan/task-011-final-implementation-prompt.md`](../07-implementation-plan/task-011-final-implementation-prompt.md) (file-exact final prompt carrying D1–D8 as proposed content, first line **DO NOT USE UNTIL CHATGPT REVIEWS, ACCEPTS, EXPLICITLY OPENS THE TASK 011 GATE, AND ISSUES THIS PROMPT**); [`../07-implementation-plan/task-011-customer-domain-gate-opening-proposal.md`](../07-implementation-plan/task-011-customer-domain-gate-opening-proposal.md) (all 15 accepted criteria: 7 Satisfied, 7 Satisfied-on-acceptance of this same package, criterion 12 assembled as the gate-act item with a re-verified blocker table); [`../03-architecture/mbq-05-branch-b-distribution-auth-decision-brief.md`](../03-architecture/mbq-05-branch-b-distribution-auth-decision-brief.md) (evidence-backed conclusion: branch B does **not** block Task 011; parallel research task recommended; 2026-07-10 distribution/PCD facts: no review-free multi-merchant route exists; candidate set B-1–B-4 with named open questions); [`../08-release-readiness/pre-implementation-readiness-signoff.md`](../08-release-readiness/pre-implementation-readiness-signoff.md) (Option A conclusion: only ChatGPT acts remain); plus stale-docs cleanup — OP-24 (MBQ-55/MBQ-04 register-row refresh notes), OP-25 (five stale READMEs refreshed; residue routed), OP-42 (binding-status ambiguity flagged in-file, decision left to ChatGPT), OP-43 (classified: green outcome undisputed, counts unreconciled), OP-44 (captures done; Customer-object section added to [`../01-research/shopify-official-api-notes.md`](../01-research/shopify-official-api-notes.md)) — and register/handoff addenda | **Accepted by ChatGPT** (2026-07-10, PR #144 control-room review, comment ID `4932704451` — a REVISE-with-required-fixes review; both required fixes applied in the same PR, see the AR-039 Acceptance Note below). The acceptance fixes the final prompt's content as binding — **still not issued**; **no gate is opened by this acceptance** ("Do not open the gate"), no prompt is usable, Task 011/012/013/014/015 remain unauthorized, MBQ-05 branch B remains undecided | Completes the exact next-session outputs specified by [`../07-implementation-plan/next-gate-readiness-roadmap.md`](../07-implementation-plan/next-gate-readiness-roadmap.md) §2 (the PR #137 analogue for the customer domain), so ChatGPT can reach either "gate is ready to open after merge" or an exact-blocker verdict without further audit | Repo/git/GitHub evidence as cited per file; merged core code read directly (read-only) 2026-07-10; official Shopify (Customer/CustomerEmailAddress/CustomerPhoneNumber/MailingAddress/customers-query/CustomerSortKeys/pagination/limits/search-syntax/versioning/access-scopes/protected-customer-data/distribution/visibility/review/billing/compliance pages) and Odoo 19 source (`res_partner.py`, `sale_order.py`, `mail_thread_blacklist.py`) — all fetched 2026-07-10, load-bearing claims adversarially verified same day | Risks unchanged and restated, not resolved: VAL-B2; MBQ-05 branch B (decision itself); MBQ-55 order portion; TD-002; SRR-03/04/09; Lite/Full framing; UI/OAuth/webhook chain; OP-25 residue routed in the signoff §5. Package-specific risk: the final prompt being mistaken for an issued/usable prompt (mitigated: top-line warning + six unmet usage conditions), or this row being mistaken for an acceptance (mitigated: explicit Proposed status) | ChatGPT performs the final merge review of PR #144; after merge: perform the distinct customer-domain gate-opening act (confirming the criterion-12 table as of that date), separately decide MBQ-05 branch B parallel authorization, optionally give the OP-42 one-line confirmation, and only later issue the final prompt verbatim in a new session | **Accepted** |
| AR-040 | 2026-07-10 | Task 011 customer import/matching implementation — draft PR, runtime validation green, ready for ChatGPT's final merge review | After PR #144 merged into `Shopify-connector` (merge commit `8b364aa360cb596dd584bbc8345b790cc7ad20ed`) and ChatGPT performed the explicit customer-domain gate-opening act (GitHub comment ID `4934249603`, PR #144) naming this exact prompt, this session verified the base placeholder against the actual current tip (no drift) and implements the `shopify_connector_sale` addon per the final implementation prompt (branch `claude/task-011-shopify-connector-solrrp`, PR #145, opened as draft): one concrete binding model (`shopify.connector.customer.binding`), declaring explicit `_name`/`_inherit` per the accepted MBQ-55 schema; the inert `customer_fallback_partner_id` store-settings field (D5, Posture A — defined, zero consumption, confirmed by source-level grep and a byte-identical-outcome behavioral test); a read-only `shopify.connector.customer.importer` `AbstractModel` implementing the final prompt §8.1 match-key sequence (existing binding → recall-safe email match, with candidate discovery via the always-safe `[('email', '!=', False)]` baseline plus mandatory Python-side `email_normalize()` comparison on both sides, applied identically to the active and archived-inclusive searches — the critical D1 rule this task's own instructions singled out — → exactly-one-active-candidate bind guarded against `binding_conflict` → archived-match check routed to `duplicate_risk` → confident no-match create, gated → ambiguous/missing-email never create, routed to `blocked_manual_review` via the existing, unmodified `JobHandlerError`/`_route_failure()` mechanism, with the exact §8.2 JSON candidate-evidence payload carried in the existing `job.log.technical_detail` field); the three required extension seams (`job_type` `selection_add` for `customer_import_sync`, a `_domain_flag_for_job_type()` override mapping it to `sale_domain_enabled`, a `_get_handlers()` override), all declared inside `shopify_connector_customer_importer.py` only — zero edits to any `shopify_connector_core` or `shopify_connector_product` file (confirmed by `git diff --stat`); `ir.model.access.csv` only, reusing the four existing groups, no new group/XML; four required test files covering every case named in final prompt §10, including the recall-safety wrapped/display-name/mixed-case email tests and their duplicate-prevention proofs. Four narrow in-task decisions were made and are recorded in [`../05-qa/task-011-customer-import-validation-results.md`](./task-011-customer-import-validation-results.md) §C: (1) `res_model`/`res_id` targeting not implemented by the dispatcher handler, mirroring Task 010's own exact precedent; (2) the always-safe `[('email', '!=', False)]` baseline chosen over the permitted `ilike`-prefilter alternative for candidate discovery; (3) partner `name` mapped from the incoming `displayName`; (4) the final prompt's own informational job-log line for an unresolved country/state code is implemented (fixed on control-room review, comment `4934451381`) — `import_customer_sync()`/`_apply_import()`/`_resolve_customer_binding()`/`_create_partner()` each gained an optional `job=False` parameter, threaded through only from the dispatcher's `_handle_customer_import_sync(job)`; `_apply_import(store, payload)` remains fully direct-testable with no job context; when a provided country/province code cannot be resolved and a `job` is present, a new `_log_unresolved_address_code()` helper appends one informational `event_type='note'` row through the existing, unmodified `job.log._system_append()` path — no new field, no core edit, no server log write; the message is minimal/operator-safe (never the specific code, partner, email, phone, or address), the bare code lives only in `technical_detail`. A second control-room review (comment `4934627954`) then found the one live Odoo.sh run this PR received turned up exactly one failure — a brittle, formatting-dependent source-substring test assertion (`test_source_level_single_execute_call_uses_fixed_query_constant`), not a functional importer defect — fixed by rewriting that test to parse the importer module with Python `ast` (exactly one `execute()` call; it references the `CUSTOMER_IMPORT_QUERY` name; no f-string/literal-string argument; the constant itself stays a read-only `query`, never `mutation`), independent of line-wrapping. **A follow-on live Odoo.sh run then came back green**, confirmed directly from the operator-provided install log: `odoo.tests.result: 0 failed, 0 error(s) of 268 tests when loading database 'adamsmen-claude-task-011-shopify-connector-solrrp-34736893'` (per-module `odoo.tests.stats` lines: `shopify_connector_core: 187 tests`, `shopify_connector_product: 61 tests`, `shopify_connector_sale: 56 tests` — the per-module sum does not arithmetically reconcile with the final 268 total, quoted verbatim rather than reconciled, per the OP-43 lesson); the previously-failing AST test now starts and completes with zero `ERROR`/traceback lines before the next test begins; the five `ERROR`-level `bad query` SQL lines during `shopify_connector_sale`'s run map one-to-one onto its own intentional not-null/uniqueness constraint tests, confirmed expected negative-test noise, not failures; the build-log docutils/RST warning (`<string>:38`/`<string>:43`) is confirmed, by direct log-position inspection, to occur during the unrelated `mail` module's own load (ten modules and ~5 seconds after `shopify_connector_sale` already finished), not any Task 011 file, and did not fail the build | **REVISE** (ChatGPT control-room review, GitHub comment ID [`4934451381`](https://github.com/AdamsOdoo/Adams/pull/145#issuecomment-4934451381)) for the job-log-note omission, fixed in the same PR; **acceptance-pending-rerun** (comment [`4934612056`](https://github.com/AdamsOdoo/Adams/pull/145#issuecomment-4934612056)) confirmed the fix and named the live-Odoo.sh-run blocker; **REVISE** (comment [`4934627954`](https://github.com/AdamsOdoo/Adams/pull/145#issuecomment-4934627954)) for the one brittle-test runtime failure, fixed in the same PR; **acceptance-pending-rerun** (comment [`4934730895`](https://github.com/AdamsOdoo/Adams/pull/145#issuecomment-4934730895)) confirmed the AST fix and named the still-stale PR body as a pre-final-review item. **This session's own closure: the follow-on live Odoo.sh run is green** (`0 failed, 0 error(s) of 268 tests`, evidence quoted verbatim above from the operator-provided install log) — this docs-only closure session records that evidence, updates this row, and refreshes the previously-stale PR #145 body. **PR #145 remains draft, unmerged — this entry does not merge it, mark it ready for review, or itself constitute ChatGPT's final merge review**, which remains the next required, separate act | This is an implementation-closure-style entry recording what was built and validated, mirroring AR-036's Task-010-closure pattern, now carried through to a **green-runtime, ready-for-final-review** state for a PR that remains draft | Runtime evidence is the operator-provided Odoo.sh install log (quoted verbatim above, not independently re-run by Claude — no live Odoo.sh/CI access exists in this session's own environment, unchanged throughout this PR's history); static validation (full detail in the validation-results doc §D/§H.4/§I.4, all clean) plus the runtime-green result in §J together satisfy final prompt §11/§12's validation requirements; ChatGPT's own final merge review of the full PR diff, this row, and the validation record is the next required act | The per-module `odoo.tests.stats` counts do not arithmetically reconcile with the final 268-test total — quoted verbatim, not resolved, consistent with the standing OP-43 caution against synthesizing per-module totals; VAL-B2, MBQ-05 (both branches), MBQ-55's order portion, TD-002, the fulfillment API model, Lite/Full packaging, and the multi-server concurrency proof requirement (SRR-03/04/09) all remain exactly as open as before this task — none is touched, resolved, or narrowed; no Task 012/013/014, Task 015, UI, webhook, OAuth, or Lite/Full packaging work is authorized or started by this task | ChatGPT performs the final merge review of PR #145 against the final implementation prompt §3–§14, this row, and the validation record (including §J's runtime-green evidence); only ChatGPT may merge or mark this PR ready for review — the customer-domain implementation gate closes the moment this PR was opened as draft (`customer-domain-gate-criteria-proposal.md` §4; final prompt §14), so no further customer-domain work may start regardless of this PR's own outcome | **Proposed for ChatGPT review** |
| AR-041 | 2026-07-10 | MBQ-05 branch B distribution/auth final decision package: final decision brief, DEC-026 proposal, next-implementation-implications, and register/handoff updates | A dedicated, docs-only MBQ-05 branch B research/decision-preparation session (Fable as strategic auditor / architecture decision researcher, not implementation worker), authorized separately from Task 011 and not interfering with it, produces: [`../03-architecture/mbq-05-branch-b-final-decision-brief.md`](../03-architecture/mbq-05-branch-b-final-decision-brief.md) (full evaluation of candidates B-1 public/limited-visibility, B-2 public/fully-visible, B-3 per-customer custom apps, and B-4 hybrid, across official evidence, PCD/compliance analysis, billing/commercial analysis, setup-wizard/Lite-Full/MVP impact, risks, and open questions, built on new 2026-07-10 gap-fill research — Built for Shopify eligibility/requirements, Partner Program Agreement accessibility and per-partner custom-app-count limits, App Store listing/ongoing-quality-check obligations, OAuth mechanics for public vs. custom apps, and Billing API mechanics — plus a five-claim adversarial verification pass, all official-source, all dated 2026-07-10); [`../04-decisions/DEC-026-distribution-auth-branch-b-proposal.md`](../04-decisions/DEC-026-distribution-auth-branch-b-proposal.md) (a **Proposed, not accepted** DEC recommending a hybrid trajectory — keep DEC-023 branch A unchanged; designate Public distribution/limited visibility as the target scalable architecture; do not adopt per-customer custom apps as the standing commercial-scale answer; treat as Phase 2+ scope under RA-003's own revisit condition); and [`../07-implementation-plan/mbq-05-branch-b-next-implementation-implications.md`](../07-implementation-plan/mbq-05-branch-b-next-implementation-implications.md) (outcome-conditional implications for OAuth/token acquisition, setup wizard, store-connection UX, compliance webhooks, PCD governance, billing/commercial packaging, Lite/Full packaging, release readiness, and UAT). Status-patch-only updates to [`master-blueprint-open-questions.md`](../03-architecture/master-blueprint-open-questions.md) (MBQ-05 row, dated note only, still Partially routed / Open), [`../08-release-readiness/open-points-closure-register.md`](../08-release-readiness/open-points-closure-register.md) (OP-05/OP-40 addenda; new OP-45/OP-46), [`../08-release-readiness/implementation-readiness-map.md`](../08-release-readiness/implementation-readiness-map.md) (distribution/auth architecture layer note), and this handoff/log | **Accepted by ChatGPT (2026-07-10)** — accepts DEC-026's hybrid recommendation as the strategic branch-B distribution/auth direction (DEC-023 branch A unchanged; Public/Limited-Visibility (B-1) designated the target scalable architecture; B-3 per-customer custom apps not adopted as the standing commercial-scale answer; Phase 2+ scope under RA-003's own revisit condition); **implementation remains gated separately — not authorized by this acceptance**; RA-003 not lifted; MBQ-04, OP-23/Q27, OP-45, and OP-46 remain open; see DEC-026 "Acceptance note" | Completes the parallel branch B research/decision task the AR-039 session's own brief recommended (§8), closing several of that brief's named open items (Built for Shopify status, Partner Program Agreement accessibility/limits, App Store ongoing obligations, OAuth mechanics detail, Billing API mechanics) with fresh official-source evidence and an adversarial verification pass, so ChatGPT can decide branch B without further broad research | ChatGPT review of the draft PR against [`../03-architecture/mbq-05-branch-b-final-decision-brief.md`](../03-architecture/mbq-05-branch-b-final-decision-brief.md) (candidate comparison table §2, evidence §3/§6, adversarial verification record §8) and [`../04-decisions/DEC-026-distribution-auth-branch-b-proposal.md`](../04-decisions/DEC-026-distribution-auth-branch-b-proposal.md) (non-authorizations §8) | Risks unchanged and restated, not resolved: the MBQ-04 encryption-posture tension if a public-app path is ever pursued (PCD Level 2 review-enforced encryption/retention/access-log/incident-response obligations vs. the accepted Task 002 plain-`Char`-plus-ACL, no-encryption-claim posture); OP-23/Q27 (Lite/Full packaging) remains open and this package's Lite/Full fit analysis is contingent on it; DEC-023's "a single pilot customer" scope-generalization question is surfaced, not resolved; Shopify's Billing API/Managed Pricing revenue-share is unsourced; the Partner Program Agreement's fee schedule and "Enforcement" page were not fetched; B-3's scalability ceiling and per-client operational burden remain officially undocumented/unquantified — none of this is decided or closed by this entry | ChatGPT reviews the draft PR; if accepted, revised, or rejected, record that decision as its own distinct act (this entry does not itself decide DEC-026); regardless of outcome, this entry does not affect Task 011/012/013/014/015 scope, files, or gate status in any way — **accepted 2026-07-10** (recorded as this row's own distinct act); acceptance is strategic-direction-only, implementation gated separately, no OAuth/wizard/App-Store/billing/webhook/UI/Task 012–015/MBQ-05-implementation work is authorized | **Accepted** |
| AR-042 | 2026-07-10 | MVP planning-completion package: baseline reconciliation, 2026-07-10 official-source research captures, DEC-027/028/029 proposals, final module/dependency architecture (PD-1..6), Lite/Full packaging proposal, implementation packets with locked prompts for Tasks 012/013/014/015, Area 6 ("Task 016"), UI U1–U3, and webhooks W1–W5, final UAT plan (24 scenarios), release-readiness execution plan, master plan, completion audit, and signoff | A single authorized Fable planning-completion session (docs-only, high-power research mode per the issued session prompt) verifies the post-PR-#147 baseline from GitHub (tip `21d59ec`; PRs #145/#146/#147 merged), reconciles stale status text (incl. a documentary AR-040 status-refresh note), builds the full planning inventory ([`../08-release-readiness/mvp-planning-completion-audit.md`](../08-release-readiness/mvp-planning-completion-audit.md) §2), closes every research-closable gap with fresh 2026-07-10 official-source captures ([`../00-source-materials/shopify-orders-inventory-fulfillment-product-partner-captures-2026-07-10.md`](../00-source-materials/shopify-orders-inventory-fulfillment-product-partner-captures-2026-07-10.md) — Orders/Inventory/FulfillmentOrders/product mutations/Partner Program incl. the OP-45 fee schedule/PCD+webhooks/Odoo 19 source/versioning-scopes-limits; 24/24 load-bearing claims adversarially re-verified plus raw-HTML spot checks), and produces the complete proposal set: DEC-027 (branch-A pilot scope), DEC-028 (credential/PCD posture ladder), DEC-029 (Lite/Full = module sets), [`../03-architecture/final-mvp-module-and-dependency-architecture.md`](../03-architecture/final-mvp-module-and-dependency-architecture.md), [`../02-product/lite-full-packaging-final-proposal.md`](../02-product/lite-full-packaging-final-proposal.md), seven implementation packets under `../07-implementation-plan/`, [`../08-release-readiness/final-mvp-uat-plan.md`](../08-release-readiness/final-mvp-uat-plan.md), [`../08-release-readiness/release-readiness-execution-plan.md`](../08-release-readiness/release-readiness-execution-plan.md), [`../08-release-readiness/implementation-ready-master-plan.md`](../08-release-readiness/implementation-ready-master-plan.md), and [`../08-release-readiness/mvp-planning-completion-signoff.md`](../08-release-readiness/mvp-planning-completion-signoff.md); register addenda (OP §3.9, MBQ session note) | **Proposed for ChatGPT review** — the master plan §1 enumerates the eleven review calls; accepting the PR without named exceptions accepts them as proposed; every locked prompt remains unusable until its own gate act | Completes the remaining research and planning so that, after acceptance, Task 012 can start from its locked prompt without another broad research or architecture exercise (the session's stated master objective) | The PR diff (Markdown-only, verified pre-commit); the captures' 24/24 CONFIRMED verification record and raw-HTML spot checks; the audit §2 completeness statement + §4 contradictions record + §7 adversarial red-team record (five reviewers; five blocker-class findings — incl. the no-store-reaches-`connected` readiness gap, fixed by the new Area-6 D-A6-7 — all fixed in-session); the red-team findings section of the PR body | Named honestly, unresolved: VAL-B2 and the concurrency proofs (SRR-03/04/09) remain external validations — no packet claims them; the flagged interpretation points (D-012-3/-4/-6/-9, D-013-5/-8, D-014-2/-6, D-015-7, D-A6-1/-5/-7, webhook MVP-tail scope) are proposals with stated defaults, not settled facts; empirical Shopify unknowns are converted to named dev-store checks inside the consuming packets; RA-003/DEC-026 gating untouched | ChatGPT reviews the draft PR, makes the eleven §1 calls (accept/revise/reject), then performs the order-domain gate act and issues the Task 012 prompt in a new session (master plan §5) | **Proposed for ChatGPT review** |
| AR-043 | 2026-07-11 | Task CORE-R1 — capability-aware readiness correction: implementation + focused correction (draft PR #149 into `Shopify-connector`) | After PR #148 merged into `Shopify-connector` (merge commit / verified base SHA `2bdb07cf33045a696311c420e577cbbb09cdfb38`) and ChatGPT's CORE-R1 gate act (comment `4948225529`, one session), this session implements Task CORE-R1 exactly per [`../07-implementation-plan/task-core-r1-readiness-correction-packet.md`](../07-implementation-plan/task-core-r1-readiness-correction-packet.md) (D-R1-1..5): **D-R1-1** real capability-aware `_check_cron_queue_health` (drain cron exists + active, read via the ONE new named read-only `sudo()` helper `_drain_cron_active_state`; no queued job stalled > `READINESS_QUEUE_STALL_MINUTES=60` via the exact discriminator `state='queued' AND NOT started_at`, `create_date`-aged; named fail reasons; no Area-6 scan-cron requirement; re-queued-with-history boundary honored); **D-R1-2** `_check_mapped_location` not-applicable pass when `inventory_domain_enabled` is False, else fail-closed `not_proven` (core settings flag only); **D-R1-3** `_check_webhook_hmac` not-applicable pass with the packet's exact reason; **D-R1-5** `action_test_connection` writes `api_health_state='normal'` **and clears `api_health_reason`** on full non-fall-forward success (the `degraded` fall-forward path untouched); **D-R1-4** `tests/test_readiness_slot_closure.py` (20 methods, incl. a real-behavior degraded→normal recovery regression) proves an eligible Lite store aggregates readiness `pass` and reaches `connected` via `action_activate()`, and that a genuine essential failure still blocks activation. Focused-correction session then **consumed CORE-R1 gate amendment `4948368039`**, updating the two authorized sudo AST guards (`test_job_log_system_append.py`, `test_credential_service.py`) from the two-site to the exact three-site inventory. Validation record: [`./task-core-r1-validation-results.md`](./task-core-r1-validation-results.md) | **Draft PR #149 open for ChatGPT review** — the CORE-R1 gate closed on draft-PR-open; ChatGPT review `4678631974` (REVISE) is closed in-session: (1) gate amendment `4948368039` consumed — both global sudo guards now enforce the exact three-site list (`shopify_connector_job_log.py`, `shopify_connector_readiness_check.py`, `shopify_connector_store_credential.py`) with AST enumeration + exact-list equality, no fourth site permitted; (2) D-R1-5 now clears the stale `api_health_reason` on recovery, with a real-behavior regression. **No unresolved sudo-guard conflict remains.** Nothing accepted/merged; PR stays draft | Implements the accepted packet's readiness correction so a store can finally reach `connected` (the red-team BLOCKER in `mvp-planning-completion-audit.md` §7.1 item 1); the D-R1-1 sudo-inventory governance conflict is resolved by the issued gate amendment rather than by editing forbidden files or dodging the guard | Local `py_compile` clean on all changed Python files; standalone AST replication green for both updated global sudo guards (exact three-site list), the three CORE-R1 source guards, and the pre-existing `_check_*`-mutation guard; **Odoo.sh runtime GREEN and quoted verbatim (OP-43 satisfied)** (ChatGPT runtime review `4678716995`) — build DB `adamsmen-claude-task-core-r1-readiness-correction-t-34779589` at head `e262738`: `shopify_connector_core: 209 tests`, `shopify_connector_product: 61 tests`, `shopify_connector_sale: 56 tests`, final `0 failed, 0 error(s) of 288 tests`; the log explicitly ran `TestReadinessSlotClosure.test_eligible_lite_store_aggregates_pass`, `.test_eligible_lite_store_reaches_connected`, and `.test_degraded_recovers_to_normal_and_clears_reason`, `TestJobLogSystemAppend.test_source_level_three_sudo_sites_total`, `TestCredentialService.test_source_level_sanctioned_sudo_sites_guard`, and the full pre-existing `shopify_connector_core` suite; one non-blocking pre-existing docs warning (Unexpected indentation / Block quote ends without a blank line; unexpected unindent), not attributed to a CORE-R1 file | No outstanding risk on the sudo inventory (exact-list equality; any fourth model-layer `sudo()` still fails both guards). Two cosmetic old-name docstring references remain in out-of-allowlist files (`test_readiness_check.py`, `test_job_dispatch.py`) — non-executing prose, forbidden to edit, no test impact. VAL-B2 and every other named external/runtime validation remain exactly as open as before — none touched | ChatGPT re-reviews PR #149 against the packet (D-R1-1..5), the validation record, and the green Odoo.sh build, then performs the final merge review; no Task 010B/011B/LC-1/012/Area-6/SEC-1/UI/webhook/PERF-1 work is authorized or started by this task | **Implementation validated — Odoo.sh runtime green (`0 failed, 0 error(s) of 288 tests`); ready for ChatGPT final merge review (not merged; no new gate authorized)** |
| AR-044 | 2026-07-12 | U0 premium operator UI visual prototype — control-room acceptance of the visual baseline (PR #152 merged into `Shopify-connector`) | The U0 visual-design session produced the editable static HTML/CSS/PNG operator-UI prototype + specs under `docs/09-ui-prototype/**` (gate `4948902516`; revision reviews `4950255482`, `4950432754`), proposing **P1–P13** and the **PD-7 selective-Owl scope** as the future operator-UI visual baseline. Design artifacts only — no Odoo implementation, no addon, no production code. | **accepted (visual baseline)** — control-room comment `4951204357` ("ACCEPT U0 visual baseline … ACCEPT AND MERGE") | Acceptance records the ranked dashboard + coherent active-state model (P1/P13), the severable 7-day trend (P2), the `--sc-border` / `--sc-border-strong #79839B` split (P3), six-chip setup compression + three-group readiness (P4/P5), binding-key vs advisory evidence + automatic Task 011 bind/create outcome screens (P6), four-column diff + fail-closed no-authority state (P7), the restrained Odoo-native token layer (P8), inline-SVG placeholders with platform FontAwesome required at implementation (P9), mobile reflow (P10) + compact Odoo-native shell at ≤ 900px (P11), `blocked_manual_review → danger` distinguished without color (P12), and the selective Owl scope for dashboard/setup/matching/diff with ordinary records Odoo-native (PD-7) | None outstanding — acceptance recorded; **PR #152 merged** at merge commit `65e915aada32930a19a14c94d23dc9bd5e6fb517` (parents `fcbbb0b` + `2c2648e`); pre-merge evidence: 41/41 source-to-render assertions + an independent adversarial visual QC (OVERALL PASS); contrast 32/32; zero JavaScript / external dependencies | **Visual acceptance only — it authorizes NO production code and opens NO implementation gate.** UI-U1/U2/U3 remain CLOSED; UI-U1 stays dependent on its existing prerequisites (Area 6 + SEC-1 merged; U0 accepted — now satisfied). P9 icon placeholders must be replaced by platform FontAwesome at implementation; the §11-vs-§6 manual-review wording correction (P12) is recorded for the design system, not applied to code | No ADR promotion required for a docs-only visual-baseline acceptance; the acceptance is recorded here and synchronized into `docs/09-ui-prototype/**` (README §6 + traceability §3 + the screen specs), `docs/01-research/research-handoff.md`, and `docs/08-release-readiness/implementation-ready-master-plan.md`. A future explicit gate act is required before any UI-U1/U2/U3 (Owl/code) work begins | **Accepted (visual baseline) — U0 complete; PR #152 merged at `65e915a`; no implementation gate opened** |
| AR-045 | 2026-07-11 | Task 011B — Customer Matching Scalability: replace the O(n) full-partner email scan with an indexed stored normalized-email lookup (draft PR into `Shopify-connector`) | After PR #149 (CORE-R1) merged into `Shopify-connector` (verified base SHA `f9c3c5fd25af3f94ee71cc2ead3821e7da85443d`) and ChatGPT's Task 011B gate act (comment `4948879507`, one session, parallel-safe with Task 010B), this session implements Task 011B exactly per [`../07-implementation-plan/task-011b-customer-matching-scalability-packet.md`](../07-implementation-plan/task-011b-customer-matching-scalability-packet.md) (D-011B-1..7): **D-011B-1** one connector-owned `_inherit='res.partner'` stored computed indexed field `shopify_connector_email_normalized` (`fields.Char`, `store=True`, `index=True` btree, `readonly=True`, `@api.depends('email')`), computed `email_normalize(partner.email, strict=False) or False` — the exact merged importer normalizer, no override/inverse/search-method/constraint/sudo; **D-011B-2** `_find_active_candidates`/`_find_archived_candidates` rewritten to a btree-indexed equality search on that column (archived path keeps `active_test=False` + `('active','=',False)`), every downstream routing/ordering/evidence-cap/error-class untouched; **D-011B-3** an equivalence corpus test retains the old full-scan path as a test-only reference and asserts `old_ids == new_ids` (active + archived) across a pathological corpus; **D-011B-5/6** duplicate/ambiguity preserved + concurrency proven by a GENUINE independent-transaction binding race (`db_connect` connections + worker thread + `pg_stat_activity` lock-wait sync + durable cleanup), no partner uniqueness constraint added; **D-011B-4/7** stored-compute backfill + a **deterministic** 100k benchmark (index-based composition with exact asserted counters) and a full customer-matching-cost probe, tagged `post_install,-standard`. Validation record: [`./task-011b-validation-results.md`](./task-011b-validation-results.md) | **Draft PR open — three focused corrections applied, production design unchanged.** First correction (ChatGPT review `4950230315`, REVISE): sequential fake-concurrency replaced with a genuine two-transaction race; benchmark corpus made deterministic with exact counters; throughput now measures full matching cost; backfill proxy fails loud; evidence wording no longer overstates unexecuted results. **Second correction (ChatGPT review `4950353232`, REVISE — execution-safety of the opt-in harness only): worker-owned cursor (nothing crosses the thread boundary but `queue`/`Event`); attributed lock-wait proof (`pg_blocking_pids(B)`∋A AND B's blocked statement mentions the binding table; wait must clear after A commits); bounded timeouts with fail-closed termination; durable cleanup that re-raises + fresh-connection zero-rows verification; complete first-collision assertions (`retry_count==1`, `next_retry_at`, attempt+retry logs); type-only redacted backfill/worker diagnostic; exact per-run marker as the benchmark corpus identity (no id-range).** **Third correction (ChatGPT review `4951165587`, REVISE — test/docs/base-alignment only): branch base-aligned onto the current integration tip `fcbbb0b` via a normal merge commit (pulls in PR #153 evidence, disjoint; all PR #153 history preserved); block evidence now requires a server-side `INSERT INTO`+binding-table regex (`query ~* BINDING_INSERT_QUERY_REGEX`, not a bare table mention), proven by a standard-CI predicate test; fresh cleanup AND verification connections apply transaction-local `lock_timeout`+`statement_timeout` so they cannot hang; the worker is daemonized as a last-resort process-liveness guard; sanitized emergency cursor-teardown failures are recorded and asserted absent; the diagnostic wording no longer claims an unredacted trace survives in host logs.** All three corrections touch only the test file + these two shared docs. **Base-alignment session (ChatGPT review `4680106356`, which ACCEPTED the third correction and set base-alignment as the runtime prerequisite): the branch was re-aligned onto the new integration tip `65e915aada32930a19a14c94d23dc9bd5e6fb517` (`Shopify-connector` after U0 / PR #152 merged) via a normal merge commit `19c0911` — the only conflict, the shared append-only `research-handoff.md`, was resolved by preserving BOTH sides completely (Task 011B + U0 entries; 0 lines lost either side; verified); no `addons/**`/`tests/**`/Task 011B file conflicted or changed; the PR diff vs `65e915a…` is exactly the 8 Task 011B files (no U0 / Task 010B / CORE-R2 artifact appears). Standard Odoo.sh suites for the aligned head `19c0911` are OUTSTANDING and operator-run — no Odoo runtime here, and no commit status/check run is posted for `19c0911` (get_status total_count 0), so no result is observed or claimed.** Nothing accepted or merged; PR stays draft | Resolves the control-room scalability finding (PR #148 review item 2 / comment `4942966937`): the merged matcher was O(partners-with-email) per incoming customer; the indexed column makes candidate discovery a single btree lookup while preserving matching policy bit-for-bit (email remains the sole automatic key — RA-006 honored) | Odoo 19 primary-source verification of `email_normalize`/stored-compute/index behavior recorded in the validation record; `python3 -m py_compile` clean on all 5 changed Python files; standalone AST replication of the source guards green; standalone simulation of the deterministic benchmark corpus + probe-mix (counters/tally exact); core-source verification of the concurrency taxonomy. Second-correction static checks (review `4950353232`): `py_compile` clean; **race-safety AST guards** green (`cr_w` worker-owned only; thread started with no cursor/Env arg; every join/wait/queue-get bounded; `_durable_cleanup` re-raises; worker closes its cursor in its own `finally`); **marker-benchmark simulation** green (in-Python and DB-marker counts both `{total:100000, active:70000, archived:30000, shared:1500, wrapped:10500, ordinary:88000, non_null:100000}`; probe tally `700/150/100/50`); greps confirm 0 broad `assertRaises(Exception)`, 0 `registry.cursor()`, 0 raw `str(exc)` emits, `pg_blocking_pids` present, 7 independent `db_connect` cursors. Third-correction static checks (review `4951165587`): base-aligned to `fcbbb0b` via a conflict-free normal merge commit (PR #153 evidence preserved); `py_compile` clean; **race-safety AST guards now 10/10** (adds: worker `daemon=True`; both cleanup+verification apply `_apply_cleanup_bounds` and re-raise; `CLEANUP_*_TIMEOUT_MS` finite positive + `set_config(…, true)` transaction-local; `_blocking_evidence` uses `query ~* BINDING_INSERT_QUERY_REGEX` and never selects the raw `query` column; `_wait_until_blocked_by` requires blocker-PID AND INSERT together); production-source 22/22 guards still pass; standard-CI `test_binding_insert_predicate_matches_only_inserts` added; **34 test methods total (32 standard + 2 opt-in)**; `git diff` confirms the three production files + `tests/__init__.py` byte-identical to `e8126ec` and **no new repository file added**. **PENDING (no Odoo runtime this session — authored, not proven; no live/dev-store Shopify fixture authorized; live Shopify validation depends on CORE-R2):** full `core`/`product`/`sale` suites green on Odoo.sh; the equivalence/routing/genuine-concurrency test results; the 100k benchmark latency/throughput numbers; the authoritative module-upgrade/backfill duration | D-011B-6 restates, does not resolve, the standing multi-server claim/dispatch concurrency caveat (SRR-03/04/09); the genuine race proves only the binding-layer route and only once it is actually run, and its second leg is a **deliberately forced clean-transaction retry** (a direct `_dispatch_one` re-invocation, NOT the scheduler's due-time selection). The additive column survives a code revert (may remain inert; any schema cleanup is a separate tested migration). Parallel Task 010B shares only the two append-only docs (`architecture-review-log.md`, `research-handoff.md`); this AR-045 numbering may collide with a parallel Task 010B row and is reconciled append-only at second-merge per the gate's parallel discipline | ChatGPT reviews the draft PR against the packet (D-011B-1..7), the validation record, and — once run — the Odoo.sh build + benchmark numbers, then performs the merge review; no Task 010B/012/LC-1/Area-6/SEC-1/inventory/fulfillment/export/UI/webhook/OAuth/PERF-1 work is authorized or started | **Implementation authored; three focused corrections applied (reviews `4950230315`, `4950353232`, `4951165587`) and ACCEPTED (review `4680106356`); production design byte-unchanged; branch re-aligned to the current integration tip `65e915a` (U0 / PR #152 merged; both sides of the shared handoff preserved) at aligned head `19c0911`; static + AST checks (race-safety 10/10, production 22/22) + benchmark simulation green; standard Odoo.sh suites OUTSTANDING (operator-run — not executable/observable here); equivalence, routing, genuine concurrency, benchmark budgets, and authoritative backfill all PENDING runtime execution (not claimed proven; no live/dev-store fixture; live Shopify validation depends on CORE-R2); draft PR — not merged, no new gate authorized** · **Exact-head runtime-closure note (2026-07-13, build 34844515 @ `9895919`, same AR-045 — no new AR ID):** the Task 011B runtime matrix was **executed for the first time** on the exact validated code SHA `9895919a6cc191cb24f694c1b601a0304fedda15` (build **34844515**, DB `…-34844515`, Odoo 19.0, PostgreSQL 16.14; build-to-commit verified by `git rev-parse HEAD` + install.log; base `9128015` proven ancestor; net scope exactly the 8 Task 011B files). **Fresh build-time `-i` install is green — `0 failed, 0 error(s) of 357 tests`, 0 WARNING**; field `store/index/readonly=t/t/t` with a real `character varying` column + a `USING btree` index; CORE-R2 Foundation schema present. Post-init `-u` reruns: product `0/53`, **sale (011B) `0 failed, 0 error(s) of 80` with all 32 `TestCustomerMatchingScalability` methods, 0 WARNING** (re-confirmed green again after all runtime ops). The **100k benchmark PASSED** with exact in-Python+DB corpus (`100000/70000/30000/1500/10500/88000/100000`) and exact probe mix (`700/150/100/50`); p50 0.328 ms / p95 0.934 ms / max 1.296 ms; 2249 cust/s. **EXPLAIN on a 100k corpus proves an Index Scan on `res_partner__shopify_connector_email_normalized_index`** (0.029 ms, 4 pages) vs a forced Seq Scan (28.6 ms, 100 043 rows removed, 5823 pages) — no full-partner scan. A **genuine single-DB `-u shopify_connector_sale` backfill** recreated the column+index and backfilled all 100 000 rows (≈4.81 s isolated; cross-validates the 4.44 s recompute proxy). **Two gates stay OPEN, reported not hidden:** (a) the **genuine independent-transaction concurrency proof deterministically fails (4/4)** on this shared/pooled Odoo.sh dev DB — root-caused to an **environment limitation** (odoo backends invisible in `pg_stat_activity`; a standalone `psycopg2` positive control proves the DB itself supports the race and correct `pg_blocking_pids` attribution; the job reaches the correct terminal `blocked_manual_review`/`binding_conflict` with exactly one binding every run), so the harness's in-process lock attribution cannot function here — needs a dedicated runtime host; (b) the **fully-authoritative isolated base→head build upgrade** cannot be provisioned (single linked DB). **Issue #157** (`res_users.notification_type` NOT-NULL) reproduced only on `-u shopify_connector_core` reruns (6 `setUpClass` errors, base-Odoo `res.users`, orthogonal to sale) — classified separately, **not fixed**. **No Task 011B production or test defect found → no code changed (LOOP 9 no-op); leak audit clean; zero synthetic residue; cron restored.** Evidence: [`./task-011b-validation-results.md`](./task-011b-validation-results.md) §18. **SRR-03 remains OPEN; live Shopify validation not performed; PR #150 kept draft/unmerged; no gate advanced.** · **Exact-head concurrency runtime-confirmation note (2026-07-14, build 34863138 @ `662e980`, DB `…-34863138`, control-room acceptance `4687882165`, same AR-045 — no new AR ID):** the §18.6 concurrency **"environment limitation / GATE OPEN"** classification is now **superseded** — the accepted §19 test-only `Registry._lock` post_install-deadlock fix (CORE-R2 §4.2 pattern; `ec07fd9`, concurrency test only, +117/−4) was **confirmed on the exact validated code SHA `662e9809c7b2443a0391f417ca2dff7daa3da29e`** on a fresh Odoo.sh build (Odoo 19.0, PostgreSQL 16.14; `git rev-parse HEAD` == the SHA, tree clean; base `9128015` proven ancestor; net scope exactly the 8 Task 011B files; **no Task 010B / issue-157 file**; the two range commits `ec07fd9` + docs-only `662e980` touch only the test + this record, production `models/**` byte-unchanged). **Fresh build install (4 modules, `--test-enable`) green — `0 failed, 0 error(s) of 357`, 0 WARNING**, benchmark/concurrency correctly absent from the standard pass. **Sale standard suite `0 failed, 0 error(s) of 80`** (90 stats), all **32** `TestCustomerMatchingScalability` methods, opt-in excluded — **re-confirmed green again after** the concurrency runs (no leaked lock). **Genuine independent-transaction concurrency = 3/3 stable:** every run reached the full 12-phase worker trail (`worker_thread_entered … after_api_environment … before_dispatch … after_commit … worker_done`), distinct backend PIDs (4116713/4116715, 4117922/4117925, 4118743/4118746), an **A-attributed binding-`INSERT` block** (`pg_blocking_pids(B)∋A` + INSERT predicate) that **cleared on A's commit**, first collision `unknown_system_error → retry_waiting` (rc=1, `next_retry_at` set), clean retry `binding_conflict → blocked_manual_review`, exactly **1 binding / 1 partner** surviving on `gid_a`, and zero cleanup residue. **`Registry._lock` restored** to the original object in the test `finally` (post worker-termination + cursor-close). **0 WARNING, 0 ERROR-level, 0 idle-in-transaction / leaked backends** (only an INFO pool `used=0` close); **cron id 3 restored active**; leak scan 0 emails/tokens/SQL/GraphQL. **Prior 100k benchmark + EXPLAIN + single-DB backfill (build `34844515`, §18) carried forward, NOT re-run** (only the concurrency test + docs changed). **No Task 011B production/test defect found → no code change** (validated concurrency SHA stays `662e980`); docs-only evidence commit advances the branch head, itself not runtime-tested. **Fully-authoritative isolated base→head backfill gate remains OPEN; issue #157 stays separate/not fixed; SRR-03 remains OPEN; live Shopify blocked; PR #150 kept open/draft/unmerged — not ready, not merged.** Evidence: [`./task-011b-validation-results.md`](./task-011b-validation-results.md) §20. |
| AR-046 | 2026-07-11 | Task 010B — Product Import Completeness: implementation (draft PR into `Shopify-connector`) | After PR #149 (CORE-R1) merged (verified base SHA `f9c3c5fd25af3f94ee71cc2ead3821e7da85443d`, no drift) and ChatGPT's Task 010B gate act (PR #149 comment `4948723366`, one draft PR only), this session implements Task 010B exactly per [`../07-implementation-plan/task-010b-product-import-completeness-packet.md`](../07-implementation-plan/task-010b-product-import-completeness-packet.md) (D-010B-1..12), completing the accepted DEC-003 product-import scope on top of the Task 010 slice. **D-010B-1** extended read-only query with explicit `variants(first:100, after:$cursor)` pagination (loop to exhaustion; 2,048 accumulated cap; malformed pageInfo / missing endCursor → `data_shape_schema_mismatch`; never a mutation). **D-010B-2** options→attributes→values→lines with the existing-attribute compatibility gate (reuse only `create_variant=='dynamic'`; incompatible `always`/`no_variant` → `product_import_attribute_conflict_mode` default `manual_review`→`binding_conflict`, else `connector_owned` distinct `"<name> (Shopify)"`; existing modes never changed) and a DB-backed global serialization lock — NEW singleton model `shopify.connector.attribute.lock`, one `noupdate=1` seed row, acquired with the verified Odoo 19 `try_lock_for_update()` (FOR UPDATE SKIP LOCKED) before any global attribute resolve/create; unavailable lock → `concurrency_race_conflict` (auto-retry, never unprotected creation); a **real two-transaction test** proves exactly one attribute is created. **D-010B-3** sparse variant instantiation via the verified `product.template._create_product_variant(combination)` (Odoo variants ≡ Shopify variants; no cartesian phantoms; structural mismatch → `binding_conflict`). **D-010B-4** base price gated on `price_source_of_truth=='shopify_authoritative'` (single→`list_price`, multi→`min` + exact additive `price_extra` decomposition via `float_compare`, undecomposable→`min`+note, no invented extras). **D-010B-5** `shopify_compare_at_price` on `product.product` (import-filled, shop-currency). **D-010B-6** primary+variant image import (HTTPS-only, redirect-HTTPS-only, content-type `image/*`, 20 MB streamed cap, bounded timeouts, no credential on the media request; checksum-ownership protection of merchant-modified images; `product_import_media_enabled` default True; failures→`shopify_temporary_server_network`, never a hold). **D-010B-7** `product_import_refresh_mode` (`snapshot_only` default | `shopify_fields`; structural adds in both; `updatedAt` payload-hash dedup). **D-010B-8** null/ARCHIVED → binding stale + note, Odoo master untouched; null-unbound → data error. **D-010B-9** N+1 removal (prefetched binding map + batched SKU/barcode candidate searches; match priority unchanged, no name matching). **D-010B-10** one savepoint per product; reads+downloads before write scope. **D-010B-12** job contract unchanged (no new job type/flag/error class; no mutation). Validation record: [`./task-010b-validation-results.md`](./task-010b-validation-results.md) | **Draft PR open for ChatGPT review** — the 010B gate closes on draft-PR-open; nothing accepted/merged; PR stays draft; no other task started | Completes the accepted DEC-003 product-import scope (attributes/variants/prices/media/refresh/archival) that the merged Task 010 slice left narrow, so order import (Task 012) can resolve lines through complete variant bindings; the concurrent-attribute duplicate race (final-convergence review `4947866018` item 5) is prevented at creation time by the `try_lock_for_update()` singleton lock, proven by a real concurrent-transaction test, not a reconciliation sweep | Verified against the **actual Odoo 19.0 source** in-session: `create_variant` selection/default/immutability write-guard; `_create_product_variant(combination, log_warning=False)` signature+semantics; dynamic-mode `_create_variant_ids` (no cartesian); `price_extra` field; `image_1920`/`image_variant_1920`; `try_lock_for_update(*, allow_referencing=False, limit=None)` = `FOR UPDATE SKIP LOCKED`, returns locked subset (empty if already locked). Shopify 2026-07 fields verified from the accepted captures (`ProductVariant.compareAtPrice` shop-currency `Money`; `variants(first,after)`+`pageInfo`; 2,048 ceiling; null-product deletion / `ARCHIVED`; `ProductVariant.image` **deprecated** but packet-named for a read — recorded). Local: `compileall`/`py_compile` clean, XML well-formed, source-guards green, 110 static test methods (was 61). **Odoo.sh + live/dev-store evidence NOT obtained this session (no runtime, no credentials) — recorded as outstanding, not faked, not waived** | Honestly outstanding before merge acceptance: the full three-suite Odoo.sh run (verbatim `0 failed, 0 error(s)`) and the §7 dev-store read-only evidence (three-option sparse; >100-variant; 2,048-variant timing probe; product+variant images with refresh + merchant-image-protection; archived; deleted-bound; incompatible same-name attribute). p95/2,048 timing not measured here. `ProductVariant.image` deprecation is a known future migration. TransactionCase cannot fully exercise multi-worker concurrency (the two-cursor test is the strongest in-repo proof; the named multi-worker caveat stands) | ChatGPT reviews the draft PR against the packet (D-010B-1..12), this validation record, and (when available) the Odoo.sh + dev-store evidence; then the merge review. No Task 011B/LC-1/012/Area-6/SEC-1/inventory/fulfillment/product-export/UI/webhook/OAuth/PERF-1 work is authorized or started by this task | **Draft PR open — awaiting ChatGPT review; Odoo.sh + live-Shopify evidence outstanding before merge (not faked, not waived); no new gate authorized** |
| AR-048 | 2026-07-15 | CORE-R2 — durable job-execution ownership & remote replay safety (design only; draft PR into `claude/core-r2-slice-2b-integration`, not `Shopify-connector`) | Control-room review (`4700703933`) of PR #163's runtime-correction attempt found that a transaction-scoped row lock (`try_lock_for_update`) prevents same-worker/same-`_drain_one` replay only — after a genuine PostgreSQL rollback, another worker can legitimately claim the released job row and re-invoke the real handler, and the recovered job's `concurrency_race_conflict` class is unconditionally auto-retried, scheduling a further automatic replay — in both cases with no proof a prior Shopify transport did not already occur. This session designs, but does not implement, the production contract that closes this gap: **effectively-once delivery, differentiated by declared operation class** (read-only / Shopify-`@idempotent` mutation / reconcile-before-retry mutation / ambiguous-manual-review mutation / local-only), a fail-closed **replay-safety registry** (`_get_replay_policies()`, mirroring the existing `_get_handlers()` seam — an undeclared `job_type` defaults to the most conservative class, never inherited read-safe behavior), and **Option A** durable ownership (commit `state='running'` + a new `attempt_id`/`owner_worker_ref` immediately after claim, *before* the handler runs; finalize only after an `attempt_id` compare-and-swap under a re-acquired lock; a new timeout-driven stale-owner sweep, never a blind requeue) — extending, not discarding, PR #163's own accepted re-raise-and-recover mechanism. Proven, not assumed: the existing `shopify.connector.call.lease`/`execute_business` (CORE-R2 Slice 1/2A, dormant — no production call site uses it) cannot be reused as-is, because its lifecycle (admit-before-call → release-after-local-reconciliation, both on independent side-cursors) ends *before* the outer job-dispatch transaction's own terminal-state commit — the exact window this gap concerns — and its designed purpose (a disconnect-quiescence signal) is different in kind from per-job-attempt ownership/outcome tracking. Smallest additive vocabulary change: no 17th `error_class` — a new `transport_attempted` job field, consulted alongside the registry, decides whether a caught `concurrency_race_conflict` may still auto-retry (no transport attempted) or must route to the **existing** `blocked_manual_review`/`manual_review_subreason='duplicate_risk'` outcome (transport attempted, ambiguous, non-idempotent) — `duplicate_risk` already exists in the accepted DEC-009 vocabulary, so no new state or error class is required for that outcome. Full evidence, four-option comparison (row-lock-only/PR #163 as-is — insufficient, proven by its own documented ownership contract; committed-running-state — recommended; dedicated attempt/lease table — evaluated, deferred as more schema than current scope needs; durable outbox protocol — evaluated, correct long-term destination but heavier than current scope, and Option A's schema is a strict subset so choosing it now does not foreclose outbox later), replay-registry design, crash/stale-owner recovery table, state/error mapping, nine-slice implementation sequencing, and self-critique against every named adversarial scenario (Worker B winning after rollback; lost/ambiguous response; non-idempotent mutation; duplicate cron workers; lease expiry/takeover; retry exhaustion; disconnect-controller interaction — flagged unanalyzed, not silently assumed fine): [`../03-architecture/core-r2-job-execution-replay-safety.md`](../03-architecture/core-r2-job-execution-replay-safety.md); decision record [`DEC-031`](../04-decisions/DEC-031-core-r2-job-execution-replay-safety.md). This session independently re-verified official Odoo 19 (`github.com/odoo/odoo` `19.0` branch) and Shopify Admin GraphQL API (`shopify.dev`) evidence directly (access date 2026-07-15), confirming: Odoo's own cron-acquisition code already treats a concurrency conflict as "move to the next job, never retry in place" (independent corroboration for this record's recovery direction); `retrying()` re-invokes the whole wrapped function including side effects and is not even part of the cron execution path; `refundCreate` and the inventory/location mutation family become Shopify-`@idempotent`-mandatory under API 2026-04 (sharpens DEC-010's inventory posture and extends the same future-candidate status to a not-yet-scoped refund domain); fulfillment's core mutations and `productSet` are confirmed **not** on Shopify's idempotent-mutation surface (supports DEC-011's and Task 015's own already-recorded non-idempotent framing). No rejected approach (RA-001…RA-024, checked in full) is re-proposed; a `pg_advisory_xact_lock` ownership/quiescence barrier is **not** re-proposed, consistent with its prior rejection recorded in `sync-engine-risk-register.md` SRR-09 (a documentation gap is flagged, not created, by this session: that rejection has no mirrored `RA-xxx` row today). | Gives the connector an explicit, evidence-grounded, source-verified production contract for durable job-execution ownership and remote replay safety before any further CORE-R2 implementation is authorized — and prevents a future mutation handler (fulfillment, inventory export, refunds, product export — all still gated/unimplemented) from silently inheriting the read-safe retry behavior that is only actually safe for today's shipped read-only handlers | Control-room review/acceptance of the semantic contract, the replay-safety registry design, and the recommended Option-A ownership architecture — **satisfied 2026-07-15, review `4701644819`, for Layer 1 only**; a future, separately-authorized implementation-gate session per architecture doc §9 (Immediate Slice 1, then Immediate Slice 2); genuine multi-worker/multi-server runtime proof (inherited from, not resolved by, SRR-04/SRR-09/DEC-025) remains owed before SRR-03 itself can close | No code, test, or PR #163 change; PR #163 stays draft/unmerged, runtime evidence retained but implementation not accepted, blocked by this replay-safety architecture; the disconnect-quiescence controller's own timeout and this record's stale-owner-sweep timeout are two independent mechanisms whose interaction is not yet jointly analyzed (flagged, §10 of the architecture doc); SRR-03 remains OPEN; Shopify-connector unchanged | ChatGPT reviews the draft PR (docs-only, targeting `claude/core-r2-slice-2b-integration`, not `Shopify-connector`) and the four-option comparison, and — **now that Layer 1 is accepted (review `4701644819`)** — later performs a separate CORE-R2 implementation-gate act naming the then-current base SHA, per architecture doc §9's Immediate Slice 1 / Immediate Slice 2 sequencing; no addon code is authorized by this review | **Proposed for ChatGPT review — design only; no gate opened.** · **Scope-narrowing revision (2026-07-15, control-room review `4701015790`, same AR-048 — no new AR ID):** the original draft made the full Option-A durable-ownership protocol (`attempt_id`/`owner_worker_ref`/`transport_attempted`, two commits per job, a new stale-owner cron) an immediate requirement — disproportionate to current UAT scope, where every implemented Shopify handler is read-only. The proposal is now split into **Layer 1** (decided now, routed to DEC-031 for acceptance): a minimal fail-closed replay-policy registry with exactly **three** declared classes (`local_only`, `remote_read_replay_safe`, `remote_effect_not_replay_safe` — not five), explicit declarations for the core diagnostic/self-test handler and the current customer/product import handlers, a fail-closed default for every undeclared `job_type`, and PR #163's existing rollback/reset/re-lock/revalidate/bounded-retry behavior **accepted as-is, unmodified**, for the current read-only scope — **no new model, field, migration, or cron proposed**. Task 012 order import is explicitly **not** pre-registered by this decision; it must declare `remote_read_replay_safe` on its own verified read-only design when implemented. **Layer 2** (Option A, persisted attempt identity, transport-ambiguity tracking, stale-owner recovery, Shopify idempotency-key persistence, reconciliation-before-retry, mutation-specific multi-worker runtime proof) is retained, unchanged in substance, as a **deferred** future architecture gate — not decided, not scheduled, and not authorized by this revision — reopened by name only when inventory export, fulfillment/tracking update, product export, refund creation, or any other Shopify mutation is authorized for implementation; it does not block current read-only product/customer/order UAT. The nine-slice future implementation sequence is replaced with two immediate slices (registry implementation; exact-head runtime validation) plus one deferred-roadmap paragraph for Layer 2 — no numbered future sessions. Evidence and option analysis are unchanged; only the recommendation, DEC-031's decision text, and the implementation slicing are narrowed. **PR #163 runtime evidence retained; PR #163 implementation still not accepted.** DEC-031 and AR-048 remain **Proposed / pending control-room acceptance**; SRR-03 remains OPEN; Prompt E remains BLOCKED; `Shopify-connector` unchanged; current UAT fast-track is read-only product/customer/order work. Updated: [`../03-architecture/core-r2-job-execution-replay-safety.md`](../03-architecture/core-r2-job-execution-replay-safety.md), [`DEC-031`](../04-decisions/DEC-031-core-r2-job-execution-replay-safety.md), this row, [`task-core-r2-slice-2b-handoff.md`](../07-implementation-plan/task-core-r2-slice-2b-handoff.md), [`research-handoff.md`](../01-research/research-handoff.md) — same PR #164 (docs-only, draft), new head. · **Formal acceptance (2026-07-15, control-room review `4701644819`, same AR-048 — no new AR ID): AR-048 status is now ACCEPTED.** The Layer 1 / Layer 2 split is accepted in substance — no architecture redesign or additional research required. **Accepted Layer 1 scope:** the minimal fail-closed replay-policy registry (`local_only`/`remote_read_replay_safe`/`remote_effect_not_replay_safe`); explicit current handler classifications (only `core_dispatch_selftest` is a registered dispatcher handler per `_get_handlers()`, verified against `shopify_connector_job_dispatch.py:145-161`; `core_readiness_check`/`core_manual_maintenance`/`core_test_connection` are job-type vocabulary outside the dispatcher registry, not registered handlers); current customer/product import handlers → `remote_read_replay_safe`; current read-only bounded retries (PR #163's mechanism, unmodified); no new schema/model/migration/cron. **Layer 2 remains deferred**, not implementation-authorized until the first Shopify-mutation domain (inventory export, fulfillment/tracking update, product export, refund creation, or any other Shopify write). This acceptance pass also applied a small docs-only editorial closure patch: registry-completeness wording corrected to key off `_get_handlers()`, not `JOB_STATE_SELECTION`; stale nine-slice references replaced with Immediate Slice 1 / Immediate Slice 2 / the deferred Layer 2 architecture gate; the Layer 1 crash-recovery claim corrected to "policy gating only," not "strictly improved." **SRR-03 remains OPEN; Prompt E remains BLOCKED; PR #163 remains draft/unmerged and untouched; Task 012 implementation is not authorized by this acceptance; `Shopify-connector` unchanged.** Updated: architecture doc, DEC-031, this row, `task-core-r2-slice-2b-handoff.md`, `research-handoff.md` — same PR #164, new closure head, merge authorized into `claude/core-r2-slice-2b-integration` only. |
| AR-047 | 2026-07-12 | Task CORE-R2 — disconnect quiescence & in-flight job contract (design only; draft PR into `Shopify-connector`) | After PR #153 merged (accepted evidence, `4950408383`; SRR-03 kept OPEN, DEF-PB-1 confirmed) and ChatGPT's CORE-R2 **design** gate act (PR #153 comment `4950413650`, docs-only, one session, verified base `fcbbb0b3fe3db9cba354a8a1c08e91036b70ec1f`), this session designs the DEF-PB-1/SRR-03 remediation and prepares a future implementation packet — **no code**. **Revision 4 (2026-07-12, review `4680299311`; base remains `cfdb05703a65f82b34a9a11364aab6fc960cca9d` via the earlier base-sync amendment, superseding `65e915a`; PR #152/U0 + PR #155/U0-closure preserved; this row is now AR-047 per the control-room ID amendment `4680291189` — repository allocation AR-044 = U0 acceptance closure, AR-045 = Task 011B/PR #150, AR-046 = Task 010B/PR #151, AR-047 = CORE-R2/PR #154 — renumbered AR-045 → AR-047, never overwriting or combining the reserved rows). Rev 4 makes the design implementation-safe with six corrections: **(A)** admission is atomic with disconnect via a store-row LOCK protocol (`FOR SHARE` admission vs `FOR NO KEY UPDATE`/`FOR UPDATE` for every generation-changing transition — the lock, not the bare lease commit, is the linearization point; both lock orders proved; concurrent admissions preserved; `store→credential` order deadlock-free); **(B)** `execute_business` frozen as a CONTEXT MANAGER wrapping the HTTP call AND its local reconciliation (lease can't end early / be forgotten); the two domain call-site edits become structural `with` blocks, not one-line swaps; **(C)** DIRECTION-C expired-lease rule (expired = unknown/live; `completed` needs zero rows; crashed holder → distinct `timed_out`; cleanup only after `timed_out`); **(D)** corrected controller selection `try_lock_for_update(limit=1)` = `FOR UPDATE SKIP LOCKED LIMIT 1` (picks the next unlocked store) + bounded `POLL_DELAY` cadence via delayed `_trigger(at=…)` (no busy loop); **(E)** ordered rollback requiring ZERO holders (workers drained first) before code/model removal** freezes the load-bearing mechanisms after rev 2's quiescence signal proved **broken**: `run_drain` runs `running`→handler→terminal in **one uncommitted transaction**, so `state='running'` is **invisible cross-transaction** and a `count(state='running')` quiesce poll returns zero mid-handler. Rev 3 therefore: (a) enforces INV-2 at an **explicit `execute_business(job, store, query)`** boundary (mandatory `job`, no `env.context` carrier) + `execute_lifecycle(purpose=…)` with a state matrix, public `execute()` removed — with an **exhaustive call-site inventory** (core test-connection = lifecycle; product + customer importers = business, requiring **two named minimal call-site edits** in the domain modules, added to the future allowlist); (b) proves INV-3 with a **committed admission-lease** (`shopify.connector.call.lease`, side-cursor committed before each call, released after local reconciliation, expiry+reaper crash recovery, **multiple leases per store**) instead of a `running` count; (c) makes admission **atomic with a single in-memory token snapshot** passed to `_send(store, body, token)` (the second credential lookup removed); (d) pins a **one-store-per-cron-invocation** controller (store-row SKIP LOCKED selection, no explicit main-cursor commit) and **removes** the "dedicated worker slot / cannot starve" claim (priority 0 = prioritization only; completion is an SLA under a healthy scheduler; safety is immediate via the gate); (e) freezes the **lifecycle state matrix** (disconnect one-way; lifecycle mutations refused during `disconnecting`; epoch bumps on every successful activation/reconnect); (f) **lease-based** timeout escalation (holder count/ids/timestamps on store fields, never a locked job row); (g) an **ordered rollback** (stop controller → freeze transitions → normalize `disconnecting` stores → clear leases → preserve audits → revert → verify). The advisory lock stays dropped (shared/exclusive variant evaluated and rejected for lack of observability). Source-verified: REPEATABLE READ (`odoo/sql_db.py:373`), `retrying()` (`odoo/service/model.py:160`), non-blocking `FOR UPDATE SKIP LOCKED` (`odoo/orm/models.py:5564`/`5592`), cron `_trigger`/priority + single-commit + `_commit_progress` (`odoo/addons/base/models/ir_cron.py:735`/`303`/`671`/`846`). Deliverables: [`../03-architecture/disconnect-quiescence-remediation-analysis.md`](../03-architecture/disconnect-quiescence-remediation-analysis.md) (26 sections) and [`../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md`](../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md); SRR-03/04/09 wording, master-plan CORE-R2 dependency, and research-handoff top entry updated. | **Proposed for ChatGPT review — design only; no gate opened.** Revision 4 answers the six `4680299311` blocking corrections (store-row lock atomic-admission protocol; context-manager reconciliation lifetime; direction-C expired-lease rule; corrected `try_lock_for_update(limit=1)` selection + `POLL_DELAY` cadence; zero-holders ordered rollback; AR-045 → AR-047 renumber), on top of rev 3's nine `4951237871` points (transaction-visibility of `running`; committed admission-lease quiescence; atomic admission + single token snapshot; bounded SET of admitted calls; explicit `execute_business`/`execute_lifecycle` + call-site inventory; frozen lifecycle matrix; lease-based timeout; ordered rollback; base merge). The analysis §26 enumerates six calls (D-CR2-A…F). Accepting the PR without named exceptions accepts these as proposals; the CORE-R2 **implementation** gate is a **separate, later** explicit act on the packet. | Gives the DEF-PB-1/SRR-03 remediation an internally-complete, source-grounded contract and an executable future packet so, once gated, implementation is fast and defensible — and so no Shopify-calling handler (incl. reads) ships/live-validates on top of an unremediated disconnect race | The runtime evidence (PR #153 §10/§14 + `scenario-06-real-timeline.md`); the Odoo 19 / PostgreSQL 16 source citations in the analysis; **live Odoo.sh + genuine two-server proof are required by the packet before SRR-03 can move off OPEN** — none claimed by this design session | Residual races stated honestly (RR-1 gate→`_send` window; RR-2 already-issued call; RR-3 timeout escalation; RR-4 multi-host; RR-5 context-stripping guarded by a source-level test; RR-6 SRR-06 concurrency-fix-needs-live-proof). No addon/test/code changed; six-file docs-only scope; SRR-03 OPEN, SRR-04/09 REDUCED-not-closed unchanged | ChatGPT reviews the draft PR, makes the six §26 calls, and — if accepted — later performs a **separate** CORE-R2 implementation-gate act (naming the then-current base SHA and re-freezing the packet's allowed-file list) in a new session; no addon code is authorized by this review | **Proposed for ChatGPT review (rev 4) — design/packet only; implementation gate CLOSED** · **Foundation-implementation revision (2026-07-12, same AR-047 — no new AR ID):** the CORE-R2 **implementation** gate was OPENED by control-room comment `4952145926` (authorized base `Shopify-connector` @ `ce504f42824807e215ee21df3dfd4eed9bb9a275`, ratifying D-CR2-A…F). **Foundation Slice 1** implements a strict subset of the packet on branch `claude/core-r2-implementation-foundation` (draft PR into `Shopify-connector`, base `ce504f`): persisted `store.connection_generation`; enqueue-captured `job.expected_connection_generation`; the secret-free committed `shopify.connector.call.lease` model (`store_id` M2o, opaque `lease_key` unique, **Integer** `job_id`, `worker_ref`, `admitted_at`, `expires_at`) + one minimal admin ACL; the atomic `_admit` (owned side cursor → store-row `FOR SHARE` → fresh state/generation gate → single token read → committed lease → commit-releases-lock); `_release_lease`; `execute_business(job, store, query, variables=None)` as a real `@contextlib.contextmanager` releasing on normal **and** exception exit; and token-read-once `_send(store, body, token)`. **Deliberately deferred to later slices (NOT in this slice):** the `disconnecting` state, `action_disconnect` rewrite, the conflicting lifecycle update-lock protocol, the disconnect controller/cron, `timed_out`/`completed` finalization + credential-clear, and the product/customer call-site migration — so the admission-vs-disconnect linearization is **not yet closed end to end**. The slice is **dormant** (no production call site enters `execute_business`; legacy `execute()` unchanged). Focused tests are **authored but not executed** (no Odoo runtime this session). **SRR-03 remains OPEN; no remediation and no runtime-green is claimed.** · **Correction revision (2026-07-12, control-room review `4680664964`, same AR-047 — no new AR ID):** three reliability blockers corrected on the same branch/PR (draft, unmerged) — (1) `execute_business` now preserves the `execute()` response/error contract (same missing-config `UserError`; missing-token → accepted `ShopifyClientError(ERROR_AUTH, REASON_TOKEN_INVALID, credential_invalid=True)` **before** the lease/`_send`; `RequestException` → `ShopifyClientError(ERROR_TEMPORARY, …)`; yields `_normalize_response(...)`, not the raw response) while keeping the two-arg `_send` seam and the single captured token; (2) genuine independent-connection tests now invoke the **real** `execute_business`/`_admit`/lease-ORM/credential/`_release_lease` (registry-cursor factory patched to real pooled cursors for the bounded window; raw SQL only for observation/cleanup) — proving real pre-`_send` lease visibility, caller-rollback independence, and two real concurrent admissions with distinct leases; (3) deterministic exception precedence (body/`_send`/normalize error stays primary; a release failure is chained via `raise … from …`, never substituted; release-once). A pre-existing fixture bug (`action_set_token` demotes `connected`→`reconnect_needed`) was also fixed. Synchronous adversarial review: API-contract corrections confirmed correct; genuine-test mechanics sound. **Still dormant; SRR-03 remains OPEN; tests authored but not executed (no Odoo runtime); no runtime-green claimed.** · **Hardening revision (2026-07-13, control-room review `4681564744`, same AR-047 — no new AR ID):** one final reliability-hardening pass on the same branch/PR (draft, unmerged), limited to `api_client.py` + `test_disconnect_quiescence.py` (+ these docs) — (1) every genuine worker/production side cursor is transaction-locally bounded (`statement_timeout` + `lock_timeout` via parameterized `set_config(..., true)` through a new `_open_bounded` helper that closes the cursor if the bound setup fails); (2) worker threads are joined and **proven dead inside the registry-cursor patch** (before restore) and again before cleanup — no live worker under the restored factory, no cleanup while a worker is live, daemon only a backstop; (3) fail-loud **sanitized** worker diagnostics via a thread-safe queue (type-only: phase + exception type + safe `error_class`; no raw exception/`str`/`repr`/`%r`/SQL/path/token), rollback and close failures surfaced separately; (4) hardened zero-residue verification (bounded cursor, unittest assertions, job-log residue checked, always closed); (5) `execute_business` successful-release path now uses a **bare `raise`** (original identity + traceback preserved, release once), dual-failure keeps `raise … from …`. Synchronous adversarial review of the hardening pass: **no confirmed defects.** **Still dormant; SRR-03 remains OPEN; tests authored but not executed (no Odoo runtime); no runtime-green claimed.** · **Runtime-validation revision (2026-07-13, runtime-operator session, same AR-047 — no new AR ID):** the foundation-slice tests were **executed for the first time** inside the authorized Odoo.sh dev build (build `34808200`, Odoo 19.0, DB `adamsmen-claude-core-r2-implementation-foundation-34808200`; build-to-commit verified by `git rev-parse HEAD` in the build container + install.log fresh-install record). Four defects were found and corrected — **all in the authorized test file `test_disconnect_quiescence.py` only; no production/model/transport code changed and no invariant (locking, generation gate, token-read-once, lease durability, exception taxonomy, cursor bounds, thread containment, cleanup) weakened:** (1) `TestBusinessAdmission` now enters Odoo's sanctioned `registry_enter_test_mode()` so `_admit`'s genuinely-independent side cursor can see the class's uncommitted fixture (was 12 `row is None` `ShopifyQuiescedError`); (2) the bare-re-raise traceback assertion now captures the **live** exception in its own handler (`unittest.assertRaises` strips `__traceback__` via `with_traceback(None)`, making the old check vacuous); (3) lease-key opacity is now proven deterministically via `uuid4` version/variant instead of a flaky decimal-substring check that false-positives when the store id's digits appear in a random 32-hex key; (4) the two-real-concurrent-admissions test decouples its worker threads from the reentrant `Registry._lock` that `ThreadedServer.run()` holds across the whole post_install phase (`service/server.py:706`) — a spawned thread's `api.Environment` → `Registry.__new__` → `with cls._lock:` otherwise dead-locks and the admission code never runs; after the fix the two genuine concurrent admissions complete in ~0.1 s with distinct committed leases (3/3 stable). Pre-fix synthetic DB residue (5 stores/10 jobs/3 leases) left by the deadlocked runs (whose `finally` hit `_assert_workers_dead` before `_cleanup`) was scrubbed, restoring zero residue and clearing a transient pre-existing `TestJobDispatch` pollution failure. Verbatim after-fix suites (clean DB): core `0 failed, 6 error(s) of 131`; product `0 failed, 0 error(s) of 53`; sale `0 failed, 1 error(s) of 41`. **All 7 remaining errors are PRE-EXISTING, non-CORE-R2 `res_users.notification_type` NOT-NULL `setUpClass` failures** (then reported as `2 failed, 18 error(s)` on the pristine slice head — **refined by the exact-head note below: the fresh install is green `0 of 325`, and these seven are a base Odoo-19 `mail` computed-field artifact seen only on post-init re-runs**), in files **outside this PR and this session's allowed-files list**, so left untouched and flagged for the appropriate non-CORE-R2 owner. Observed green: real admission (lease committed pre-`_send`, cross-connection visible, survives caller rollback, two concurrent coexist with distinct keys/correct jobs, released on exit); API-contract parity (`UserError`/`ERROR_AUTH`-before-lease/normalized-result/`ERROR_TEMPORARY`/GraphQL-normalize/token-once/legacy-2-arg); exception precedence (primary preserved, release chained, bare-reraise identity+traceback, release-once); zero synthetic residue; **no live Shopify call**. Evidence: [`task-core-r2-validation-results.md`](task-core-r2-validation-results.md) §4.1–4.3. **SRR-03 remains OPEN — runtime-green of the admission half does NOT close the disconnect-quiescence remediation; the disconnect controller, the conflicting lifecycle update-lock/epoch bump, and Direction-C finalization stay deferred, so admission-vs-disconnect linearization is not closed end to end. PR #156 stays draft/unmerged; no gate advanced.** · **Exact-head runtime-closure revision (2026-07-13, build 34818964 @ `c0d4559`, same AR-047 — no new AR ID):** the full runtime matrix was re-run inside the authorized Odoo.sh dev build for the **exact validated code SHA `c0d455938b4a087407d6c712acbcc8bcf1b06feb`** (build **34818964**, DB `…-34818964`; superseding the earlier `34808200` numbers). The **fresh build-time `-i` install** (all four modules, `--test-enable`, demo) is **fully green — `0 failed, 0 error(s) of 325 tests`**, with every CORE-R2 class and every one of the seven `notification_type` classes running and passing. Post-init re-runs (`-u`/`--test-tags`, identical counts): core `0 failed, 6 error(s) of 122` (+9 post = 131), product `0 failed, 0 of 53`, sale `0 failed, 1 of 41`; CORE-R2 classes green (`TestCallLeaseModelSchema` 7, `TestBusinessAdmission` 18, `TestApiClient` 20, `TestJobEnqueue` 10, `TestGenuineRealAdmission` 9 — 3/3 stable, no deadlock). **The seven `res_users.notification_type` `setUpClass` errors are a base Odoo-19 `mail` computed-field artifact that manifests only on post-init re-runs (not on the green fresh install); the failing traceback is 100% base Odoo (zero CORE-R2 frames), the seven test files are byte-identical on `ce504f`, and the CORE-R2 diff never touches `res.users` — base/pre-existing, not a CORE-R2 regression** (a literal separate-DB `ce504f` run was impossible: no `createdb`, single-DB platform — RR-F). Zero WARNING lines; every SQL ERROR is either that base artifact or an expected duplicate-/required-field negative-test assertion in a passing test; zero synthetic residue; no token/credential/GraphQL/payload/worker-exception leakage; no live Shopify call. **No CORE-R2 defect found → no code change** (validated code SHA stays `c0d4559`); docs-only evidence commit advances the branch head. Evidence: [`task-core-r2-validation-results.md`](task-core-r2-validation-results.md) §4.1/§4.3/§4.4. **SRR-03 remains OPEN; PR #156 kept draft/unmerged; Slice 2 not started.** |
| AR-049 | 2026-07-15 | Wave 1 packet dependency reconciliation (docs-only; Sol's Wave 1 hard-stop resolution) | Sol's first Wave 1 launch stopped before any branch/PR/code (issue #167 comment `4980808811`), reporting three conflicts: (1) `task-sec1-security-hardening-packet.md` requires "Area 6 merged runtime-green" because D-SEC1-3 names `action_manual_retry()`/`action_cancel()` (D-A6-5, Area 6's own scope) as sanctioned doors, but Area 6 is unauthorized Wave 2+ scope gated on Task 012; (2) SEC-1's §5 allowlist names `addons/shopify_connector_sale/models/shopify_connector_order_binding.py`, which does not exist (Task 012 unstarted); (3) `module-lifecycle-uninstall-design.md`'s accepted LC-1 design assumes LC-1 lands before SEC-1 (its cancellation write uses the current non-su mechanism specifically because SEC-1 hasn't landed), but the Wave-0-accepted recommended order and `mvp-completion-program.md` §4 both placed SEC-1 before LC-1. Claude control room independently re-verified all three against primary sources — `git grep addons/**/*.py` for `action_manual_retry`/`action_cancel`/`action_resolve_manual_review` returns zero matches; `git ls-tree` confirms no order-binding file exists; the design doc's own final-convergence note explicitly requires LC-1-before-SEC-1 ordering — and confirmed each is real, not merely a documentation gap. **Resolution (routed to [`DEC-034`](../04-decisions/DEC-034-wave-1-packet-dependency-reconciliation.md)):** re-freeze Wave 1 to CORE-R1 → LC-1 → Task JOB-ACTIONS (NEW packet, `../07-implementation-plan/task-job-actions-generic-core-packet.md`, extracting D-A6-5's two generic job-control methods into their own independently gated Wave 1 prerequisite — resolving Area 6's own "Flagged for ChatGPT" ownership question) → SEC-1 (rescoped to the three binding models + job model that exist at the Wave 1 baseline, order-binding line removed, LC-1's historic-job helper and Task JOB-ACTIONS's two methods added to its D-SEC1-2 sanctioned-writer inventory) → SRR-03 closure. A binding-extension contract requires every future binding model (Task 012 order, Task 013 location/inventory, Task 015B media) to declare its own `_odoo_binding_field_name()` seam in its own packet when created, fail-closed (`False`) by mixin default in the interim. No Area-6 scan/cron/domain scope and no Task 012 file are absorbed into Wave 1. | Gives Wave 1 a coherent, implementation-ready internal order and exhaustive per-stage allowlist with no forward reference to unauthorized scope, so a fresh Sol launch does not hard-stop on the same packaging contradiction a second time; preserves DEC-030's already-accepted LC-1 design unchanged; resolves Area 6's own packet text flag rather than inventing a new design | Re-verification against the live baseline (SHAs, `git grep`/`git ls-tree` — obtained this session) and a control-room adversarial consistency check of the reconciled packet set before merge (Phase 7 of this session's task) | This is a sequencing/packet-ownership correction, not MVP scope expansion — no addon code, protected reference, or Wave boundary changes; if the adversarial check finds a residual inconsistency, the PR stays draft and DEC-034 stays Proposed rather than being marked Accepted prematurely | Control-room completes the adversarial consistency check (Phase 7) and, if ACCEPT, marks DEC-034 Accepted, marks the docs-only PR ready, merges into `mvp/program-integration` with a normal merge commit, and records the merge SHA in issue #167; a fresh Sol Wave 1 launch follows, referencing the corrected packets | **Proposed for control-room review — reconciliation drafted, adversarial consistency check pending before AR-049/DEC-034 acceptance.** · **Formal acceptance (2026-07-15, same AR-049 — no new AR ID):** the Phase 7 adversarial consistency check ran ten independent verifier agents against the live PR diff; 9/10 PASS, one real gap found (D-SEC1-2's protected-field set omitted `cancel_reason`, the field Task JOB-ACTIONS's `action_cancel()` writes under its own audited/permission-gated contract — would have stayed a plain ACL-writable field, unaudited and ungated, once SEC-1's su-guard landed) and corrected on the same PR (field added to D-SEC1-2's protected set with rationale; `write({'cancel_reason': …})` added to D-SEC1-7's negative test matrix). **AR-049 status is now ACCEPTED; DEC-034 is Accepted.** No addon/test code changed by this session; the reconciliation PR merges docs-only into `mvp/program-integration`; Wave 2 remains unauthorized; SRR-03 remains OPEN. |

| AR-050 | 2026-07-15 | Task LC-1 — accepted DEC-030 lifecycle implementation | Implement the already-accepted disable-first plus supported-domain-uninstall posture: permanent historic job type, set-once original type, audited cancellation of non-terminal rows, terminal retyping without log loss, domain selection-removal callables, additive upgrade backfill, and fail-closed historic dispatch | **Implementation and prior exact-head runtime conform to accepted DEC-030; corrected-head regression repeat GREEN (build 34995642)** | This is the smallest implementation of the locked lifecycle design and keeps domain-owned bindings subject to the documented export/re-derivation posture rather than moving them into core | Build `34986844` at prior exact head `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723` proved the lifecycle suite `0/0/9` plus full fresh/standard suites; the corrected-head regression repeat ran on build `34995642` (SHA `95db3db`) as part of the full `0/0/644` standard suite | Physical uninstall intentionally loses domain binding/mapping tables; rollback must not drop the additive column or delete audit/business data | Corrected-head repeat recorded in `task-lc1-validation-results.md`; Claude reviews only at the complete Wave 1 boundary | **Implemented on draft PR #172; prior runtime green; corrected-head repeat GREEN (build 34995642); awaiting final Claude merge decision** |
| AR-051 | 2026-07-15 | Task JOB-ACTIONS — generic core manual retry/cancel doors | Implement accepted D-JA-1 as a pure additive job-model extension with exact state and role gates, mandatory cancellation reason, one audit row, and no bypass; reserve only the two write-site elevations for SEC-1 | **Implementation and prior exact-head runtime conform to accepted DEC-034; corrected-head regression repeat GREEN (build 34995642)** | Keeps generic job controls in core without importing Wave 2 Area 6 scan/domain-trigger scope and gives SEC-1 concrete sanctioned action doors | Build `34986844` at prior exact head `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723` proved the full standard suite green; the corrected-head action regression repeat ran on build `34995642` (SHA `95db3db`, `TestJobActions` (9) green) as part of the full `0/0/644` standard suite | Incorrect group or state gates could bypass recovery policy; SEC-1 must add only the reserved elevation while preserving behavior | Corrected-head repeat recorded in `task-job-actions-validation-results.md`; Claude reviews only at complete Wave 1 boundary | **Implemented on draft PR #172; prior runtime green; corrected-head repeat GREEN (build 34995642); awaiting final Claude merge decision** |
| AR-052 | 2026-07-15 | Task SEC-1 — protected job/binding mutations and PII controls | Implement accepted D-SEC1-1..7 on the existing Wave 1 substrate, including all product-owner completeness/runtime rulings: protected `original_job_type`; lifecycle audit carrier; `env.company` consistency; accepted CORE-R2 recovery transitions; test-only AST guard compatibility; and the complete binding identity/structure/system-state/provenance/imported-snapshot surface | **CORRECTION IMPLEMENTED — corrected exact-head runtime GREEN (build 34995642, SHA `95db3db`, full standard suite `0/0/644`)** | Claude's review found the binding guard covered identity but omitted provenance and imported snapshots. Product-owner ruling `4988842625` required the complete current surface and a fail-closed extension seam. Commit `36974edc68c1985e6ccfae8f6bb5c7386f820156` implements that correction without new ACLs, groups, models, tables, job types/sources, context bypasses, dispatcher changes, or Wave 2 scope | Static proof: all changed Python parses; exact 16/17/14 protected sets asserted; all four roles receive create/alter/clear negative coverage; sanctioned override/import/mask/retention behavior remains covered; product importer sudo inventory changes only at the accepted existing-variant refresh and image-checksum writes (relative to this correction commit's immediate parent — see `task-sec1-validation-results.md`'s Sudo-delta note for the full-wave count). Prior build `34986844` remains valid for prior SHA `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723`; the corrected exact-head Odoo.sh matrix has since run and is green | The correction is production security code and therefore could not inherit the earlier exact-head runtime conclusion; it has now been independently runtime-proven at the corrected head. SRR-03 remains CLOSED because its independent runtime proof and production path are unchanged. No exactly-once remote-effects or DEC-031 Layer 2 claim | The complete corrected-head matrix ran in the single runtime-operator handoff (build 34995642); PR #172 is back with Claude for the final macro-wave re-review and merge decision; Claude alone may merge | **Draft PR #172 corrected-head runtime-green; awaiting final Claude merge decision; SRR-03 CLOSED; Wave 2 unauthorized** |
| AR-053 | 2026-07-17 | Fable gap-closure PR #173 — final control-room product/architecture decision & merge review | Review the corrected **docs-only** PR #173 (`fable/mvp-remaining-gap-closure` @ `09078a8` → `mvp/program-integration` @ `1e46c23`) as a complete product/architecture/UX/QA/Waves-2–6-readiness package; **confirm** binding Class A rulings A-1..A-18; **decide** Class B PD-B1..B7; **disposition** Class C TA-C1..C8; **validate** Class D EQ-D1..D7 fail-closed classification; **confirm** Class E post-MVP | **Accepted with amendments (docs-only)** — every Class B item decided, every Class C item decided or routed to a named dedicated gate; this reconciliation commit records the decisions and the PR is then **merged** into `mvp/program-integration` with a normal (no-squash/no-rebase) merge commit | Class A faithfully/consistently recorded across canonical docs, Wave 2–6 DoRs, QA/UAT matrices, and 28 rendered prototypes; DEC-031 Layer 2 design verified against all **8 hard safety properties** (no lock across a remote call; never exactly-once; intent-before-send; not-attempted vs uncertain; reconcile-before-retry; fail-closed without a registered strategy; Wave-1 mechanics preserved; crash/stale-owner coverage); cross-document consistency **clean** (no current-facing contradiction on masking / Mode-2-optional / Wave-1-pending / SRR-03-open / PR-172-unmerged / exactly-once / Layer-2-Accepted / Wave-2-authorized) | Identity re-verified live from GitHub (PR open/draft/**clean**; head `09078a8` / base `1e46c23` exact; protected refs `acd8c46` / `dd6ecb8` / `a5d4543` unchanged); all **126** changed paths under `docs/**`; **no** `addons/**` / security / ACL / migration / test / manifest change | **DEC-031 Layer 2 is NOT DEC-accepted** — its design is accepted only as the authoritative *proposal* routed to a dedicated pre-Wave-3 architecture gate; the amendments (B1 = 24 h default; B4 = exact mode-switch scan boundary; B5 = Mode-1-outbound is **Full**, not Lite) could mis-scope a wave if ignored, so each is written into its canonical doc; no implementation authorized. **Clarification (AR-054, below): the "SRR-03-open" phrase in this row's own "Evidence required" column is a checklist item — it names one of the *false/stale claim patterns the cross-document search confirmed were absent*, not a statement that SRR-03 was open as of this row's date. SRR-03 was, and remains, CLOSED continuously since Wave 1's merge (2026-07-16); this note exists only because the phrase reads ambiguously out of context, per an exact-codebase preflight agent's independent misreading during the AR-054 session — not because any document asserted SRR-03 was reopened.** | Decision record: [`../04-decisions/fable-proposed-decision-pack.md`](../04-decisions/fable-proposed-decision-pack.md) §Control-room decisions (2026-07-17); B1 → lifecycle §7; B4 → fulfillment-operating-modes §6; B5/C3 → modular-architecture §6/§9; C5 → SEC-2 packet §D. Wave 2 opens only via its own separate decision-acceptance + DoR + packet re-acceptance + exact-base preflight session | **PR #173 ACCEPTED with amendments; reconciled by this commit and merged into `mvp/program-integration`; DEC-031 Layer 2 not accepted; Wave 2 unauthorized/unstarted; protected refs unchanged** |
| AR-054 | 2026-07-17 | Wave 2 decision-acceptance + Definition-of-Ready + Task 012/Area-6 packet re-acceptance + exact-base preflight | Perform the exact next-session prompt from `research-handoff.md`'s top entry: exact-codebase preflight of `shopify_connector_core`/`_product`/`_sale` against the Task 012 packet, its decision closure, and the Area-6 packet; reconcile the live tracker; **accept** the Wave 2 Definition of Ready and its §3 gate-decision table; **re-accept** the Task 012 packet (with its 2026-07-16 addendum) as one canonical contract; **accept** the Area-6 order-scan slice only; **disposition** every open question (OQ-A..E, OQ-COD-6, OQ-RB-1/5/6) via a fresh official-Shopify-evidence check where time-sensitive; freeze the exhaustive allowed/forbidden file list; issue (not execute) the locked Sol Wave 2 prompt | **ACCEPTED (docs-only)** — Wave 2 gate PR opened as an early draft, reviewed, and merged into `mvp/program-integration` with a normal (no-squash/no-rebase) merge commit | Exact-codebase preflight (14-agent parallel research fan-out over the full addon trees and the full planning corpus) found the Task 012/Area-6 packets structurally sound and consistent with live code, with two real gaps neither packet had named: no `order_domain_enabled` settings flag exists and core's `ACCEPTED_DOMAIN_FLAGS` has no extension seam (resolved by reusing `sale_domain_enabled`, exactly as `customer_import_sync` already does — zero core edit); and no AST guard yet scopes future order-domain files to `execute_business`-only (resolved by requiring Task 012/Area-6 to add their own guard, mirroring the existing product/sale pattern). Both recorded in [`DEC-035`](../04-decisions/DEC-035-wave-2-open-question-dispositions.md) as EQ-PF-1/EQ-PF-2 | Live GitHub identity verification (starting SHA `a34c68e84aada288dad3dc22a6afe94f5ace0652` confirmed exact; PR #172/#173 merge states, Wave 1 acceptance, SRR-03 closure, protected refs, and absence of any existing order-domain code all independently re-verified, not assumed); a live fetch of current `developer.shopify.com` Admin GraphQL docs (`OrderDisplayFinancialStatus`, `OrderTransaction`/`Kind`/`Status`, `OrderSortKeys`) for the mixed-transaction and `OrderSortKeys` open questions, cited with exact URLs and access status in DEC-035 | Mixed-transaction orders (OQ-D) needed a genuine design decision beyond "fail closed to review" — resolved by reusing the binding mixin's existing `status='review'` value plus a forced quotation-hold, rather than inventing a new `manual_review_subreason` selection value, since no sibling module has ever extended that specific field and doing so would have introduced untested vocabulary; the chosen mechanism uses only already-tested, already-protected vocabulary | Decision record: [`DEC-035-wave-2-open-question-dispositions.md`](../04-decisions/DEC-035-wave-2-open-question-dispositions.md); packet re-acceptance sections in `task-012-order-import-implementation-packet.md` §0 and `area-6-sync-triggers-implementation-packet.md` §0; `wave-2-definition-of-ready.md` §3/§5; locked prompt at [`../06-prompts/sol-wave-2-order-import-locked-prompt.md`](../06-prompts/sol-wave-2-order-import-locked-prompt.md). Wave 2 implementation opens only when that locked prompt is separately issued to a Sol session | **Wave 2 Definition of Ready: ACCEPTED. Task 012 packet: RE-ACCEPTED. Area-6 order-scan slice: ACCEPTED. Wave 2: AUTHORIZED TO START. Wave 2 implementation: NOT STARTED. Protected refs unchanged. DEC-031 Layer 2 remains unaccepted.** |
| AR-055 | 2026-07-17 | Wave 2 Task 012 plus Area-6 order-scan implementation | Implement the accepted Wave 2 contract in `shopify_connector_sale`: PII-free protected order binding; four-query read-only Shopify order importer; exact financial/tax/customer/product fail-closed mapping; per-store policy; `order_import_sync` and enqueue-only `order_import_scan`; scheduled/manual/backfill discovery with 30-minute overlap and exact preview token; no Shopify mutation or Layer 2 | **Implementation assembled and static/source review green; runtime decision deferred until exact-head Odoo.sh evidence** | The implementation reuses accepted core enqueue/dispatch/replay/lifecycle/security seams, the product binding chain, and the customer match sequence without changing core/product/customer files; explicit caps and sudo sites are recorded in the validation files | Exact-head Odoo 19 fresh install and baseline upgrade; focused 86-test order suite; full core/product/sale regression; two genuine database-concurrency proofs; lifecycle/uninstall/reinstall; SRR-03 smoke; residue/session/lease/worker/cursor/cron/token/credential/PII audit | Runtime compatibility, module upgrade and teardown cannot be inferred from static parsing; three-decimal currency onboarding remains fail-closed pending named dev-store evidence; read-only dev-store UAT is preferred but may be deferred to Wave 6 | Run the operator matrix in `task-012-order-import-validation-results.md` and `task-area6-order-scan-validation-results.md` at the then-current PR #176 head; append runtime evidence before control-room review | **Evidence pending — hard-stop 5; PR #176 draft/unmerged; Wave 3+ unstarted** |

_**AR-041 numbering / merge-sequencing note (2026-07-10, ChatGPT control-room
review comment
[`4934676616`](https://github.com/AdamsOdoo/Adams/pull/146#issuecomment-4934676616)
on PR #146) — REVISE applied in the same PR.** This row was originally
drafted and pushed as "AR-040"; ChatGPT's review caught that **PR #145**
(the Task 011 customer-import implementation draft PR, opened earlier and
currently under runtime-fix review) already owns **AR-040** for its own
package. This row and every self-reference to it across the package
(`research-handoff.md`, `mbq-05-branch-b-final-decision-brief.md`,
`DEC-026-distribution-auth-branch-b-proposal.md`,
`mbq-05-branch-b-next-implementation-implications.md`,
`open-points-closure-register.md` §3.7,
`implementation-readiness-map.md` §4.1, and the MBQ-05 row in
`master-blueprint-open-questions.md`) were renumbered to **AR-041** in the
same PR #146, before any acceptance. **Merge-sequencing rule, recorded
here per that review comment: PR #145 owns AR-040; PR #146 owns AR-041;
PR #146 must not be merged ahead of PR #145 unless ChatGPT explicitly
decides to reorder/renumber the AR sequence; if PR #145 merges first, PR
#146 must be refreshed against the new `Shopify-connector` tip before its
own final merge review.** This note does not itself accept DEC-026, does
not decide MBQ-05 branch B, and does not affect PR #145 or Task 011 in
any way._

_**AR-046 renumber / base-sync + allocation-reconciliation note (2026-07-12,
control-room base-synchronization amendment then AR-allocation
reconciliation).** The Task 010B row above was originally drafted and pushed
as **AR-044** (its four revision notes were "AR-044 revision note 1-4"). When
`Shopify-connector` advanced to `cfdb05703a65f82b34a9a11364aab6fc960cca9d`
(PR #155, U0 acceptance closure), the control room assigned **AR-044 to the U0
acceptance-closure row**; to avoid a collision, this row and its four revision
notes were first renumbered **AR-044 → AR-045** (then the next free ID; AR-044
retained for U0, not overwritten or combined). The control room subsequently
**reconciled the concurrent post-U0 allocation** (reviews
[`4680290878`](https://github.com/AdamsOdoo/Adams/pull/151#pullrequestreview-4680290878)
/ [`4680322434`](https://github.com/AdamsOdoo/Adams/pull/151#pullrequestreview-4680322434)):
**AR-045** is reserved for **Task 011B / PR #150** as the earliest concurrent
post-U0 row, and **AR-047** for **CORE-R2 / PR #154**. Task 010B was therefore
**ultimately assigned AR-046** (AR-044 → AR-045 → **AR-046**). Self-references
to the Task 010B AR row elsewhere in the package
(`task-010b-validation-results.md`,
`task-010b-product-import-completeness-packet.md`, `research-handoff.md`) were
updated to **AR-046** to match. The table's review IDs stay monotonic and
chronologically explicable — AR-044 (U0, 2026-07-12) precedes AR-046 (Task
010B, 2026-07-11) in the table: Task 010B originated earlier by date but
received the higher ID **AR-046** after the collision and the later
allocation reconciliation. **AR-045 is intentionally absent from this branch**
— it belongs to Task 011B / PR #150, which may not yet exist in the
integration base, so a temporary numeric gap (AR-044 → AR-046) is expected
here. This note records the renumber; it changes no Task 010B decision or
evidence._

_**PR #146 refresh note (2026-07-10) — merge-sequencing rule executed.**
PR #145 merged into `Shopify-connector`, merge commit
`7e83abba502c898fa413822c4d9b4866138a454a` (Task 011 customer import/
matching, `shopify_connector_sale`, runtime-green). Per the AR-041
numbering note above, this docs-only refresh session rebased branch
`claude/mbq-05-branch-b-research-w0m0p3` (PR #146) onto that new
`Shopify-connector` tip. The AR-040 row directly above (Task 011) is
PR #145's own already-merged content, preserved verbatim; the AR-041 row
is this package's unchanged, still-**Proposed** content. No entry was
duplicated. This refresh does not accept DEC-026, does not decide MBQ-05
...
on PR #144 (branch `claude/task-011-readiness-closure-wj4g2c`, base
`Shopify-connector`). The review decision was **REVISE before merge**,
accepting the package in principle (docs-only scope confirmed; final
prompt properly locked; gate closed; Tasks 011–015 unauthorized; D1–D8
recommendations; MBQ-05 branch B correctly non-blocking) and requiring:
**(1)** the D1 email-candidate discovery rule be made recall-safe — no
prefilter may exclude a partner whose `email_normalize(partner.email)`
equals the incoming normalized email (display-name/wrapped/mixed-case
forms); `'=ilike'` must not be used; the recall-safe substring prefilter
`('email', 'ilike', normalized_incoming)` + mandatory Python-side
normalized comparison (or no prefilter at all) applies to both the
active and archived-inclusive searches, with new test expectations —
applied to
...
D1/D8; **(2)** this merge-safety status patch — AR-039 recorded as
Accepted (this note), a post-merge-safe handoff acceptance note, and the
final prompt's usage-condition wording updated to distinguish
satisfied-by-this-acceptance/merge conditions from the still-unsatisfied
gate-act/base-verification/issuance conditions, with the DO-NOT-USE lock
preserved unweakened. **This acceptance authorizes nothing further:**
the customer-domain gate remains **closed** (the comment's own words:
"Do not open the gate"); the final prompt is **not issued** and not
usable; **Task 011, 012, 013, 014, and 015 remain unauthorized**;
**MBQ-05 branch B remains undecided**; no implementation of any kind may
start. The next acts after PR #144 merges are ChatGPT's own: the
distinct, explicit gate-opening act (confirming the gate proposal's
criterion-12 table as of that date), the separate MBQ-05 branch B
decision, and — only after the gate act — issuing the final prompt
verbatim in a new session. This note does not itself merge PR #144._

_**AR-038 Acceptance Note (2026-07-10) — PR #143 accepted by ChatGPT as
docs-only planning/audit closure**, via control-room review comment
[`4932199585`](https://github.com/AdamsOdoo/Adams/pull/143#issuecomment-4932199585)
on PR #143 (branch `claude/shopify-connector-planning-ue8ew7`, base
`Shopify-connector`), applied after the branch was refreshed against the
current `Shopify-connector` head (now including PR #142's merge commit
`8514d74dba1e606c77489b84165e991b8d9c11da`). ChatGPT accepts the AR-038
planning-closure package — the project readiness master audit, open-points
closure register, implementation readiness map, final pre-implementation
roadmap, UAT readiness gap analysis, and next-gate readiness roadmap — as
**docs-only planning/audit closure**. **This acceptance authorizes
nothing:** no implementation of any kind is authorized; **no
customer-domain gate is opened** (it remains closed); **no Task 011, 012,
013, 014, or 015 is authorized**; **no usable implementation prompt is
issued** — the eight ChatGPT decisions D1–D8 in
[`next-gate-readiness-roadmap.md`](../07-implementation-plan/next-gate-readiness-roadmap.md)
remain decision inputs/recommendations only, not final decisions. **The
next action after PR #143 merges is a separate Task 011 final-prompt /
customer-domain gate-opening-proposal session** — not performed by this
acceptance or this note. **MBQ-05 branch B remains a separate, distinct,
not-yet-made ChatGPT decision**, unaffected by this acceptance. This note
does not itself merge PR #143, open any gate, or start Task 011._

_**AR-037 Merge-Record Note (2026-07-09, post-merge) — PR #140 merged into
`Shopify-connector`, merge commit
`0e138d9ad8e14a9d0766122f30beea2e19df549a` (merged 2026-07-09T19:00:34Z,
verified via GitHub `pull_request_read`: `merged: true`, base
`Shopify-connector`, head `claude/task-011-customer-readiness-5xunqw`).**
This note records the merge only. The AR-037 row's acceptance (comment
`4928377625`) predates the merge and is unchanged by it. Consistent with
that acceptance's own explicit limits: merging PR #140 does **not** open
the customer-domain gate (still **closed**; criteria accepted **as
criteria only**, 7 of 15 satisfied per
...
§3/§5), does **not** authorize Task 011 or Task 012 implementation, and
does **not** draft or issue any implementation prompt. The row's
"PR #140 remains draft, unmerged" wording was accurate when written and
is superseded by this note. Recorded by the AR-038 audit session._
...
_**AR-046 revision note (2026-07-12, control-room static review `4950202231`
— Task 010B REVISE before runtime validation).** PR #151 stayed draft; the
first static review found several overclaimed decisions at head `1065cdf`,
all corrected on the same PR within the review's narrow file scope (the
importer, the template-binding model, four test files; docs): (1)
**pagination** is now strictly shape-validated (`data`/`product`/`variants`/
`pageInfo` mappings, `nodes` a list, Boolean `hasNextPage`, non-empty
`endCursor` when true) — a missing/null/wrong-type `pageInfo` is never
treated as a completed single page, all violations route to
...
field, stamped only after full success, skipping media/DB work on an
unchanged product — enqueue-level `payload_hash = updatedAt` recorded as an
**Area-6 obligation**, not implemented here, no running job's payload hash
mutated; (3) **same-URL image ownership** now compares the current Odoo
image checksum against the recorded connector checksum (merchant edit/clear
protected, not silently skipped); (4) **SVG/spoofed bodies rejected** (SVG
content-type rejected + Pillow raster validation, body never exposed); (5)
**bounded media memory** via `SpooledTemporaryFile` + `ExitStack` cleanup on
every path (no dict of full byte strings); (6) **`connector_owned` refresh**
resolves the exact Shopify-name or exact `"<name> (Shopify)"` line and fails
closed when both exist; (7) the **concurrency proof** now runs the real
`_apply_import` path across two genuinely independent PostgreSQL connections
(`odoo.sql_db.db_connect`) with committed synthetic stores — B holds the
lock, A's concurrent full import gets `concurrency_race_conflict`, A retries
and reuses B's committed attribute (one attribute, both products bound,
durable cleanup); no shared `TestCursor` is presented as independent
concurrency. Test methods 110 → **135**. **No runtime/live claim added** —
Odoo.sh and dev-store evidence remain outstanding (validation record §10).
PR #151 stays **open/draft/unmerged**; this revision stops for another
ChatGPT static review. No other task authorized._

_**AR-046 revision note 2 (2026-07-12, control-room static review
`4950339305` — Task 010B REVISE again before Odoo.sh).** The second static
review accepted Revision 1 and found five remaining reliability blockers at
head `5688ec1`, all corrected on the same PR (files: the importer, the
attribute-lock model, four test files; docs): (1) **pagination
forward-progress + identity guards** — `_fetch_product_with_all_variant_
pages` tracks seen cursors and rejects a repeated/non-progressing
`endCursor` (so a `hasNextPage=true`/same-cursor/zero-node response cannot
loop forever), requires every `nodes` element to be a mapping with a
non-empty GID, rejects duplicate variant GIDs within/across pages, and
requires each page's product `id` to equal the requested GID — all →
`data_shape_schema_mismatch`, no write; (2) **O(1) media handles** —
each image is streamed to its own secure `mkstemp` file, closed
immediately, with only the path retained and exactly one path reopened at
write time, and an `ExitStack` that unlinks every path on all exit paths
(replacing the Revision-1 many-open-`SpooledTemporaryFile` design; O(1)
open handles and bounded RAM at the 2,048-variant ceiling, no arbitrary
cap); (3) **mid-stream network classification** — a `RequestException`
raised while iterating `iter_content()` is caught, the response closed, the
partial file unlinked, and re-raised as `shopify_temporary_server_network`
(previously it escaped to `unknown_system_error`); (4) **archived routed
before media** — a bound archived product marks bindings stale with no
media/master write (a broken image URL cannot block stale marking), and a
first-seen unbound archived product creates no bare Odoo product/binding,
raising the existing `mapping_missing` class; (5) **lock/concurrency wording
corrected** (no behaviour change) — the singleton row lock is
transaction-scoped (savepoint release does not free it; held to outer
commit; serializes the rest of the transaction and, in a `run_drain` batch,
potentially the rest of the batch), the docstrings/comments/docs/PR body are
corrected, and the test is renamed
`test_overlapping_transactions_serialize_to_one_global_attribute` and
described as two independent **overlapping** (not simultaneous)
transactions; the lock-hold/throughput exposure is recorded as a mandatory
Odoo.sh/dev-store measurement, not a closed performance claim. Test methods
135 → **155**. **No runtime/live claim added** — Odoo.sh and dev-store
...
_**AR-046 revision note 3 (2026-07-12, control-room static review
...
_**AR-046 revision note 4 (2026-07-12, control-room acceptance `4951264328`
...
_**AR-046 revision note 5 (2026-07-12, control-room base-synchronization
amendment — Task 010B base sync to `cfdb057`).** `Shopify-connector` advanced
to `cfdb05703a65f82b34a9a11364aab6fc960cca9d` via merged **PR #155** (U0
acceptance closure). That exact tip was merged into the Task 010B branch with
a **normal merge commit** (no rebase/force/squash). PR #155's U0
acceptance-closure artifacts — the new research-handoff top entry, the U0
**AR-044** row (this ledger), the master-plan U0 status, and the
`docs/09-ui-prototype/**` updates — are **preserved unchanged**. The
**AR-044 collision** (this Task 010B row was also drafted as AR-044) was
resolved per control-room item 7 by renumbering the Task 010B row and its
four revision notes off the U0-assigned **AR-044** (ultimately to **AR-046**
after the later concurrent-allocation reconciliation — see the "AR-046
renumber / base-sync + allocation-reconciliation note" above for the full
AR-044 → AR-045 → AR-046 history); U0 keeps AR-044 (not overwritten or
combined). The two shared-doc
conflicts (`research-handoff.md`, this log) were resolved by **retaining both
sides completely** with chronological accuracy; **no addon or test file
conflicted**. The PR diff versus the new base is **Task-010B-only** (the
`shopify_connector_product` addon + the four Task 010B docs) plus the AR
renumber; `Shopify-connector` (`cfdb057`) is an ancestor of the new head.
**No production/test code changed; no runtime run.** Odoo.sh three-suite
...
_**AR-046 runtime-execution closure note (2026-07-13, runtime-operator session
— exact-head Odoo.sh runtime matrix; no code change).** The full Task 010B
runtime matrix was **executed for the first time** inside the authorized
Odoo.sh dev build for the **exact validated code SHA
`db534f833cf3636184681801dcd7e13636e09245`** (build **34828304**, Odoo 19.0, DB
`adamsmen-claude-product-import-completeness-010b-5l-34828304`; build-to-commit
proven by `git rev-parse HEAD` in the build container + the build's fresh-install
`install.log`; integration base `9128015` confirmed ancestor; working tree clean).
This closes the "opt-in performance harness **NOT executed**" gap (AR-046
revision notes / validation §0e–§0g) and the "Odoo.sh three-suite run **NOT
executed**" gap (validation §10). **Fresh build install (`-i … --test-enable`,
all four modules + demo): GREEN `0 failed, 0 error(s) of 433 tests` (0
`notification_type` violations).** Post-init standard suites (`-u`/`--test-tags`):
core `0 failed, 6 error(s) of 131`, **product `0 failed, 0 error(s) of 161`
(clean)**, sale `0 failed, 1 error(s) of 41` — **all 7 errors are the base
Odoo-19 `res_users.notification_type` `setUpClass` artifact (issue #157),
re-run-only, byte-identical helper, 100% base traceback, outside
`shopify_connector_product` and this branch's allowed-files list; not fixed or
absorbed here** (matches the AR-047 exact-head classification). **Opt-in
`sc010b_performance` — all four tests GREEN** with verbatim `SC010B_PERF_EVIDENCE`:
A (100-variant) `1.3845 s / 1673 q / 100 variants / 100 bindings / 1 attr`;
B (2,048-variant) `23.8545 s / 28967 q / 2048 variants / 2048 bindings /
declared_cartesian_space 2112 / 2 attrs` (no cartesian phantom; RSS ≈133 MB, 0
major page faults, no network/media); C (lock/retry on genuine independent PG
...
`run_drain`) `0.1322 s / jobs 2 / api_calls 2 / 1 attr` (exactly two seam calls,
no external network, no unrelated job consumed). **Stability: lock/retry 3/3
(+1 combined = 4), run_drain 3/3 (+1 = 4)** — all PASS, no deadlock/serialization/
timeout/residue, post-test query counts invariant (272 / 219). **Zero WARNING
lines**; every SQL ERROR is either the #157 base artifact or an expected
`assertRaises` negative-path constraint rejection in a passing test. **Zero
synthetic residue** (fresh bounded verifier: all categories 0; 0 total jobs; 0
store-credentials; no temp media; 0 idle DB sessions; no lingering worker); **no
token/header/GraphQL/media-byte/temp-path/worker-exception leakage; no live
Shopify call.** **Zero runtime defects → no code change (validated SHA stays
`db534f8`); docs-only evidence commit.** **SRR-03 remains OPEN; live/dev-store
Shopify evidence stays the only outstanding runtime gate (gated on CORE-R2
runtime-green); PR #151 kept draft/open/unmerged, not marked ready, not merged.**
Evidence: [`./task-010b-validation-results.md`](./task-010b-validation-results.md)
§0h._


| AR-056 | 2026-07-17 | Wave 2 complete pre-runtime implementation/traceability audit | Audit every accepted Task 012, DEC-035 and Area-6 requirement against production, positive/negative/security/concurrency tests, lifecycle proof and static tooling before freezing Odoo.sh input | **Accepted for runtime candidacy; runtime result remains pending** | Corrected permanent-binding and enqueue collision handling under PostgreSQL REPEATABLE READ; strengthened exact financial/configuration/ACL/atomicity/pagination tests; added one exact two-field sale-order read sudo so connector-only Reviewer/Admin authorization is deterministic without granting generic Sales access | 86 unique test methods; five query-only GraphQL operations; exact 50-field protected set; five-site sudo inventory; compile/AST/XML/CSV/manifest/import/duplicate-ID/mutation/tax-create/placeholder scans clean | Odoo model setup, ACL enforcement, PostgreSQL overlap, install/upgrade/uninstall/reinstall, regression and residue/security scans remain runtime-only; read-only dev-store proof may defer to Wave 6 | Run the complete matrix recorded in Task 012 and Area-6 validation at the exact final PR #176 head | **Static green; PR draft; no Odoo pass claimed; Wave 3 unstarted** |
| AR-057 | 2026-07-18 | Wave 3 Gate A — DEC-031 Layer 2 architecture closure and Stage 0 packet preparation (draft PR #177) | Normalize the inconsistent `L2-D1`–`L2-D13`/`L2-D14`/`L2-D15` Layer 2 decision numbering into one complete, gap-free `L2-D1`–`L2-D38` inventory via a 27-agent code/documentation audit, official-Shopify/Odoo-source refresh, and adversarial review of 35 numbered decisions + 15 lettered risks (8 thematic clusters + 1 consolidation pass); cut a Wave 3 Stage 0 implementation packet and a locked (not issued) Sol prompt from the result; incorporate a mid-session binding control-room parallel-audit ruling ([PR #177 comment `5012854989`](https://github.com/AdamsOdoo/Adams/pull/177#issuecomment-5012854989)) correcting isolation-level, CAS-field, THROTTLED, batching, clean-cursor-invariant, and security-guard assumptions | **PROPOSED FOR CONTROL-ROOM ACCEPTANCE — NOT YET ACCEPTED; package explicitly NOT FROZEN** | This session (Session A) does not accept its own decision package, per its own governing rule. Eight decisions are explicitly BLOCKING (`mutation_attempt.job_id` field type + C2 cursor placement — D20; open-transaction-vs-network-call proof — D22; disconnect-quiescence/sweep-timeout interaction — D28; `mutation_domain` field ownership — D35; N=3 inconclusive-cap persistence scope — D17; repo-wide AST-tooling-maturity contradiction — D37; full runtime/concurrency/crash-injection proof plan — D38) requiring explicit control-room decisions or empirical verification not available from documentation alone. A separately-tracked "Session C" code/architecture audit is stated by the control room to still require reconciliation before any freeze; this session cannot access its content and does not claim to have reconciled with it | Every corrected fact independently re-verified against primary sources by this session (not merely accepted from the ruling): Odoo 19 `odoo/sql_db.py` confirms `ISOLATION_LEVEL_REPEATABLE_READ` set on every cursor; Shopify's `InventoryQuantityInput`/`InventorySetQuantitiesInput`/idempotency-mandate changelog/rate-limits pages confirm `changeFromQuantity`, the 2026-04 mandatory-idempotency date, the 24h retention window, and the undocumented THROTTLED-execution guarantee — full citations in [`shopify-layer2-mutation-safety-refresh-2026-07-18.md`](../00-source-materials/shopify-layer2-mutation-safety-refresh-2026-07-18.md) | Getting the two field-ownership/schema BLOCKING items (D20, D35) wrong reproduces failure classes DEC-030 and the existing `call_lease` lock-contention lesson already exist to prevent; shipping D38's runtime proof incompletely (simulation substituted for genuine OS-process crash injection) would be evidence-inverted given SRR-04/SRR-09 remain open | Control-room independent review of DEC-036, the corrected design doc, and the Stage 0 packet; resolve or explicitly risk-accept every BLOCKING item; reconcile against Session C's audit; only then may the locked Sol Stage 0 prompt be issued | **DEC-036: PROPOSED FOR CONTROL-ROOM ACCEPTANCE. Layer 2 design: CONTROL-ROOM ACCEPTANCE CANDIDATE — NOT YET ACCEPTED. Stage 0 packet: PROPOSED — LOCKED PROMPT NOT ISSUED. Wave 3 DoR: STILL PROPOSED — GATE A COMPLETE, GATE B PENDING. No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify request or mutation; PR #177 open/draft/unmerged; Wave 3 unstarted.** |
| AR-058 | 2026-07-19 | Wave 3 Gate A — final consolidated Sessions-2-and-3 correction batch (draft PR #177) | Apply, in one documentation-only correction batch, every binding decision in the control-room's final consolidated ruling ([PR #177 comment `5014689445`](https://github.com/AdamsOdoo/Adams/pull/177#issuecomment-5014689445)) reconciling an independent official-source audit ("Session 2") and an independent adversarial architecture audit ("Session 3") against AR-057's package, following a preliminary review ([PR #177 comment `5013028262`](https://github.com/AdamsOdoo/Adams/pull/177#issuecomment-5013028262)) that found ten further corrections beyond AR-057's own eight BLOCKING items | **CONTROL-ROOM ACCEPTANCE CANDIDATE — CORRECTIONS APPLIED, NOT YET ACCEPTED; zero remaining architecture blockers** | All eight of AR-057's explicitly BLOCKING decisions are resolved: `mutation_attempt.job_id` (`Many2one`-restrict) and C2 cursor placement (dedicated side cursor) — D20; the open-transaction-vs-network-call question, resolved by construction — D22; the disconnect-quiescence interaction, resolved to an awareness-based design rather than a timeout race — D28; `mutation_domain` field ownership, resolved to a registry-validated indexed `Char` (a third option AR-057's original two-option framing omitted) — D35; the N=3 inconclusive-cap's persistence scope, resolved per-attempt-sufficient given the retry-eligibility sequencing guarantee — D17; the AST-tooling-maturity contradiction, reclassified as implementation sizing rather than an architecture blocker — D37; the runtime/concurrency/crash-injection proof requirement, resolved to four distinct proof-environment layers (not three), correctly classified as a Stage 0 merge-acceptance criterion rather than a precondition to beginning implementation — D38 | Additional corrections applied: the observed-outcome/resolution model made genuinely orthogonal — `observed_outcome` immutable once it leaves `pending`, never overwritten by a human/reconciliation resolution, read only through a new shared effective-disposition helper — D10/D11; the single, unsafe `request_fingerprint` (which excluded `changeFromQuantity`) split into `business_intent_fingerprint`/`exact_request_fingerprint`, the latter including `changeFromQuantity` and the idempotency key — D5; two idempotency defect error codes routed to fail-closed manual review instead of ordinary DEC-009 auto-retry — D6; the reconciliation-job linkage corrected from a non-FK Char to a required `Many2one`-restrict — D14; retention corrected from unconditional retain-forever to indefinite-for-unresolved plus configurable-masking-for-resolved-terminal — D32; security confirmed installing against the current four-role model with an explicit SEC-2 re-key obligation, rejecting Session 3's proposal to install against the future two-role model — D30/D31. Session 3's proposed single six-value outcome enum is explicitly rejected; Session 2's own conclusion that Odoo uses PostgreSQL default Read Committed isolation remains explicitly rejected (Odoo 19 REPEATABLE READ governs, independently re-verified, unchanged from AR-057) | Getting the observed-outcome-immutability correction wrong would have let a human/reconciliation judgment silently overwrite machine-observed transport evidence — the exact failure class the three-layer model exists to prevent; getting the fingerprint split wrong would have left idempotency-key reuse decisions relying on a hash that omits a transmitted request parameter, a genuine duplicate-mutation risk | Full reconciliation record, including every Session 3 recommendation accepted vs. rejected and the rationale for each: `DEC-036` Part 0.5. Control-room independent review and acceptance act on the corrected package remains the next required step; only then may the locked Sol Stage 0 prompt be issued | **DEC-036: CONTROL-ROOM ACCEPTANCE CANDIDATE — CORRECTIONS APPLIED, NOT YET ACCEPTED, zero remaining architecture blockers. Layer 2 design: CONTROL-ROOM ACCEPTANCE CANDIDATE — NOT YET ACCEPTED. Stage 0 packet: PROPOSED — LOCKED PROMPT NOT ISSUED. Wave 3 DoR: GATE A CORRECTION COMPLETE — CONTROL-ROOM FINAL REVIEW PENDING, GATE B NOT STARTED. No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify request or mutation; PR #177 open/draft/unmerged; Wave 3 unstarted. Recommendation: READY FOR FINAL CONTROL-ROOM GATE A REVIEW.** |
| AR-059 | 2026-07-19 | Wave 3 Gate A — final mechanical consistency correction and control-room acceptance act (draft PR #177) | Apply one final narrow documentation-only consistency correction closing the six remaining textual contradictions identified by the control room's final Gate A review ([PR #177 comment `5014806430`](https://github.com/AdamsOdoo/Adams/pull/177#issuecomment-5014806430)); then, on control-room acceptance, apply two clerical merge-closure fixes and record the Gate A acceptance-status act across the Gate A documentation set | **ACCEPTED — CONTROL-ROOM GATE A** | The correction batch closed the CAS-terminology, gate-numbering, batching, Stage 0 allowed/forbidden-list, registry-owner, and D1 field-count contradictions AR-058 left open; the control room then independently verified the corrected package (PR #177 comment [`5015044226`](https://github.com/AdamsOdoo/Adams/pull/177#issuecomment-5015044226)) and accepted it, subject to two clerical merge-closure conditions (DoR gate-table wording; Stage 0 packet §4 addendum sentence), both applied in the acceptance/closure commit | Identity re-verified live from GitHub before each act (PR open/draft/unmerged/mergeable; exact head each time; exactly the authorized files changed; protected refs unchanged); no addon/code/test/security/XML/manifest/CI change in either commit | No new architecture decision, blocker, or open question was introduced by either the correction batch or the acceptance act; DEC-036's Category I (implementation-proof) and Category II (tunable-constant) items remain open Stage 0 merge-acceptance criteria, not architecture blockers | Merge PR #177 into `mvp/program-integration` with a standard merge commit; verify the merge commit and integration tip; Stage 0 and Gate B open only via their own separate control-room issuance/acceptance acts | **DEC-036: ACCEPTED — CONTROL-ROOM GATE A. Layer 2 design: ACCEPTED through DEC-036 D1–D38. Stage 0 packet: ACCEPTED — IMPLEMENTATION PROMPT NOT YET ISSUED. Locked Sol prompt: LOCKED, unissued, ready for separate control-room issuance after merge. Wave 3 DoR: GATE A ACCEPTED; GATE B NOT STARTED; STAGE 0 IMPLEMENTATION NOT YET STARTED. No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify request or mutation. PR #177 authorized for the acceptance act and controlled merge.** |
| AR-060 | 2026-07-19 | Wave 3 Gate B — Task 013/013B inventory-readiness acceptance candidate (draft PR #179) | After PR #177 merged (verified merge commit `3a2043cb8d45a4b9bc7bdb3ea39b58515e706da9`, `mvp/program-integration` confirmed identical, per PR #177 comment `5015174971`), this session produces the Wave 3 Gate B documentation-only acceptance candidate: [`DEC-037`](../04-decisions/DEC-037-wave-3-inventory-gate-b.md) closes every remaining Task 013/013B contradiction carried forward by DEC-036 Part 0.5 — `changeFromQuantity` CAS field name throughout (superseding stale `compareQuantity` references in the inventory operating model and the Task 013 packet's addendum heading); binding-owned idempotency (`last_push_idempotency_key`/`last_push_params_hash`) removed from the inventory-level-binding schema and superseded by attempt-owned Layer 2 idempotency (`shopify.connector.mutation.attempt`); unexplained Shopify-side drift made explicitly review-case-first and blocking, never auto-overwritten (superseding the packet's "push over after logging a drift note" posture); one-pair-per-request made binding MVP behavior, not a floor awaiting batching; a complete `inventorySetQuantities`/`inventoryActivate` Layer 2 mutation-domain matrix (DEC-037 §4) with a new activation-then-set-quantities sequencing design (§5, two distinct Layer 2 attempts, never combined); the Task 013 job contract frozen (§7); Task 013B's Layer-2-non-applicability made explicit in its own packet (new §0); a 17-scenario dev-store mutation-validation plan; and two locked, unissued Sol prompts | **PROPOSED FOR CONTROL-ROOM GATE B ACCEPTANCE — NOT YET ACCEPTED; Claude did not accept its own package** | Two narrow official-source verifications performed live against `shopify.dev` (2026-07-19, API 2026-07) beyond the accepted Gate A capture: `inventoryActivate`'s omitted-quantity default (`available`/`onHand` default to zero) and its `userErrors` shape (`UserError` — `field`/`message` only, **no dedicated error-code enum**, unlike `inventorySetQuantities`) — both folded into the matrix's `inventory_activate` row with an explicit fail-closed default for the one genuinely open classification question (message-based idempotency-defect matching, verification deferred to the dev-store plan's scenario 8) | Identity gate independently re-verified live before any edit (PR #177 merge commit and `mvp/program-integration` tip; branch/PR non-existence; all three binding comments read in full; Stage 0 branch exists with no PR, not merged; protected refs unchanged); DEC-037 does not reopen any DEC-036 decision — it propagates the fourteen already-binding decisions unchanged and resolves only the domain-specific questions DEC-036 explicitly deferred to Gate B | Misclassifying `inventoryActivate`'s error shape (no code enum) as if it had the same rich taxonomy as `inventorySetQuantities` would silently under- or over-classify activation failures; resolved by payload-shape classification (null vs. non-null `inventoryLevel`) rather than message-string matching wherever a structural signal exists, with message-matching reserved only for the idempotency-defect codes and flagged for dev-store verification | Control-room review and acceptance of DEC-037; only then may either locked Sol prompt (Task 013, Task 013B) be issued, and only after Stage 0 (Task 013) or Task 013 (Task 013B) is separately merged and runtime-proven | **DEC-037: PROPOSED FOR CONTROL-ROOM GATE B ACCEPTANCE. Task 013 packet: GATE B ACCEPTANCE CANDIDATE — NOT IMPLEMENTATION AUTHORIZED. Task 013B packet: GATE B ACCEPTANCE CANDIDATE — NOT IMPLEMENTATION AUTHORIZED. Both locked Sol prompts: LOCKED, unissued. Wave 3 DoR: GATE A ACCEPTED; GATE B CONTROL-ROOM REVIEW PENDING; STAGE 0 STATUS REPORTED FACTUALLY (Sol branch exists, no PR, not merged). No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify mutation (two Shopify reads only, for official-source verification). PR #179 open/draft/unmerged.** |
| AR-061 | 2026-07-19 | Wave 3 Gate B Revision 2 — job-model correction per control-room REVISE ruling (draft PR #179) | The control room reviewed AR-060's Gate B Revision 1 candidate and returned **REVISE, NOT REJECTED** (PR #179 comment [`5015619162`](https://github.com/AdamsOdoo/Adams/pull/179#issuecomment-5015619162)): Revision 1's same-job, two-sequential-mutation-attempt design for `inventoryActivate`/`inventorySetQuantities` conflicted with Gate A's binding "one mutation job : one Shopify mutation request : one attempt row" rule and with the actual Stage 0 extension seam. This session applies one coherent documentation-only correction batch implementing all six binding corrections: (1) three standalone job types — `inventory_push_sync` demoted to orchestration/read-only (no mutation, no `mutation.attempt` row), `inventory_activate` and `inventory_set_quantities` each its own mutation job (`job_type == mutation_domain`); (2) exact Stage 0 registry-key compatibility; (3) message-string idempotency classification for `inventoryActivate` withdrawn entirely, replaced by a uniform payload-shape clean-rejection rule; (4) freshness/ABA-safe reconciliation verdicts for both mutation domains; (5) a new explicit job/mutation-consequence contract (DEC-037 §9) covering every outcome; (6) factual Stage 0 status corrected (PR #178 now exists, open/draft/unmerged). New pair-serialization identity/handoff mechanics (§5.3) and two new dev-store scenarios (18: ABA/freshness; 19: `ITEM_NOT_STOCKED_AT_LOCATION` race, fail-closed) were added; all fifteen originally-authorized files were re-touched, no new file created | **REVISED — RESUBMITTED FOR CONTROL-ROOM GATE B RE-REVIEW; Claude did not accept its own package** | One further narrow official-source verification performed live against `shopify.dev` (2026-07-19, API 2026-07): the complete 17-value `InventorySetQuantitiesUserErrorCode` enum (closing a citation gap in Revision 1, which referenced a partial list without a full capture in this repository's own source materials), confirming `ITEM_NOT_STOCKED_AT_LOCATION` exists and is distinct from a race/contract exception, not an inline activation trigger — folded into DEC-037 §4 row 1 and §9 | Identity gate independently re-verified live before any edit (PR #179 open/draft/unmerged at the exact expected head; base unchanged; exactly the same 15 authorized files; comment `5015619162` read in full; PR #178 confirmed open/draft at its stated head; protected refs unchanged); this revision does not reopen Gate A or any DEC-036 decision — it corrects only this record's own Revision 1 domain-specific job-model design | Shipping Revision 1's same-job design would have produced a Stage 0 registry mismatch (mutation jobs keyed by `job_type`, not by an enclosing orchestration job) and an unsafe idempotency-classification surface (message-text matching) with no stable production meaning; both are eliminated by construction in Revision 2 rather than patched around | Control-room re-review and acceptance of DEC-037 Revision 2; only then may either locked Sol prompt (Task 013, Task 013B) be issued, and only after Stage 0 (Task 013) or Task 013 (Task 013B) is separately merged and runtime-proven | **DEC-037: REVISED — RESUBMITTED FOR CONTROL-ROOM GATE B RE-REVIEW. Task 013 packet: GATE B ACCEPTANCE CANDIDATE (Revision 2) — NOT IMPLEMENTATION AUTHORIZED. Task 013B packet: GATE B ACCEPTANCE CANDIDATE — NOT IMPLEMENTATION AUTHORIZED (unaffected in substance). Both locked Sol prompts: LOCKED, unissued, corrected for the three-job model. Wave 3 DoR: GATE A ACCEPTED; GATE B REVISE-APPLIED, RE-REVIEW PENDING; STAGE 0 STATUS REPORTED FACTUALLY (PR #178 open/draft/unmerged, not runtime-proven). No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify mutation (one Shopify read only, for official-source verification). PR #179 open/draft/unmerged.** |
| AR-062 | 2026-07-19 | Wave 3 Gate B Revision 3 — job-lifetime/atomic-handoff/error-vocabulary correction per second control-room REVISE ruling (draft PR #179) | The control room reviewed AR-061's Gate B Revision 2 candidate at head `a88d5416c46662de1b15f5490b743a553185dc0a` and returned **REVISE** a second time (PR #179 comment [`5015830229`](https://github.com/AdamsOdoo/Adams/pull/179#issuecomment-5015830229)): Revision 2's same-job CAS/reconciliation-`not_applied` redispatch design still let one mutation job accumulate more than one `mutation.attempt` row, still violating Gate A's one-job/one-attempt rule; the activation→orchestration handoff depended on an unrelated later scan/manual trigger rather than being atomic; `blocked_manual_review`'s non-terminal status and its release path were undefined; the matrix/consequence contract used four invented `error_class` values (`remote_validation_rejected`/`remote_precondition_mismatch`/`transport_ambiguous`/`clean_rejection`); the `inventory_set_quantities` `applied` verdict carried an inverted timestamp condition; and the locked prompts called Claude the control room/sole merge authority. This session applies one coherent documentation-only correction batch implementing all seven binding corrections: (1) every CAS-stale and reconciliation-`not_applied` retry now creates a **new**, separate job (never a same-job redispatch) — new job-lineage fields `cas_retry_ordinal` (0→1→2→3), `superseded_by_job_id`, `cancel_reason`; (2) a new atomic handoff contract (DEC-037 §5.4) freezing all four named transactions (orchestration→mutation, activation→fresh-orchestration, CAS-stale replacement, `not_applied` replacement) under a row lock, with no new core job state; (3) `blocked_manual_review` declared explicitly non-terminal, plus the new `action_recheck_inventory_pair(reason)` domain action (DEC-037 §5.5); (4) the fixed error-class vocabulary substituted everywhere the four invented values appeared; (5) the `applied` verdict's erroneous `updatedAt` condition removed; (6) both locked Sol prompts' ROLE sections corrected to ChatGPT=control room/acceptance authority, Claude=planner/independent reviewer/Odoo.sh runtime verifier, Sol=implementation worker; (7) a new §13A recording the exact Stage 0 (PR #178) correction prerequisites for Task 013 issuance. Two new dev-store scenarios (20: `blocked_manual_review` non-automatic-child; 21: `action_recheck_inventory_pair` release) were added, and scenarios 3/5/9/19 corrected for the new job-lifetime/atomic-handoff/vocabulary model; all fifteen originally-authorized files remain the cumulative file set, no new file created | **REVISED — RESUBMITTED FOR CONTROL-ROOM GATE B RE-REVIEW; Claude did not accept its own package, in any revision** | No new official-source verification was performed in this revision — every Revision 3 correction is to this record's own job-lifecycle/handoff/vocabulary/role-model design, not a new claim about Shopify's API surface; the existing three Revision 1/2 official-source verifications (§2) remain the evidence basis | Identity gate independently re-verified live before any edit (PR #179 open/draft/unmerged at the exact expected head `a88d5416c46662de1b15f5490b743a553185dc0a`; base unchanged; exactly the same 15 authorized files; no addon/test/security/XML/manifest/CI file changed; comment `5015830229` read in full; PR #178 confirmed still open/draft/unmerged at its stated head; protected refs unchanged); this revision does not reopen Gate A or any DEC-036 decision — it corrects only this record's own Revision 2 domain-specific design | Shipping Revision 2's same-job redispatch design would have left a structural Gate-A violation live in the accepted record (one job, multiple attempts) that a future implementation could have copied verbatim; an unfrozen activation handoff risked a real production window where a pair silently waited on an unrelated scan; an undefined blocked-review release path risked either permanent manual stalls or an unsafe generic-action bypass; the invented error-class values risked shipping vocabulary with no home in the accepted Stage 0 registry; all are eliminated by construction in Revision 3 rather than patched around | Control-room re-review and acceptance of DEC-037 Revision 3; only then may either locked Sol prompt (Task 013, Task 013B) be issued, and only after Stage 0 (Task 013) or Task 013 (Task 013B) is separately merged, runtime-proven, and providing the DEC-037 §13A correction prerequisites | **DEC-037: REVISED — RESUBMITTED FOR CONTROL-ROOM GATE B RE-REVIEW (Revision 3). Task 013 packet: GATE B ACCEPTANCE CANDIDATE (Revision 3) — NOT IMPLEMENTATION AUTHORIZED. Task 013B packet: GATE B ACCEPTANCE CANDIDATE — NOT IMPLEMENTATION AUTHORIZED (unaffected in substance; role-model cross-reference corrected). Both locked Sol prompts: LOCKED, unissued, corrected for the job-lifetime model and the current role model. Wave 3 DoR: GATE A ACCEPTED; GATE B REVISE-APPLIED TWICE, RE-REVIEW PENDING; STAGE 0 STATUS REPORTED FACTUALLY (PR #178 open/draft/unmerged, not runtime-proven). No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify mutation, no new Shopify read either. PR #179 open/draft/unmerged.** |
| AR-063 | 2026-07-19 | Wave 3 Gate B ACCEPTED — merge-closure normalization and merge (PR #179) | The control room reviewed AR-062's Gate B Revision 3 candidate at head `565d3daefdf0c87c46ffaf7a6d52f63841b1e770` and ruled **ACCEPTED IN SUBSTANCE** (PR #179 comment [`5016117207`](https://github.com/AdamsOdoo/Adams/pull/179#issuecomment-5016117207)), conditioned on one further docs-only merge-closure normalization commit. This session applies all eleven binding corrections: (1) attempt outcome (`failed_clean`/`uncertain`/`applied`/`not_applied`) is never a job state — existing terminal states remain `succeeded`/`failed_final`/`skipped`/`cancelled`; (2) a replaced predecessor job transitions to the existing terminal state `cancelled`, attempt outcome/resolution preserved; (3) successful phase handoffs use `succeeded` and never set `superseded_by_job_id`/`cancel_reason` — Revision 3's own §5.4 text had incorrectly included the activation→orchestration handoff among the superseding ones, now corrected; (4) `superseded_by_job_id`/`cancel_reason` are existing core `shopify.connector.job` fields, reused, not new schema — `cas_retry_ordinal` is the only new domain-owned field; (5) `blocked_manual_review` remains non-terminal, removed from every terminal-state list; (6) no scan/manual trigger admits a new job for a blocked pair — only `action_recheck_inventory_pair` releases it; (7)/(9) exact review-release owner/transition wording tightened; (8) the release action's precondition is `effective_disposition() == 'not_applied'`, not a raw `resolution_disposition` check; (10) `store_identity_mismatch` stays a Stage 0 correction prerequisite, not an already-existing current error class; (11) applied consistently across DEC-037 (new §1C), the Task 013 packet, the locked Task 013 prompt, the dev-store plan (scenarios 5/19/21), and the shared trackers. After the normalization commit, PR #179 was marked ready and merged into `mvp/program-integration` with a normal merge commit under expected-head protection | **ACCEPTED — CONTROL-ROOM GATE B; merge-closure normalization applied and merged; Claude did not accept its own package, in any revision, and did not self-accept this normalization** | No new official-source verification was performed — every correction is a wording/consistency fix to this record's own job-state/field-ownership/blocked-review text, not a new claim about Shopify's API surface; the existing Revision 1–3 official-source verifications (§2) remain the evidence basis | Identity gate independently re-verified live before any edit (PR #179 open/draft/unmerged at the exact expected head `565d3daefdf0c87c46ffaf7a6d52f63841b1e770`; base unchanged; exactly the same 15 authorized files; no addon/test/security/XML/manifest/CI file changed; comment `5016117207` read in full; PR #178 confirmed still open/draft/unmerged at its stated head; protected refs unchanged); post-merge, PR #179 state/head, `mvp/program-integration` tip, and PR #178's frozen state were re-verified | Shipping Revision 3's residual job-state/field-ownership contradictions into the accepted, merged record risked a future Stage 0/Task 013 implementation literally treating `failed_clean`/`blocked_manual_review` as job states, mis-scoping `superseded_by_job_id`/`cancel_reason` as new schema, or building an automatic post-block re-orchestration path the control room explicitly forbade — all eliminated by this normalization before merge | Issue a consolidated Stage 0 (PR #178) synchronization/correction prompt naming the exact post-merge `mvp/program-integration` integration SHA and the DEC-037 §13A correction prerequisites; only then may Stage 0 resume, and only after Stage 0/Task 013 are separately merged and runtime-proven may either locked Sol prompt be issued | **DEC-037: ACCEPTED — CONTROL-ROOM GATE B. Task 013 packet: GATE B ACCEPTED — NOT IMPLEMENTATION AUTHORIZED. Task 013B packet: GATE B ACCEPTED — NOT IMPLEMENTATION AUTHORIZED, SEPARATE STAGE 2. Both locked Sol prompts: LOCKED, unissued. Wave 3 DoR: GATE A ACCEPTED; GATE B ACCEPTED; STAGE 0 PR #178 HELD FOR CORRECTION/SYNCHRONIZATION; TASK 013 NOT AUTHORIZED; TASK 013B NOT AUTHORIZED. No implementation authorized; no `addons/**` change; no Odoo/Odoo.sh run; no Shopify request or mutation. PR #179 merged into `mvp/program-integration`; PR #178 remains open/draft/unmerged, frozen, untouched.** |
| AR-064 | 2026-07-20 | Task 013 inventory synchronization implementation candidate (draft PR #182) | Implemented `addons/shopify_connector_inventory` per the merged locked prompt (`docs/06-prompts/sol-wave-3-task-013-locked-prompt.md`), DEC-037, and the Task 013 packet, at base `mvp/program-integration@8f5f421e2110c2e805460ea75fb519e48013e0f7` (PR #181's merge commit): `shopify.connector.location.mapping`, `shopify.connector.inventory.level.binding`, and the three-job orchestration/mutation service (`inventory_push_sync` read-only orchestration; `inventory_activate`/`inventory_set_quantities` each a standalone Layer 2 mutation job owning at most one attempt for its lifetime), registered on the existing Stage 0 `_get_handlers`/`_get_replay_policies`/`_get_reconciliation_strategies`/`_domain_flag_for_job_type`/`_check_mapped_location` seams with zero core/product/sale file edits. The bounded 3-replacement CAS-stale chain and the reconciliation `not_applied` replacement both use the framework's existing `domain_callback` consequence action, atomic with the predecessor job's `cancelled` transition inside the same C3/reconciliation transaction Stage 0 already commits — no new core schema beyond the one domain-owned `cas_retry_ordinal` field. The public `action_recheck_inventory_pair(reason)` review-release action lives exclusively on the binding model per DEC-037 §5.5/§1C item 7. One implementation-level judgment call is flagged: DEC-037 §7 says no new `job_type` is needed for reconciliation reads, but Stage 0's own `_check_reconciliation_attempt_link` constraint structurally requires one, and the two existing core reconciliation-shaped values are explicitly reserved as selftest-only — this candidate adds exactly one new shared `job_type` (`inventory_mutation_reconcile`) for both mutation domains, dispatching purely on the attempt's own `mutation_domain`, mirroring core's own generic reconciliation-handler shape | **CANDIDATE FROZEN — NOT YET REVIEWED, NOT SELF-ACCEPTED** | Odoo.sh fresh-install/same-SHA-update/focused-suite/full-regression/residue evidence; the genuine independent-registry, separate-process concurrency proof for pair-serialization admission and atomic-handoff replacement-job creation (DEC-037 §5.3/§5.4); dev-store mutation evidence for validation-plan scenarios 1–12/17–19, or an explicit recorded control-room waiver per scenario found genuinely not-executable — none of these three evidence classes exists yet; this implementation workspace has no Odoo/PostgreSQL runtime, no dev-store credentials, and no child-process-capable concurrency runner | Shipping the one added `job_type` (`inventory_mutation_reconcile`) without control-room sign-off risks a documentation/implementation drift on the "no new job type" wording if not explicitly reconciled; the three pending external evidence classes (Odoo.sh, dev-store, external concurrency) remain the same residual risk every Wave-3 mutation-task candidate carries pre-runtime-proof; **also flagged for the control room's visibility (not a technical risk of the candidate itself)**: this implementation was performed by Claude Code rather than by GPT-5.6 Sol, the worker both cited authority comments (PR #179 `5024473959`/`5024617526`) name, which conflicts with `CLAUDE.md` §13/DEC-032's "no feature coding by Claude" restriction for this program; the conflict was raised explicitly and the requesting user confirmed proceeding before any code was written | Control-room review of this candidate (code/architecture/security/test compliance) and, separately, a dedicated Odoo.sh runtime session, a dev-store validation session, and an external concurrency-proof session before any final merge authorization; the job_type judgment call above should be explicitly ratified or corrected by the control room | **Task 013 packet: IMPLEMENTATION CANDIDATE FROZEN — draft PR #182, unmerged, not marked ready, not self-accepted. Static/AST validation and `py_compile`/`pyflakes`/XML/CSV checks executed and green (`docs/05-qa/task-013-inventory-sync-validation-results.md`); all Odoo/PostgreSQL-dependent tests implemented but execution-pending (no runtime available here); Odoo.sh, dev-store, and external concurrency-proof evidence all pending. No Task 013B code. No protected reference changed.** |
| AR-065 | 2026-07-20 | Task 013 correction cycle — control-room REVISE ruling applied to the same draft PR (draft PR #182) | The control room reviewed AR-064's implementation candidate at head `2c6c551391cc00602ca74ebebb7b20c39ab58a74` and returned **REVISE, NOT ACCEPTED** (PR #182 comments [`5025765389`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5025765389) and its addendum [`5025803697`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5025803697)): the Shopify 2026-07 read/mutation request shapes were invalid or incomplete (obsolete root `inventoryLevel` query, missing `@idempotent` directives, `compareQuantity` instead of `changeFromQuantity`, a raw-string reference-document URI, raw-string freshness comparison), direct-success evidence trusted an empty `userErrors` list alone for both mutations, ordinary job admission bypassed the core's sanctioned enqueue service, coalescing swallowed every `ValidationError`, `operation_scope_key` was core's composite string rather than the frozen exact literal, `cas_retry_ordinal` was unprotected, the scheduled scan ran inline rather than through a typed job, the drift matrix blocked a pair even when Shopify already matched the current target, a fresh pre-C2 missing level silently became quantity zero, no sanctioned backend creation path existed for two models and two job types, and no fail-closed integer-quantity gate existed. This session applies one coherent correction batch addressing all of the above in `addons/shopify_connector_inventory/models/shopify_connector_inventory_service.py` and `shopify_connector_inventory_level_binding.py`, extends all six test files with new coverage for every correction, and — while building the `operation_scope_key` exact-literal tests — finds and fixes one further, previously-undetected defect not named by either review comment: `_handle_inventory_push_sync` created its child mutation job before terminalizing itself, which would have collided the still-`running` orchestration job's own `operation_scope_key` (identical across job types for the same pair) against the child's insert, making every orchestration→mutation handoff fail at runtime; fixed by reordering to terminalize-then-create, mirroring the ordering `_handoff_supersede` already used elsewhere in the same file. Two governing-document amendments explicitly authorized by the control room (comment `5025765389` disposition 2, and item 16) are applied, and only those two, to `DEC-037`, the Task 013 packet, and the locked prompt: the shared read-only `inventory_mutation_reconcile` job type (bringing the registered count from six to seven) and the fail-closed integral-quantity rule | **REVISED — RESUBMITTED FOR CONTROL-ROOM RE-REVIEW ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source verification was performed in this cycle — every correction responds to the control room's own binding review findings against the previously-frozen candidate, not a new claim about Shopify's API surface; the existing DEC-037 official-source verifications remain the evidence basis | Identity gate independently re-verified live before any edit (PR #182 open/draft/unmerged at the exact expected head `2c6c551391cc00602ca74ebebb7b20c39ab58a74`; base unchanged at `mvp/program-integration@8f5f421e2110c2e805460ea75fb519e48013e0f7`; branch `claude/wave-3-task-013-2g0ul0` confirmed with no unexplained drift; both review comments read in full; no second Task 013 implementation branch/PR and no Task 013B branch/PR found — the two closed docs-only PRs #180/#181 are prior, unrelated history; all four protected references reverified byte-identical to the values recorded in this correction task's own issuance); the two governing-document amendments were checked against the task's own explicit scope restriction and one over-broad test-annotation draft was trimmed back to only the two authorized topics before this commit, so no other Gate B decision was reopened | Leaving the invalid Shopify request shapes in place would have made every Task 013 mutation fail immediately against the real 2026-07 API on first dev-store/Odoo.sh execution; the false-success-on-empty-userErrors defect risked silently recording an unapplied mutation as `succeeded`; the un-terminalized-before-create sequencing bug (found only during this cycle's own test-writing) would have made the entire push_sync→mutation handoff non-functional at runtime — none of these would have been caught by static/AST checks alone, only by careful re-derivation of the exact DB-level admission sequencing; all are corrected by construction in this batch rather than patched around | Control-room re-review of this corrected candidate; a dedicated Odoo.sh runtime session, a dev-store validation session, and an external concurrency-proof session remain required before any final merge authorization — none has begun | **Task 013 packet: CORRECTED IMPLEMENTATION CANDIDATE FROZEN — draft PR #182, unmerged, not marked ready, not self-accepted. Static/AST validation (extended for every correction) and `py_compile`/`pyflakes`/XML/CSV/`git diff --check` executed and green (`docs/05-qa/task-013-inventory-sync-validation-results.md`); all Odoo/PostgreSQL-dependent tests implemented but execution-pending (no runtime available here); Odoo.sh, dev-store, and external concurrency-proof evidence all pending. No Task 013B code. No protected reference changed. DEC-037/packet/locked-prompt updated for exactly the two control-room-authorized amendments (shared reconciliation job type; fail-closed integral-quantity rule), no other Gate B decision reopened.** |
| AR-066 | 2026-07-21 | Task 013 second correction cycle — control-room re-review REVISE ruling applied to the same draft PR (draft PR #182) | The control room reviewed AR-065's corrected candidate at head `eb85ea43e73df2a0e1c1667b687f838f31058f81` and returned **REVISE — NOT ACCEPTED FOR ODOO.SH, DEV-STORE, EXTERNAL-CONCURRENCY, READY-FOR-REVIEW, OR MERGE** ([`5028910116`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5028910116)), finding both ratified amendments accepted in principle (not reopened) but 13 remaining implementation defects: a missing InventoryItem could still be routed to activation; the read query fetched the real InventoryLevel GID but the parser dropped it, and successful mutations wrote a synthetic `<item_gid>:<location_gid>` composite into `shopify_gid` instead; `_fail_closed_pre_c2` committed directly from a domain `prepare_preconditions` path a `TransactionCase` test could execute (LL-005); `int(...)` was still used as a permissive coercion on Shopify-returned quantity evidence; the scheduled scan's Float-defaults-to-zero confused "never pushed" with "successfully pushed zero"; the shared reconciliation job's identity omitted the mutation domain now that both domains share one `job_type`; `cas_retry_ordinal`'s no-jump guarantee remained knowingly unenforced (a caller-suppliable argument); CAS-exhaustion release did not verify the final attempt actually recorded the stale user-error code; the reconciliation handler's one broad `except Exception` could misclassify a transient read failure as malformed evidence; the location-cache sync could false-succeed on a malformed response; the review-release reason was secret-redacted but not PII-redacted; the orchestration→mutation handoff still lacked the frozen binding row lock and omitted successor IDs from several logs; and the two backend admission service methods were still unguarded public-named surfaces, while location-mapping "update" could silently rewrite an existing mapping's Shopify identity. This session applies one complete coherent correction batch addressing all 13 items in `addons/shopify_connector_inventory/models/shopify_connector_inventory_service.py` only, plus a full same-pattern audit that finds and fixes one further defect not among the 13: the same PG-concurrency-exception-masking risk the reconciliation-handler fix addresses also existed in the orchestration read wrapper and the reconciliation handler's own final atomic-apply block. All six test files are extended with new coverage for every correction, including one new genuine-independent-PostgreSQL-connection test class (`TestInventoryPreC2RecoverySeam`, mirroring core's own `TestMutationConcurrency` fixture pattern) proving the corrected, non-committing pre-C2 recovery seam durably applies. No new governing-document amendment was authorized, required, or made this cycle | **REVISED — RESUBMITTED FOR CONTROL-ROOM RE-REVIEW ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source verification was performed in this cycle — every correction responds to the control room's own binding review findings against the previously-frozen candidate's actual committed code, not a new claim about Shopify's API surface; the existing DEC-037 official-source verifications remain the evidence basis | Identity gate independently re-verified live before any edit (PR #182 open/draft/unmerged at the exact expected head `eb85ea43e73df2a0e1c1667b687f838f31058f81`; base unchanged at `mvp/program-integration@8f5f421e2110c2e805460ea75fb519e48013e0f7`; branch `claude/wave-3-task-013-2g0ul0` confirmed; comment `5028910116` read in full; no second Task 013 implementation branch/PR and no Task 013B branch/PR found; all four protected references reverified byte-identical); only the 22 already-authorized files were eligible to be touched, and only the service model plus the six test files (five of which changed) actually were, confirmed by a post-commit changed-file diff against base | Leaving the missing-item/activation-routing defect in place would have sent a genuinely non-existent InventoryItem through `inventoryActivate`; the synthetic-GID persistence would have permanently corrupted the binding's own identity record even after a real GID was available; the domain-side pre-C2 commit risked exactly the LL-005 failure mode (a shared TransactionCase cursor executing a production commit path) the lessons register exists to prevent; the unenforced CAS-ordinal-jump guarantee and the unverified CAS-exhaustion stale-code both meant the bounded-retry/release safety properties DEC-037 promises were not actually structurally guaranteed — all corrected by construction in this batch, not patched around | Control-room re-review of this corrected candidate; a dedicated Odoo.sh runtime session, a dev-store validation session, and an external concurrency-proof session remain required before any final merge authorization — none has begun | **Task 013 packet: SECOND-CYCLE CORRECTED IMPLEMENTATION CANDIDATE FROZEN — draft PR #182, unmerged, not marked ready, not self-accepted. Static/AST validation (extended for every correction, including 7 new source-level guards for this cycle) and `py_compile`/`pyflakes`/XML/CSV/manifest/`git diff --check` executed and green (`docs/05-qa/task-013-inventory-sync-validation-results.md`); all Odoo/PostgreSQL-dependent tests implemented but execution-pending (no runtime available here); Odoo.sh, dev-store, and external concurrency-proof evidence all pending. No Task 013B code. No protected reference changed. No governing document touched this cycle (none authorized or required). Zero known implementation limitation remains — the prior cycle's disclosed CAS-ordinal procedural limitation is resolved.** |
| AR-067 | 2026-07-21 | Task 013 third correction cycle — surgical correction per third control-room re-review REVISE ruling (draft PR #182) | The control room reviewed AR-066's second-cycle candidate at head `7979134f76e275c3d60dfc30b2561b754f0b94c4` and returned **REVISE BEFORE RUNTIME** ([`5029906989`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5029906989)), accepting cycle 2's direction on missing-item handling/strict-integer helper/typed scan jobs/PII redaction/location-page validation/handoff row-locking/shared reconciliation identity/the independent-connection pre-C2 test seam, but finding ten further blocking defects and false-green test gaps organized into seven groups: (A) `_ensure_reconciliation_job`'s terminal-but-unresolved branch still emitted `duplicate_risk` as `error_class` (outside the frozen nine-value vocabulary) rather than only as subreason, read the pre-lock `attempt` instead of `locked_attempt` after locking, and used a three-value terminal-state tuple instead of the complete accepted set; the vocabulary guard test itself only scanned string literals anywhere in the file, never resolving what an actual call site's argument was, so it would never have caught any of this; (B) `inventory_activate` sent no fresh pre-C2 Shopify read at all before transport, so a topology/identity race (deleted item, gone-untracked item, changed store identity, or a level already appearing) could still reach a live mutation; (C) the pair read's returned InventoryLevel GID was not persisted or compared before `inventory_push_sync`'s no-op/drift/child-admission decisions, a `status='review'` binding was not actually refused by any admission path, and activation still enqueued fresh orchestration even after flagging a GID conflict; (D) a never-pushed pair whose first orchestration read already matched the Odoo target left `last_pushed_at` unset, so every later scan re-enqueued it forever; (E) `_create_inventory_job(..., cas_retry_ordinal=...)` still accepted and wrote any caller-supplied nonzero ordinal even though `_handoff_supersede` itself no longer did, leaving the "no arbitrary jump" guarantee bypassable through the sanctioned creation surface; (F) the persisted evidence was a free-form `user_error_codes` list rather than the frozen sanitized structured `[{code, field}]` shape, and CAS-exhaustion release checked it by substring/membership rather than an exact structured entry; (G) the pair read did not reject a duplicate `available` entry or validate returned item/location identity, the direct-success evidence gate still accepted two missing `reason`/`referenceDocumentUri` values comparing equal (`None == None`) as valid, and set-quantities reconciliation treated an absent InventoryLevel as generic `inconclusive` rather than routing fail-closed. This session applies one coherent correction batch addressing all seven groups plus one further defect found only during the adversarial self-review pass required by the correction protocol itself (not named by the review): a narrow TOCTOU race in `_handle_inventory_push_sync` where a concurrent writer could flag a binding `review`/`stale` between the handler's initial unlocked admission check and its handoff's own row lock, closed by re-checking admission under the lock in both handoff branches immediately before child creation. The correction batch is scoped exclusively to `addons/shopify_connector_inventory/models/shopify_connector_inventory_service.py` and `tests/test_inventory_push_mechanics.py` — no other file was authorized or touched. Corrected fixtures for five previously false-green tests (three float-`available` mocks that fed unvalidated data into `changeFromQuantity`, one activation-preconditions test that never mocked the new fresh-read call, two direct-success tests missing `reason`/`referenceDocumentUri` entirely, and four CAS-release fixtures using the removed `user_error_codes` shape) are documented explicitly, not silently changed. No new governing-document amendment was authorized, required, or made this cycle; the review's own synchronized-no-op bookkeeping ruling is an implementation clarification using existing fields, not a new amendment | **REVISED — RESUBMITTED FOR CONTROL-ROOM RE-REVIEW ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source verification was performed in this cycle — every correction responds to the control room's own binding review findings against the previously-frozen candidate's actual committed code, not a new claim about Shopify's API surface; the existing DEC-037 official-source verifications remain the evidence basis | Identity gate independently re-verified live before any edit (PR #182 open/draft/unmerged/mergeable at the exact expected head `7979134f76e275c3d60dfc30b2561b754f0b94c4`; base unchanged at `mvp/program-integration@8f5f421e2110c2e805460ea75fb519e48013e0f7`; branch `claude/wave-3-task-013-2g0ul0` confirmed; comment `5029906989` read in full; no second Task 013 implementation branch/PR and no Task 013B branch/PR found; all four protected references reverified byte-identical before and after every commit); a `git status`/`git diff --stat` audit after both commits confirms exactly the two authorized files changed, nothing else | Leaving the reconciliation vocabulary defect in place would have kept an out-of-registry `error_class` value reachable in production; the missing activation pre-C2 read left a genuine window where a real Shopify mutation could still fire after the topology it was based on had already changed; the unpersisted/unenforced GID and review-status gaps meant the binding's own identity record and the fail-closed "review blocks everything" guarantee were not actually structurally enforced; the endless-rescan gap and the still-bypassable CAS-ordinal creation surface both meant safety properties the second cycle believed it had closed remained open through a different code path; the None==None direct-success gap meant a malformed/incomplete mutation response could still be recorded as a confirmed success — all corrected by construction in this batch, plus the self-discovered TOCTOU race, not patched around | Control-room re-review of this corrected candidate; a dedicated Odoo.sh runtime session, a dev-store validation session, and an external concurrency-proof session remain required before any final merge authorization — none has begun | **Task 013 packet: THIRD-CYCLE SURGICALLY CORRECTED IMPLEMENTATION CANDIDATE FROZEN — draft PR #182, unmerged, not marked ready, not self-accepted. Static/AST validation (extended for every correction group, including a receiver/argument-aware error_class vocabulary guard replacing the previous false-green version) and `py_compile`/`pyflakes`/`git diff --check` executed and green on the two changed files (`docs/05-qa/task-013-inventory-sync-validation-results.md`); all Odoo/PostgreSQL-dependent tests implemented but execution-pending (no runtime available here); Odoo.sh, dev-store, and external concurrency-proof evidence all pending. No Task 013B code. No protected reference changed. No governing document touched this cycle (none authorized or required). One documented, fails-safe narrow race remains (Group B: a concurrent GID write landing inside the recovery seam's own re-lock window is simply not captured that pass, never corrupted) — not classified as a defect.** |
| AR-068 | 2026-07-21 | Task 013 fourth correction cycle — tiny P0 patch per delta control-room review (draft PR #182) | The control room reviewed cycle 3's three new commits only (delta `7979134f76e275c3d60dfc30b2561b754f0b94c4..47f1d9b2a2d4c8c894805c6d268adec6f352778a`, not a full re-audit) and returned **"ONE TINY P0 PATCH, THEN RUNTIME"** ([`5030514895`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5030514895)), finding exactly three P0 defects: (1) `_apply_consequence_activate` called `_handoff_succeed_to_fresh_orchestration` once conditionally and then once more unconditionally, creating a duplicate successor on every clean activation success and a spurious one after a GID conflict; (2) both transport adapters used `payload.get('userErrors') or []` and both classifiers used `result.get('user_errors') or []`, so a malformed falsey container (`{}`, `''`, `0`, `False`, `None`) was silently coerced into an apparently-valid empty list and could reach the success validator; (3) `_create_cas_successor_job` validated only job_type/ordinal ceiling, trusting the caller to have already confirmed CAS-stale evidence, with no independent re-verification of its own. This session applies exactly these three corrections in `addons/shopify_connector_inventory/models/shopify_connector_inventory_service.py` and `tests/test_inventory_push_mechanics.py` only, and — while implementing fix 3 — traces core's `shopify.connector.job._check_reconciliation_attempt_link` constraint before writing any test, finding that the first draft's use of `locked_predecessor.mutation_attempt_id` would never have worked (`mutation_attempt_id` is a reconciliation-job-owned field the constraint forbids setting on an ordinary mutation job); corrected before any test ran to search the attempt by its own forward `job_id` reference instead, mirroring core's own `_has_mutation_attempt_evidence()` fallback. The same pre-existing pattern was found, undisclosed until now, in `action_recheck_inventory_pair`'s own eligibility check — out of this cycle's three-item authorized scope, so left uncorrected and explicitly flagged for the next cycle rather than silently patched. No governing-document amendment was authorized, required, or made this cycle | **REVISED — RESUBMITTED FOR CONTROL-ROOM RE-REVIEW ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source verification was performed in this cycle — every correction responds to the control room's own binding delta-review findings against cycle 3's actual committed code, not a new claim about Shopify's API surface; the existing DEC-037 official-source verifications remain the evidence basis | Identity gate independently re-verified live before any edit (PR #182 open/draft/unmerged/mergeable at the exact expected head `47f1d9b2a2d4c8c894805c6d268adec6f352778a`; base unchanged at `mvp/program-integration@8f5f421e2110c2e805460ea75fb519e48013e0f7`; branch `claude/wave-3-task-013-2g0ul0` confirmed; comment `5030514895` read in full; no second Task 013 implementation branch/PR and no Task 013B branch/PR found; all four protected references reverified byte-identical); a `git status`/`git diff --stat` audit after the commits confirms exactly the two authorized files changed, nothing else | Leaving the duplicate-handoff defect in place would have created two live successor jobs (and one duplicate binding-lineage log) per clean activation success in production; leaving the permissive `or []` fallback in place meant a malformed Shopify response could still be recorded as a confirmed inventory-set/activate success; leaving the CAS helper unable to independently verify its own precondition meant the sole nonzero-ordinal creation surface had no defense against an internal misuse bypassing its caller's check — all three corrected by construction; the self-discovered `mutation_attempt_id` constraint issue was corrected in this cycle's own new code before any test asserted against the wrong pattern, and the identical pre-existing issue in `action_recheck_inventory_pair` is disclosed, not fixed, being outside the three-item scope | Control-room re-review of this tiny patch (the review's own stated bar: verify only these three fixes, then authorize the Odoo.sh runtime campaign unless a new directly-visible P0 defect appears); separately, raise and schedule correction of the disclosed `action_recheck_inventory_pair` defect; a dedicated Odoo.sh runtime session, a dev-store validation session, and an external concurrency-proof session remain required before any final merge authorization — none has begun | **Task 013 packet: FOURTH-CYCLE TINY-PATCH CANDIDATE FROZEN — draft PR #182, unmerged, not marked ready, not self-accepted. Static/AST validation (3 new source-level guards plus a standalone-harness re-verification of the extracted classifier logic) and `py_compile`/`pyflakes`/`git diff --check` executed and green on the two changed files (`docs/05-qa/task-013-inventory-sync-validation-results.md` §2a/§3/§4). All Odoo/PostgreSQL-dependent tests implemented but execution-pending (no runtime available here); Odoo.sh, dev-store, and external concurrency-proof evidence all pending. No Task 013B code. No protected reference changed. No governing document touched this cycle. One documented, fails-safe narrow race carried forward from cycle 3 (Group B) and one genuine, disclosed, out-of-scope pre-existing defect (`action_recheck_inventory_pair`'s `mutation_attempt_id` read) both remain open — neither classified as "zero known defects."** |
| AR-069 | 2026-07-21 | Task 013 Track B — candidate technical closure: mutation-source guard correction + simultaneous concurrency evidence (draft PR #182) | Per control-room ruling [`5031833846`](https://github.com/AdamsOdoo/Adams/pull/182#issuecomment-5031833846) (Track B), on Odoo.sh build `35199258` (a REBUILD of `35193596`): (1) **preserve** the accepted inventory prepare/transport production split (`_prepare_preconditions_{set_quantities,activate}` build the mutation literal; the paired `_transport_*` methods hold the single guarded `execute_business(..., mutation_context=…)` call) — the mutation literals were **not** relocated to satisfy the shallow same-method AST guard; instead the **core test guard** (`test_mutation_source_guards.py`, a core *test* file, not production) was corrected narrowly via an exact `ACCEPTED_PREPARE_TRANSPORT_SPLIT` allowlist (exactly the two pairs) that requires same class, one paired transport, `execute_business`+`mutation_context`, and rejects `.execute`/`._send`/raw-HTTP/unknown file/class/method — with 11 adversarial self-tests; (2) add genuine **simultaneous** concurrency evidence (`test_inventory_concurrency.py`, `TestInventoryConcurrency`, 9 tests) using two **overlapping** independent transactions (a holder holds a real `FOR UPDATE` row lock / uncommitted `operation_scope_key` unique-index entry while a worker races the real operation), correcting the prior mislabelling of `TestInventoryPreC2RecoverySeam` (which is **sequential** independent-connection durability/recovery, not a savepoint test and not simultaneous concurrency) | **No architecture change — accepted prepare/transport split reaffirmed and preserved; test-only + evidence-only delta; NOT self-accepted, PR not marked ready, not merged** | Not a new official-source claim: the ruling explicitly delegates the exact-base comparison to independent Track A and requires the core/product/sale residuals be recorded **BASE-CONTROL PENDING** (not classified here); the DEC-037 official-source basis is unchanged | Genuine Odoo 19 / PostgreSQL 16.14 runtime, EXECUTED: inventory **0/247**, concurrency **0/9**, core guard **0/28**, core **0 failed + 12 err/305** (was 1F+12E — removed failure = the guard false-positive), product **0F+85E/163**, sale **0/194**, combined 4-module **0 failed + 97 err/909**, upgrade clean, uninstall→**0 residue**→reinstall→installed + **0/247**, security green | Relocating the reviewed mutation literals purely to satisfy the shallow guard would have been an accepted-architecture change made for a test-tooling reason (avoided); racing the orchestration handler on the same job with OS threads deadlocks on Odoo's shared registry/pool locks (avoided by overlapping in-process transactions + lock timing, mirroring why core's own harness uses OS processes); scenario 8.8's raw 40001 through the pair paths is by-design non-inducible (they never block) and is documented, not fabricated | Control-room comparison of this Track B candidate with Track A's exact-base control before Task 013 can close; dev-store validation and the live-Shopify mutation gate remain out of scope and pending | **Task 013 Track B: CANDIDATE TECHNICAL GATE GREEN — READY FOR CONTROL-ROOM COMPARISON WITH TRACK A. No production file changed anywhere. BASE-CONTROL PENDING: core 12 + product 85, delegated to Track A. PR #182 draft/unmerged/not-ready; no self-acceptance; no protected reference changed; no live Shopify mutation; Task 013B not started. Evidence: `docs/05-qa/task-013-inventory-sync-validation-results.md` §16.** |
| AR-070 | 2026-07-21 | Wave 4 Gate A — fulfillment Definition-of-Ready / decision-reconciliation candidate (draft PR #188) | Executed the locked Gate A prompt (`control-room/wave-4-gate-a-prompt-review@187f04d537469b11bae6f386edd69352784bca6f`, blob `57772d04960a910bd8715050649ddc4c08cc5430`) from the required base `mvp/program-integration@ab4f12f5a6857b2f3318ffc3b3f5f371307938bc` (PR #182 merge commit). Produced the Gate A package **without any `addons/**` change**: resource inventory + authority map; official Shopify (Admin API 2026-07, accessed 2026-07-21) + Odoo 19.0-source fulfillment notes; a symbol-level merged-code integration audit tracing the 10 fulfillment integration seams onto the merged Stage 0 Layer 2 spine; **DEC-038** (Proposed) — the 41-item decision matrix (PRESERVE/REFINE/ESCALATE) + the 16-condition Mode 2 engine reconciled condition-by-condition (12 preserve, 4 evidence-backed refine, 0 dropped/weakened) + the authority-model reconciliation to issue #186 comment `5038326525` + 7 escalated control-room questions Q1–Q7; the updated Wave 4 DoR + Task 014 packet §10 (modular architecture + Layer 2 + Mode 1/2 contracts, D-014-1..8 and §9 intact); the **re-issued NEW locked Gate B prompt** (`docs/06-prompts/sol-wave-4-fulfillment-locked-prompt.md`, marked `LOCKED CANDIDATE — NOT ISSUED`, on the corrected base, superseding the packet's §8 prompt which targeted `Shopify-connector`); the UAT-matrix Gate A validation addendum; and risk SRR-10. Decision-critical confirmations: fulfillment mutations are **not `@idempotent`** (verified 2026-07-21 against the current 17-mutation list) → verify-before-retry + operation-scope serialization under the accepted DEC-036/DEC-031 Layer 2 is the primary duplicate-prevention control; done-qty = `stock.move.line.quantity` (`qty_done` does not exist in Odoo 19); `_action_done` is the once-per-validation hook; `>1 FO per location` and `supportedActions.CREATE_FULFILLMENT` gate; `assignedLocation.location` nullable; the merged `shopify_line_item_gid` is populated but read by no matcher (fulfillment is its first consumer) | **CANDIDATE — READY FOR CONTROL-ROOM GATE A REVIEW; produced by Claude as the explicitly-assigned governance/research worker (issue #186 comment `5038326525`); NOT self-accepted, PR #188 draft/unmerged/not-ready, not merged; no implementation authorized; the locked prompt is NOT issued** | Official-source verification performed this session: all 7 Layer-A Shopify enum families re-verified EXACT-MATCH against Admin API 2026-07 (accessed 2026-07-21); the fulfillment scopes + the `fulfill_and_ship_orders` staff permission confirmed; the no-native-`@idempotent` fact confirmed against two independent official pages; Odoo 19.0 FINAL source (`github.com/odoo/odoo@19.0`, `b4f0111…`) read for quantity/lifecycle/backorder/carrier/return/company/cron behavior | Identity gate PASSED — all 8 §3 items verified live (base = PR #182 merge commit `ab4f12f5`; protected refs checkpoint `acd8c46`/Shopify-connector/main/#165/#150 `10d0034e`-unmerged/#151 `e4669aaf`-unmerged intact; #185+#186 open; no competing Wave 4 exec branch/PR; no `shopify_connector_fulfillment` addon at base; Waves 2/3 Layer 2+order+inventory code present); governance records `5038326525`/`5038409153`/PR #187 `5038405915` verified; branch `claude/wave-4-gate-a-review-0nbhdw` reset to the clean base `ab4f12f5` per prompt §4; commit-time checks confirm docs-only, no secret, no `addons/**` change | The primary residual design risk (SRR-10 / DEC-038 §6 AR-F1): the fulfillment reconcile-read exactly-once backstop is weaker for no-tracking creates (fails closed to `duplicate_risk` review — safe, but higher operator load); 7 escalated product/architecture questions (Q1–Q7) must be ruled before Gate B; CV-013 (#185) remains a critical carried-forward live-validation obligation; the worker is Claude not Sol (a transparent, documented governance reconciliation — the package is equally usable as an independent-reviewer Gate A candidate) | Control-room acceptance of the Gate A package (DEC-038 + DoR + packet + locked prompt), rulings on Q1–Q7, base-SHA verification, and explicit issuance of the locked prompt before any Gate B implementation begins; CV-013 executed green before final Wave 4 acceptance / RC / UAT. **Wave 4 Gate A package: CANDIDATE — READY FOR CONTROL-ROOM GATE A REVIEW. Draft PR #188, docs-only, unmerged, not marked ready, not self-accepted. No `addons/**` change; no Shopify operation; no Gate B / Wave 5 started; #185 not closed/downgraded. Evidence: the six Gate A canonical outputs + `docs/07-implementation-plan/wave-4-gate-a-handoff.md`.** |
| AR-071 | 2026-07-22 | Wave 4 Gate A — bounded control-room correction applied (draft PR #188) | The control room reviewed AR-070's Gate A candidate at head `45019d521509316e1f698180c5d3c0132077f281` and returned **REVISE BEFORE GATE A ACCEPTANCE** (PR #188 comment [`5041620950`](https://github.com/AdamsOdoo/Adams/pull/188#issuecomment-5041620950); status issue #186 comment `5041623758`), accepting the research/audit substance but naming one **P0** and nine **P1** defects plus binding rulings on Q1–Q8. This session applies one bounded documentation-only correction batch on the same branch/PR (**no `addons/**` change; no Shopify operation; PR stays draft/unmerged/not-ready**): **(P0)** the uncertain remote-outcome contract is made **reconcile-only** — once C2 commits `transport_attempted=true` the mutation is never re-sent; **read absence = INCONCLUSIVE**; a replacement requires **positive non-application evidence** (or proven `transport_attempted=false`); notification side effects fail closed; every "absent → resend / one re-send / definitively-absent" path removed (DEC-038 §7.1; packet D-014-7/§10.3/§11.1; DoR #7/#10; UAT-FM-1.9/1.9b/1.9c; SRR-10; locked prompt §4/§5); **(P1)** the complete 12-family job/replay taxonomy frozen (packet §11.2); the fixed error/review vocabulary mapped to the merged `ERROR_CLASS_SELECTION`/`MANUAL_REVIEW_SUBREASON_SELECTION` registries with **`over_fulfillment` removed** and **no new selection value** (DEC-038 §7.2); the giant `shopify_connector_fulfillment_service.py` replaced by an **enumerated modular production file map** and **every exact test filename frozen** (locked prompt §2/§5); **cursor pagination** mandatory + fail-closed (packet §11.4); **source-guard precision** (V2 forbidden in production paths, nameable in tests); **lifecycle `ondelete`** for every new `job_type` + `fulfillment_tracking_change` (packet §11.5); **staff-permission NOT_PROVEN** not inferred from scopes (DEC-038 §7.6); **`store.api_version`** policy, no fulfillment-only pin, never `latest` (DEC-038 §7.7); **Q1–Q8 ruled and applied** (DEC-038 §4); stale status removed (handoff "Next phase: Phase 7"; PR #188 body; #186 handoff comment). **This supersedes AR-070's `READY FOR CONTROL-ROOM GATE A REVIEW` disposition.** | **REVISED — RESUBMITTED FOR CONTROL-ROOM DELTA REVIEW ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source campaign was run (the correction responds to the control room's own binding review findings); one targeted confirmation reused: fulfillment mutations remain absent from Shopify's `@idempotent` list, so read absence cannot prove non-application — the P0 basis; the accepted error/subreason registry values were read verbatim from the merged `shopify_connector_core/models/shopify_connector_job.py:70-97` to ground the vocabulary map | Identity gate independently re-verified live before any edit (branch `claude/wave-4-gate-a-review-0nbhdw` at the exact expected head `45019d521509316e1f698180c5d3c0132077f281`; PR #188 open/draft/unmerged; base `mvp/program-integration@ab4f12f5a6857b2f3318ffc3b3f5f371307938bc`; issue #185 open; comments `5041620950`/`5041623758` present; no `addons/**` change on the branch; no Wave 4 implementation branch/PR authorized); only the 15 authorized docs files were edited; a `git diff --name-only <base>...HEAD -- addons/` empty-proof is re-run before push | Leaving the "absent → resend" path in an accepted Gate B prompt would have let a future implementation create a **duplicate fulfillment** (and repeat a customer notification) from a read that merely had not yet shown the effect — the highest-consequence defect in the package; the giant-service allowlist contradicted the modularity contract; the `over_fulfillment` label + fixed-window pagination + fulfillment-only version pin were latent implementation traps — all removed by construction here | Control-room **delta** review of this corrected package (DEC-038 + packet + DoR + locked prompt + UAT matrix + risk register + trackers); on acceptance, rulings-confirmation + base-SHA verification + explicit issuance of the locked prompt before any Gate B implementation; CV-013 (#185) executed green before final Wave 4 acceptance / RC / UAT. **Wave 4 Gate A package: CANDIDATE — READY FOR CONTROL-ROOM DELTA REVIEW. Draft PR #188, docs-only, unmerged, not marked ready, not self-accepted. No `addons/**` change; no Shopify operation; no Gate B / Wave 5 started; #185 open and critical; Q1–Q8 ruled/applied. Evidence: the corrected Gate A canonical outputs + `docs/07-implementation-plan/wave-4-gate-a-handoff.md` §5.** |
| AR-072 | 2026-07-22 | Wave 4 Gate A — final control-room micro-correction applied (draft PR #188) | The control room's **delta review** of AR-071's corrected package at head `50929ed9159f9ca507c0c50b0a21d3af0fe1f658` returned **REVISE — FINAL NARROW REVISION** (PR #188 comment [`5042183642`](https://github.com/AdamsOdoo/Adams/pull/188#issuecomment-5042183642); status issue #186 comment `5042185019`), accepting the corrected substance but naming five narrow decision-critical defects. This session applies one final documentation-only micro-correction on the same branch/PR (**no `addons/**` change; no new file; no Shopify operation; PR stays draft/unmerged/not-ready**): **(P0)** post-C2 `NOT_APPLIED` **no longer authorizes any resend** — post-C2 reconciliation has only **APPLIED / INCONCLUSIVE**, and a replacement is reachable only from `transport_attempted=false` or a **synchronous `userErrors` clean rejection** (DEC-038 §7.1 B/D/E + #29; packet §11.1/§10.3/D-014-7; DoR #10/§7; UAT-FM-1.9; SRR-10; locked prompt §4/§5); **(P1)** one **shared** `fulfillment_mutation_reconcile` job type replaces the two per-domain reconciles, linked to a single `mutation_attempt_id`, dispatched by its `mutation_domain`, owning/inheriting **no** remote-effect operation scope, with an explicit lock→supersede→**flush**→insert handoff ordering (DEC-038 §7.3 + #24/#26; packet §11.2); the taxonomy is frozen at **exactly ten job types** (the freeze-defeating "validated/consolidated at Gate B" caveat removed) with **`fulfillment_review_release`** demoted to a sanctioned-service helper (not a job type); **`webhook` removed as a Wave 4 source** (forbidden this wave — DEC-038 #35; UAT-FM-1.8/2.12/2.14); the `fulfillment_tracking_change` trigger-origin uses a **dedicated** `_normalize_tracking_change_trigger_origin_on_uninstall` callable (not the job-type sink `_reassign_to_historic_job_type`), normalizing the removed value to core `fulfillment_picking_validation` with audited provenance (DEC-038 §7.5 + #13; packet §11.5); the active DEC-038 41-item matrix reconciled to the applied rulings (#13/#21/#24/#26/#29/#35) and the nonexistent §14 cross-ref fixed; PR #188 body commit-count wording corrected. **This supersedes AR-071's `READY FOR CONTROL-ROOM DELTA REVIEW` disposition.** | **REVISED — RESUBMITTED FOR CONTROL-ROOM FINAL DELTA ACCEPTANCE ON THE SAME DRAFT PR; Claude did not accept its own correction, did not mark the PR ready, and did not merge** | No new official-source campaign (the micro-correction responds only to the control room's own binding delta-review findings); the reconcile-only rule rests on the already-verified fact that fulfillment mutations are absent from Shopify's `@idempotent` list, so no request-specific non-application proof exists post-C2 | Identity gate independently re-verified live before any edit (branch `claude/wave-4-gate-a-review-0nbhdw` at the exact expected head `50929ed9159f9ca507c0c50b0a21d3af0fe1f658`; PR #188 open/draft/unmerged; base `mvp/program-integration@ab4f12f5a6857b2f3318ffc3b3f5f371307938bc`; issue #185 open; comments `5042183642`/`5042185019` present; no `addons/**` change on the branch; no Wave 4 implementation branch/PR authorized); only the authorized docs files were edited; a `git diff --name-only <base>...HEAD -- addons/` empty-proof + a no-new-file check are re-run before push | Leaving "positive NOT_APPLIED → replacement" abstract in an accepted Gate B prompt would have let a future implementation invent an unsafe non-application predicate and restore the duplicate/notification risk the first correction closed; a reconcile job inheriting the mutation's remote-effect operation scope would have diverged from the proven inventory Layer 2 pattern and could collide at handoff; a taxonomy declared frozen but "revisitable at Gate B", a webhook source in a webhook-forbidden wave, and a trigger-origin `ondelete` that cleans only `job_type` were latent traps — all removed by construction here | Control-room **final delta acceptance** of this corrected package; on acceptance, base-SHA verification + explicit issuance of the locked prompt before any Gate B implementation; CV-013 (#185) executed green before final Wave 4 acceptance / RC / UAT. **Wave 4 Gate A package: CANDIDATE — READY FOR CONTROL-ROOM FINAL DELTA ACCEPTANCE. Draft PR #188, docs-only, unmerged, not marked ready, not self-accepted. No `addons/**` change; no new file; no Shopify operation; no Gate B / Wave 5 started; #185 open and critical; Q1–Q8 ruled/applied. Evidence: the corrected Gate A canonical outputs + `docs/07-implementation-plan/wave-4-gate-a-handoff.md` §6.** |
| AR-073 | 2026-07-22 | U0 operator UI — navigation menu scope (draft PR into `mvp/program-integration`) | The U0 prompt (§12) specifies a five-item first-level menu (Dashboard, Stores, Sync Center, Error & Review Center, Logs); the accepted `screen-inventory-and-navigation-map.md` describes a seven-entry *end-state* menu (Dashboard, Sync Center, Error Center, Catalog & Matching, Inventory, Fulfillment, Configuration→Store Settings). U0 implements the prompt's deliberate five-item subset and forbids empty placeholders for later waves. | **BUILDER DECISION — IMPLEMENTED, PENDING INDEPENDENT REVIEW** | Stores surfaced top-level for U0 (Configuration branch is not built); Logs surfaced top-level and also reachable from the job form; Mutation Evidence nested under Error & Review Center. Not a hard conflict: the prompt's §12 is a self-consistent U0 scoping choice. Reconcile with the seven-entry map when U1–U3 land. Flagged per CLAUDE.md "raise conflicts". |
| AR-074 | 2026-07-22 | U0 operator UI — dashboard aggregate service is an `AbstractModel` | The single read-only dashboard data provider (`shopify.connector.ui.dashboard`) is a `models.AbstractModel` exposing one public `@api.model get_dashboard_data`, called from the Owl client action via `orm.call`. Chosen so the only ACL additions in U0 are the two transient wizards (§19), consistent with the existing abstract-service pattern (readiness_check, dispatch, enqueue). | **BUILDER DECISION — IMPLEMENTED, RUNTIME-VERIFY RPC DISPATCH** | Reads are bounded `search_count` + one `limit`-ed `search_read`; an Auditor-group data-layer gate provides defence in depth. Runtime item: confirm RPC dispatch to an `AbstractModel` public method under Odoo 19 (expected; standard dashboard pattern). |
| AR-075 | 2026-07-22 | U0 operator UI — mutation-evidence indicator uses existing `mutation_attempt_id` | The job model is outside the U0 allowlist and cannot gain a computed `has_mutation_evidence` field; the job list/form use the existing `mutation_attempt_id` to indicate and hide generic retry/cancel on evidence-linked jobs. | **BUILDER DECISION — IMPLEMENTED (honest UX approximation)** | The server's `_has_mutation_attempt_evidence` guard (which also covers the reverse `attempt.job_id` link) remains the authoritative control; a rare reverse-linked job that still shows a button is refused server-side with a clear error. |
| AR-076 | 2026-07-22 | U0 Stage R1 — Administrator boundary on the four public store lifecycle/probe actions (`store.py`) + Odoo-19 view-validation fixes | Genuine Odoo.sh runtime (PR #192) proved (a) an install-blocking Odoo-19 view-validation P0 — search `<group>` rejects `expand`/`string` and the field-accessibility validator rejects `active_id` in a stat-button context — and (b) the known Test Connection direct-RPC P1: on the unfixed tree an Auditor/Operator/Reviewer/plain caller of `action_test_connection` was **not denied at all** and created a job + two `job.log` rows (the probe funnels job/log/credential/transport work through `sudo()` before the late non-sudo store write). Correction: plain `<group>` + `active_id`→`id` in the four search/form views; one `_ensure_connector_admin_boundary()` enforcing the **existing** `group_shopify_connector_admin` at the top of `action_test_connection`/`action_activate`/`action_disconnect`/`action_reconnect`, before any side effect — mirroring the guard already in `action_force_disconnect`. The framework superuser (`env.su`) is exempt (crons/controller/test harness); a real RPC caller can never be `su`, so this is a strict tightening. | **BUILDER DECISION — IMPLEMENTED + RUNTIME-VERIFIED (Stage R1, pre-rebuild); awaiting DEC-040 independent review; not self-accepted/merged** | No new role/group; no lifecycle redesign; Administrator outcomes unchanged; one-revert U0 rollback preserved (no schema/data migration/new job state). `action_reconnect` (shares the probe), `action_disconnect` (audited-no-op sudo job) and `action_activate` (store-row lock) were the confirmed analogous defects and are covered by the same guard. Corrected-tree evidence: U0 suite 63/0/0; core 368 tests 0 failed (11 environmental `autopost_bills` at_install errors); leak scan clean. Exact-build proof + browser tours/HOOT (container thread limit) are Stage R2. |
| AR-077 | 2026-07-22 | U0 Stage R2 — `action_mark_reconnect_needed` Administrator boundary + accepted test/evidence corrections (PR #192) | Independent review of the exact Stage R1 head `0fa512d2ecec028f6e6bc4198de441c9f50224c1` (PR #192 comment `5049668193`) returned **REVISE**, confirming one new P1 the AR-076 sweep missed: `action_mark_reconnect_needed` (`store.py:1237`, pre-existing code, byte-identical to base) had **no** `_ensure_connector_admin_boundary()` call — on a `disconnecting`/`disconnected` store its audited-no-op branch created a `sudo()`-backed Job/JobLog with **no denial at all**, the exact defect class AR-076 claimed to have eliminated everywhere. Control room accepted the verdict verbatim (`5049734472`) and authorized one consolidated correction. This session applied it: `_ensure_connector_admin_boundary()` is now the first statement in `action_mark_reconnect_needed`; `test_ui_visibility_matrix.py`'s direct-call test now asserts zero lifecycle-lock/transport/Job/JobLog/store/credential deltas for all five privileged public store actions (including `action_mark_reconnect_needed`'s two dangerous `disconnecting`/`disconnected` branches); `test_ui_actions.py` gained two mutation-resolution-wizard refusal-path tests (non-uncertain, already-resolved); `test_ui_source_guards.py`'s controller/OAuth guard now scans the whole production tree via the AST instead of a four-filename-prefix gate, with a synthetic-fixture rejection proof; the dashboard sparkline gained a textured (non-colour) failure-bar fill plus a per-day accessible text equivalent; `ui-u0-validation-results.md` §11/§12 were corrected to state the `526ad63` ACCEPT never covered the Stage R1 security code and that no final ACCEPT exists yet for `0fa512d` or this correction. | **CLOSED — independently accepted and merged.** Fresh independent delta review of exact head `a13f67210269277826e78b23be1fab5e0caffec5` returned `ACCEPT` (PR #192 comment `5050387258`; no P0/P1/material P2); control room accepted the verdict and the exact-SHA runtime evidence (build `35308219`) in comment `5050525557`. A separate closure session merged PR #192 into `mvp/program-integration` via merge commit `8818c7714f46eefe51c6b452b5e3f24d155f26fb`. No new architecture decision introduced by this closure. | No new role/group/state/field/model/job type; the one internal caller of `action_mark_reconnect_needed` (`_apply_probe_failure`, via `_run_connection_probe`) always reaches it only after its own caller (`action_test_connection`/`action_reconnect`) already passed the identical guard, so the fix is a no-op re-check for every legitimate internal path; one-revert U0 rollback preserved (no schema/data migration). Static validation only in this session (py_compile, AST parse of every changed file, the hardened guard logic run directly against the real tree, allowlist conformance) — no Odoo.sh runtime executed and none claimed. The exact-head independent delta review and a fresh Odoo.sh build remain outstanding before any closure session may ready-mark/merge. |
| AR-078 | 2026-07-22 | Wave 4 Gate B — fulfillment/tracking backend implementation (draft PR #189) | Executed the issued Gate B locked prompt (`docs/06-prompts/sol-wave-4-fulfillment-locked-prompt.md`, blob `ad7418f846ae0479471306c3ae997ac4eb60df4b`; issued by issue #186 comment `5043052341` + PR #188 comment `5042975042` source/origin amendment) from the exact base `mvp/program-integration@01f072dd4d83b7b39737452a686244a3a8c00332`. Implemented the new `shopify_connector_fulfillment` addon (one `shopify.connector.fulfillment.service` split across the enumerated responsibility files) + the one named core edit (`REQUIRED_MVP_SCOPES` swap + its test): D-014-1 binding + per-fulfillment/per-line inbound evidence; the ten frozen job types with historic-sink `ondelete` and the **dedicated** `_normalize_tracking_change_trigger_origin_on_uninstall` trigger-origin callable; the Q1 operation-scope override for the two mutation types only (shared `fulfillment_mutation_reconcile` owns no remote-effect scope); both fulfillment_create + fulfillment_tracking_update 7-callback Layer 2 strategies (anonymous-form GraphQL module constants, **no `@idempotent`**, RA-022 FulfillmentOrder-only, RA-023 explicit 2-hop line matching); reconcile-only post-C2 (**APPLIED / INCONCLUSIVE only**, read absence = INCONCLUSIVE, cap 3 → `duplicate_risk`, no resend, no repeated notification); admission (one create per picking, per-FO line decomposition), the `stock.picking._action_done` trigger + tracking-change hook; the 16-condition Mode 2 evaluator + Q6 carrier fail-closed + local validate; Mode 1 review actions + the review-release sanctioned helper (manual_sync replacement, not a job type); inbound observation + origin classification + 7-family state normalization; reconciliation/reconnect/mode-switch scans + state machine + cron; fulfillment readiness seam (write-scope essential, API-version gate Q7, staff-permission NOT_PROVEN warning Q8); ACL + multi-company record rules. All 22 frozen test filenames + the out-of-band spawn concurrency harness present. | **CANDIDATE — READY FOR ONE EXHAUSTIVE CONTROL-ROOM REVIEW; produced by Claude Code as the authorized Gate B implementation worker (issue #186 comment `5043052341`); NOT self-accepted; draft PR #189 unmerged/not-ready; no live Shopify mutation; no Gate C/Gate D/Wave 5 work started** | The **frozen core** source-guard logic (`test_mutation_source_guards.py`) and the fulfillment source guards were executed standalone in the workspace against the production tree: **0 violations** on the mutation-literal-guarded-transport guard (the anonymous-operation module constants keep the frozen `ACCEPTED_PREPARE_TRANSPORT_SPLIT` allowlist untouched), 0 on raw-transport / `_send`, 0 on V2/legacy / holds / `@idempotent` / `qty_done` / `location.mapping` / `webhook`. `py_compile` clean over all production + test files. Odoo runtime is **not available in this workspace** → the `TransactionCase` suite is `IMPLEMENTED—RUNTIME PENDING` (Gate C). No new official-source campaign (the accepted Gate A package governs). | Identity gate PASSED — base resolves exactly to `01f072dd`; PR #188 merged at that state; issue #185 (CV-013) open/critical; issue #186 comments `5043052341`/`5043013284` + PR #188 comments `5042975042`/`5042183642`/`5041620950` verified; protected checkpoint `acd8c4691e72cf5590f2a56228b08f183b76cd9a` unchanged; locked-prompt blob matches; no competing authorized Wave 4 Gate B branch/PR; `shopify_connector_fulfillment` not previously implemented. Branch `claude/wave-4-fulfillment-gate-b` (the frozen name from the issuance comment; this session's harness branch was `claude/wave-4-gate-b-fulfillment-90jhqh`). | The single hardest constraint was the frozen `test_mutation_source_guards.py` allowlist (`ACCEPTED_PREPARE_TRANSPORT_SPLIT` asserted to be exactly the two inventory pairs, un-editable): resolved by writing the fulfillment mutation documents as **anonymous** GraphQL operations (module constants) so the core named-mutation regex never matches, keeping the frozen guard green while this addon's own `test_fulfillment_source_guards.py` provides the real guard. The taxonomy vs `UNIQUE(store, picking)` tension was resolved to one fulfillment_create per picking (per-FO line decomposition inside one call; backorder chains → separate pickings → separate fulfillments). | Control-room **one exhaustive review** of draft PR #189; on acceptance, Gate C Odoo.sh runtime + Gate D dev-store campaigns; CV-013 (#185) executed green before final Wave 4 acceptance / RC / UAT. **Wave 4 Gate B: CANDIDATE — READY FOR ONE EXHAUSTIVE CONTROL-ROOM REVIEW. Draft PR #189, unmerged, not marked ready, not self-accepted. No live Shopify mutation; no Gate C/D/Wave 5 started; #185 open and critical. Evidence: `docs/05-qa/task-014-fulfillment-tracking-validation-results.md`.** |
