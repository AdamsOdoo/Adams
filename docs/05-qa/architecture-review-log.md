...
| AR-050 | 2026-07-15 | Task LC-1 — accepted DEC-030 lifecycle implementation | Implement the already-accepted disable-first plus supported-domain-uninstall posture: permanent historic job type, set-once original type, audited cancellation of non-terminal rows, terminal retyping without log loss, domain selection-removal callables, additive upgrade backfill, and fail-closed historic dispatch | **Implementation and prior exact-head runtime conform to accepted DEC-030; corrected-head regression repeat pending** | This is the smallest implementation of the locked lifecycle design and keeps domain-owned bindings subject to the documented export/re-derivation posture rather than moving them into core | Build `34986844` at prior exact head `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723` proved the lifecycle suite `0/0/9` plus full fresh/standard suites; the later binding-security correction requires a corrected-head regression repeat | Physical uninstall intentionally loses domain binding/mapping tables; rollback must not drop the additive column or delete audit/business data | Record the corrected-head repeat in `task-lc1-validation-results.md`; Claude reviews only at the complete Wave 1 boundary | **Implemented on draft PR #172; prior runtime green; corrected-head repeat pending** |
| AR-051 | 2026-07-15 | Task JOB-ACTIONS — generic core manual retry/cancel doors | Implement accepted D-JA-1 as a pure additive job-model extension with exact state and role gates, mandatory cancellation reason, one audit row, and no bypass; reserve only the two write-site elevations for SEC-1 | **Implementation and prior exact-head runtime conform to accepted DEC-034; corrected-head regression repeat pending** | Keeps generic job controls in core without importing Wave 2 Area 6 scan/domain-trigger scope and gives SEC-1 concrete sanctioned action doors | Build `34986844` at prior exact head `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723` proved the full standard suite green; the later binding-security correction requires a corrected-head action regression repeat | Incorrect group or state gates could bypass recovery policy; SEC-1 must add only the reserved elevation while preserving behavior | Record the corrected-head repeat in `task-job-actions-validation-results.md`; Claude reviews only at complete Wave 1 boundary | **Implemented on draft PR #172; prior runtime green; corrected-head repeat pending** |
| AR-052 | 2026-07-15 | Task SEC-1 — protected job/binding mutations and PII controls | Implement accepted D-SEC1-1..7 on the existing Wave 1 substrate, including all product-owner completeness/runtime rulings: protected `original_job_type`; lifecycle audit carrier; `env.company` consistency; accepted CORE-R2 recovery transitions; test-only AST guard compatibility; and the complete binding identity/structure/system-state/provenance/imported-snapshot surface | **CORRECTION IMPLEMENTED / STATIC-GREEN — corrected exact-head runtime pending** | Claude's review found the binding guard covered identity but omitted provenance and imported snapshots. Product-owner ruling `4988842625` required the complete current surface and a fail-closed extension seam. Commit `36974edc68c1985e6ccfae8f6bb5c7386f820156` implements that correction without new ACLs, groups, models, tables, job types/sources, context bypasses, dispatcher changes, or Wave 2 scope | Static proof: all changed Python parses; exact 16/17/14 protected sets asserted; all four roles receive create/alter/clear negative coverage; sanctioned override/import/mask/retention behavior remains covered; product importer sudo inventory changes only at the accepted existing-variant refresh and image-checksum writes. Prior build `34986844` remains valid for prior SHA `05bb4631d3fdf3c6c8b54c09deb7e0b1dc72f723`; corrected exact-head Odoo.sh matrix is required | The correction is production security code and therefore cannot inherit the earlier exact-head runtime conclusion. SRR-03 remains CLOSED because its independent runtime proof and production path are unchanged. No exactly-once remote-effects or DEC-031 Layer 2 claim | Run the complete corrected-head matrix in the single runtime-operator handoff, then return PR #172 to Claude for macro-wave re-review; Claude alone may merge | **Draft PR #172 in correction/revalidation; SRR-03 CLOSED; Wave 2 unauthorized** |
