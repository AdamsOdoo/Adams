...
| AR-052 | 2026-07-15 | Task SEC-1 — protected job/binding mutations and PII controls | Implement accepted D-SEC1-1..7 on the existing Wave 1 substrate, including the two product-owner completeness rulings: protect `original_job_type`; reuse the lifecycle maintenance audit carrier; validate current/target companies against `env.company` before narrow sudo | **Implementation follows accepted DEC-034/DEC-028 and binding rulings; runtime review pending** | Closes the RPC/ORM mutation and PII exposure surfaces server-side without ACL duplication, a new audit table, store ownership, or context flags | Exact-head Odoo.sh four-role/security, lifecycle, importer, retention, full-suite, leak/residue and genuine SRR-03 concurrency evidence on PR #172 | Incorrect sudo inventory or audit atomicity could create a bypass; masking is irreversible by accepted design; no runtime result exists yet | Record runtime in `task-sec1-validation-results.md`; keep SRR-03 OPEN until independent-connection proof; Claude reviews only at complete Wave 1 boundary | **Implemented at `60ac416`; runtime blocked by unavailable Odoo.sh access (hard-stop 5); PR #172 remains draft** |
