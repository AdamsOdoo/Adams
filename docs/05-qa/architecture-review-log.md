...
| AR-011 | 2026-07-03 | Master Blueprint Product, Customer, and Sale/Order Domain Blueprint | **Proposed via [`DEC-014`](../04-decisions/DEC-014-master-blueprint-product-customer-sale.md)** — Master Blueprint Part B: product-template/product-variant binding under `shopify_connector_product`; customer/order binding under `shopify_connector_sale`; product import/export/update flow (variant-mutation-strategy direction, draft/publish mechanism via `Product.status`+`publishablePublish`, media/price handling per DEC-007 §2/§3); customer import/matching (default-customer-fallback direction, proposed email-only match-key recommendation); order import (whole-order-hold rule for unmatched product lines, two-path customer-resolution rule, total-check guard definition routed to the existing `financial total mismatch` error class, proposed order-import operator-touchpoint recommendation, gateway→journal mapping concept); cross-domain sequencing; consolidated error/retry-class mapping (no new class added to the fixed 16-class registry); open-questions register updates for MBQ-23–31 plus new rows MBQ-55–58 | **Proposed for ChatGPT review** (not yet accepted) | Synthesizes already-accepted DEC-003/006/007/012/013 content; two small, targeted official-doc checks performed this sprint (Shopify `productSet`/`productVariantsBulkCreate`/`Product.status`/`publishablePublish` reference pages, accessed 2026-07-03; Odoo 19 accounting/taxes documentation, accessed 2026-07-03, inconclusive) — no broad research performed, per the sprint's "targeted checks only" instruction | ChatGPT/Fable review of the Part B blueprint document against DEC-003/006/007/012/013 and RA-001–023 — not yet reviewed | Blueprint detail being read as code-level commitment (mitigated: proposed-names-only discipline continued from Part A, MBQ-01/02/55 unresolved); the automated-vs-interactive duplicate-prevention-preview clarification being read as weakening DEC-003/006's "no blind create" rule (mitigated: synchronous confirmation preserved for every ambiguous/binding-conflict/duplicate-risk state; only the confident-unambiguous case is relocated to retrospective sync-center/dashboard visibility); MBQ-26/MBQ-31 recommendations being mistaken for already-decided outcomes (mitigated: both explicitly labelled "recommendation to ChatGPT" throughout, both name their register decision owner as ChatGPT (Sprint B)) | RB-14 → **proposed** [`../04-decisions/DEC-014-master-blueprint-product-customer-sale.md`](../04-decisions/DEC-014-master-blueprint-product-customer-sale.md) (Proposed for ChatGPT review, 2026-07-03); next, pending acceptance → Master Blueprint Sprint C (inventory/fulfillment domain blueprint), not started | **Proposed** |
