...
| AR-015 | 2026-07-04 | Proposed MBQ Decision Batch 1 | **Proposed via [`DEC-018`](../04-decisions/DEC-018-mbq-decision-batch-1.md)** — a controlled first decision batch executing Part E's own "ChatGPT batch" (§4): proposed decisions, with evidence/options/risk/register-impact wording, for **MBQ-06** (readiness-check essential-vs-nice-to-have split), **MBQ-08** (store-disconnect data-retention posture), **MBQ-17** (reconciliation cadence/scope posture), **MBQ-33** (first-push guard granularity), **MBQ-34** (ongoing inventory apply mode), **MBQ-41** (fulfillment notification-UI granularity), **MBQ-45** (roles→groups mapping / admin-vs-functional surface split), **MBQ-52** (Shopify API-version pinning/upgrade policy), **MBQ-54** (domain-module uninstall/disable lifecycle), and **MBQ-60** (`stock_delivery`/`delivery` dependency for fulfillment tracking) — each adopting a direction an already-accepted Master Blueprint part (mostly DEC-013/DEC-015) already carried as a ChatGPT-owned recommendation, none new architecture. **MBQ-62** (Odoo-event-triggered job-source classification) is **not forced to a decision** — DEC-018 §4 checks it against all six fixed Part A §D.2 job-source values, finds none a defensible fit, and recommends splitting it into its own dedicated follow-up decision record rather than repeating the exact unauthorized-vocabulary-extension failure mode Fable's finding C2 already caught during DEC-015's review. **MBQ-64 and MBQ-65 are explicitly out of scope**, routed to a separate currency/webhook residual decision sprint per the task's own scope instruction. **No MBQ row is resolved, marked accepted, or silently changed by this proposal or by AR-015 itself — every row in the register remains exactly as open as before until and unless ChatGPT accepts DEC-018.** | **Proposed for ChatGPT review — NOT accepted** | Synthesizes already-accepted DEC-013/DEC-015 recommendations (as quoted in this log's AR-010/AR-012 rows and in the open-questions register's own acceptance-patch notes) plus Part A/C blueprint sections (§B.1/§B.3/§D.2/§E.1–E.6/§I.3–I.4/§J.1–J.2; §A.5/§A.7/§B.5/§B.6) read directly this session; no new Tier-1 platform-fact claim introduced; no external Shopify/Odoo research performed, per the task's own "avoid external research unless absolutely necessary" scope instruction — every proposed decision rests on evidence Parts A/C and DEC-013/015 already established and cited | Not yet reviewed — checked against `rejected-approaches-log.md` (RA-008 blind first-push, RA-009 hidden/default-on notification, and the full RA-001–023 list) before proposing; none of the ten proposed decisions reintroduces a binding rejected approach; MBQ-62 checked with special strictness per the task's own instruction and found not to have a defensible existing-vocabulary mapping | A posture recommendation being mistaken for a decision (mitigated: DEC-018's own Status section and every table cell label the batch "Proposed... NOT accepted"; register-impact wording in DEC-018 §5 is explicitly drafted-only, not applied); MBQ-62 being forced into a weak mapping under batch-review time pressure (mitigated: explicitly declined, with a named split recommendation, rather than forcing a sixth-value guess); this batch being mistaken for an implementation-gate-opening act (mitigated: DEC-018 §7 restates the gate stays closed regardless of this batch's outcome, and that ~35 other blocking MBQ rows remain untouched even if the batch is accepted in full) | Awaiting ChatGPT review of [`DEC-018`](../04-decisions/DEC-018-mbq-decision-batch-1.md); if accepted, an acceptance-patch note (mirroring the DEC-013–017 pattern) would update this row and the register; if MBQ-62's split recommendation is accepted, a dedicated follow-up DEC would be the next step for that row specifically | **Proposed** |
...
ChatGPT review; NOT accepted.** Prepared after DEC-017 was confirmed
accepted (2026-07-04) and PR #80 confirmed merged into `Shopify-connector`
(merge commit `403d17fc16c6854b0bd9f3ce3161ff61cc0e1570`). This session
proposes
...
(recommended split into its own follow-up DEC instead). **MBQ-64 and MBQ-65
remain out of scope**, reserved for a separate currency/webhook residual
decision sprint. **This session does not resolve any MBQ, does not mark any
decision accepted, does not mark DEC-018 accepted, does not open the
implementation gate, does not create any implementation task, and does not
change DEC-003 through DEC-017 or any accepted Master Blueprint Part A–E
content.** Per `CLAUDE.md` §10, checked against `rejected-approaches-log.md`
before proposing — no proposed decision reintroduces RA-008, RA-009, or any
other binding rejected approach. Recommended next (pending this review):
ChatGPT's review of DEC-018/AR-015, then — if accepted — an acceptance-patch
note updating this row and the open-questions register, and a dedicated
follow-up session for MBQ-62 (and, separately, the MBQ-64/MBQ-65 currency/
webhook residual sprint). **Implementation remains blocked; the
implementation gate remains closed** regardless of this session's or
DEC-018's outcome._
