...
| AR-044 | 2026-07-12 | Task CORE-R2 — disconnect quiescence & in-flight job contract (design only; draft PR into `Shopify-connector`) | After PR #153 merged (accepted evidence, `4950408383`; SRR-03 kept OPEN, DEF-PB-1 confirmed) and ChatGPT's CORE-R2 **design** gate act (PR #153 comment `4950413650`, docs-only, one session, verified base `fcbbb0b3fe3db9cba354a8a1c08e91036b70ec1f`), this session designs the DEF-PB-1/SRR-03 remediation and prepares a future implementation packet — **no code**. **Revision 2 (2026-07-12, review `4951115877`)** re-evaluated Option E: enforcement is moved to the **central Shopify API client** `execute()` (fail-closed) against a **persisted connection epoch** — `store.connection_generation` vs an enqueue-captured `job.expected_connection_generation`, read in a **fresh cursor** so a fast disconnect→reconnect cannot revive an old job; a **complete in-flight taxonomy** now covers **claimed-but-not-started (row-locked)** jobs (revision 1's "queued jobs are unlocked" claim was false); job cancellation is **non-blocking** (`try_lock_for_update()` + `SKIP LOCKED` + bounded polling); the quiesce controller is a **dedicated, high-priority, trigger-driven `ir.cron`** (not a drained job — no starvation, no "reschedule-self-marked-succeeded"); **timeout escalation is recorded on independently-writable store-level fields** (incl. `disconnect_stuck_job_id` as a plain Integer) + a new audit job, **never** the locked job row; **the advisory lock is DROPPED** in favour of store-row optimistic state+epoch under `retrying()`; Phase-1 RPC return means **request accepted**, not completed; a Phase A/B/C side-effect contract preserves local reconciliation of an admitted call. Source-verified: REPEATABLE READ (`odoo/sql_db.py:373`), `retrying()` (`odoo/service/model.py:160`), non-blocking `FOR UPDATE SKIP LOCKED` (`odoo/orm/models.py:5564`/`5592`), cron `_trigger`/priority + single-commit + `_commit_progress` (`odoo/addons/base/models/ir_cron.py:735`/`303`/`671`/`846`). Deliverables: [`../03-architecture/disconnect-quiescence-remediation-analysis.md`](../03-architecture/disconnect-quiescence-remediation-analysis.md) (26 sections) and [`../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md`](../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md); SRR-03/04/09 wording, master-plan CORE-R2 dependency, and research-handoff top entry updated. | **Proposed for ChatGPT review — design only; no gate opened.** Revision 2 answers the nine `4951115877` blocking points (in-flight taxonomy incl. locked rows; request-vs-completed; central enforcement + epoch; controller starvation/state machine; timeout-without-locked-write; bounded coordination; broadened critical path incl. reads; side-effect phases; executable INV-2 tests). The analysis §26 enumerates six calls (D-CR2-A…F). Accepting the PR without named exceptions accepts these as proposals; the CORE-R2 **implementation** gate is a **separate, later** explicit act on the packet. | Gives the DEF-PB-1/SRR-03 remediation an internally-complete, source-grounded contract and an executable future packet so, once gated, implementation is fast and defensible — and so no Shopify-calling handler (incl. reads) ships/live-validates on top of an unremediated disconnect race | The runtime evidence (PR #153 §10/§14 + `scenario-06-real-timeline.md`); the Odoo 19 / PostgreSQL 16 source citations in the analysis; **live Odoo.sh + genuine two-server proof are required by the packet before SRR-03 can move off OPEN** — none claimed by this design session | Residual races stated honestly (RR-1 gate→`_send` window; RR-2 already-issued call; RR-3 timeout escalation; RR-4 multi-host; RR-5 context-stripping guarded by a source-level test; RR-6 SRR-06 concurrency-fix-needs-live-proof). No addon/test/code changed; six-file docs-only scope; SRR-03 OPEN, SRR-04/09 REDUCED-not-closed unchanged | ChatGPT reviews the draft PR, makes the six §26 calls, and — if accepted — later performs a **separate** CORE-R2 implementation-gate act (naming the then-current base SHA and re-freezing the packet's allowed-file list) in a new session; no addon code is authorized by this review | **Proposed for ChatGPT review (rev 2) — design/packet only; implementation gate CLOSED** |
