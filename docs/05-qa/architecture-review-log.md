...
| AR-030 | 2026-07-08 | Task 006B sync engine architecture gate | Proposed domain-neutral sync-engine architecture gate / [`DEC-025`](../04-decisions/DEC-025-task-006-sync-engine-gate.md), synthesizing the accepted Task 006A research package (PR #123–#127) into architecture principles, a proposed core-engine shape, a job-trigger model, a retry/failure policy, an idempotency/duplicate-prevention layering, the Odoo execution substrate, Shopify API implications, and a core-vs-domain responsibility table — built directly on already-accepted [`DEC-005`](../04-decisions/DEC-005-sync-orchestration-strategy.md), [`DEC-009`](../04-decisions/DEC-009-error-retry-idempotency-strategy.md), [`DEC-013`](../04-decisions/DEC-013-master-blueprint-core-substrate.md), and [`DEC-024`](../04-decisions/DEC-024-task-005-closure.md); full detail in [`../03-architecture/sync-engine-architecture-gate.md`](../03-architecture/sync-engine-architecture-gate.md) | **Proposed for ChatGPT review — NOT YET ACCEPTED** | Consolidates already-accepted Task 006A evidence into a single proposed architecture gate so a future implementation-scope session has a synthesized shape to draft against; **no implementation authorization of any kind is proposed or implied** | ChatGPT review of PR #128 (this proposal) against [`../05-qa/task-006b-architecture-gate-review-checklist.md`](../05-qa/task-006b-architecture-gate-review-checklist.md); confirmation that every Task 006A open item (VAL-B2, MBQ-05, TD-002, fulfillment API model, product first-sync dedup thresholds, Lite/Full packaging) remains preserved, not silently resolved; confirmation that the named concurrency claims (job-acquisition locking, disconnect race, multi-server coordination) remain flagged as requiring live Odoo.sh/multi-server runtime proof before any implementation relies on them | Premature implementation starting before this gate (or a follow-on implementation-scope document) is actually accepted; the unresolved job-claiming-concurrency and multi-server-coordination questions being silently assumed safe; token-acquisition (MBQ-05), fulfillment-API-model, product-dedup-threshold, and Lite/Full-packaging assumptions being treated as settled when none is decided by this proposal | ChatGPT reviews DEC-025 and `sync-engine-architecture-gate.md`; if accepted, a separate future acceptance patch updates DEC-025's status to Accepted and this row's Review decision/Status to Accepted, and names the next recommended session (a separately-scoped, separately-authorized Task 006C implementation-scope drafting session) — none of that is performed by this row | **Proposed** |
