...
| AR-016 | 2026-07-04 | Proposed MBQ-62 Odoo-side event job-source classification | **Proposed via [`DEC-019`](../04-decisions/DEC-019-mbq-62-odoo-event-job-source.md)** — a dedicated follow-up decision record for the one row DEC-018 declined to force a same-batch answer for: whether Part A §D.2's fixed six-value job-source vocabulary should be extended with a seventh value for Odoo-side event triggers (an inventory push enqueued by a relevant Odoo stock change; a fulfillment creation triggered by a validated `stock.picking`), or classified another way. Evaluates four options (add a seventh value; existing source + trigger-origin metadata; a separate trigger-origin dimension with no seventh value; defer Odoo-side event triggers from Phase 1) against dashboard and retry-policy impact, and **proposes** extending the vocabulary with a seventh value, `odoo_event`, paired with a required trigger-origin sub-classification naming the specific Odoo event ("inventory stock-change trigger" / "fulfillment picking-validation trigger") — combining the useful parts of the first and third options while rejecting the second and fourth. **This is a proposal only — no MBQ row is resolved, no register wording is applied, and no implementation authorization or gate-opening act is performed.** | **Proposed for ChatGPT review — NOT accepted** | Synthesizes already-accepted DEC-009/DEC-010/DEC-011/DEC-013 content plus DEC-018's own strict per-value analysis (§4, independently re-confirmed, not re-derived from scratch) and Part A §D.2/§D.5/§D.8/§D.10/§F.1/§G.1/§G.7 and Part C §A.7/§A.13/§B.3/§B.12; no new Tier-1 platform-fact claim introduced; no external Shopify/Odoo research performed, per this session's scope instruction — every claim rests on evidence already established and cited in Parts A/C and DEC-009/010/011/018 | Not yet reviewed — checked against `rejected-approaches-log.md` in full before drafting; no row addresses job-source vocabulary, so nothing is reintroduced; the four options were evaluated with the same strictness DEC-018 applied, explicitly declining a weak mapping onto an existing value in favor of either a genuine seventh value or an honest deferral (deferral itself found unavailable for fulfillment, per DEC-011) | Awaiting ChatGPT review of [`DEC-019`](../04-decisions/DEC-019-mbq-62-odoo-event-job-source.md); if accepted, a future acceptance-patch session would apply DEC-019 §6's pre-drafted MBQ-62 register wording, mirroring the DEC-013–018 acceptance-patch pattern | A proposal being mistaken for a decision (mitigated: DEC-019's Status section and this row both state "Proposed for ChatGPT review — NOT accepted" and that no MBQ row is resolved by this record); the proposed semantic value `odoo_event` being read as itself unresolved because its implementation mechanics are open (mitigated: DEC-019 §5/§6, clarified this session, state plainly that `odoo_event` is proposed as the semantic job-source value and, if accepted, becomes the accepted seventh Part A §D.2 value — only implementation mechanics and exact field/model/constant placement (Odoo model names, field names, Python constants, XML IDs, storage/Selection-field mechanics, trigger-origin field/model implementation, MBQ-16 constants) remain implementation planning; DEC-019 is still proposed and not accepted); this record being mistaken for a gate-opening or implementation-authorizing act (mitigated: DEC-019 §7 restates the gate remains closed and no implementation task is created under any outcome) | **Proposed** |
