...
| AR-079 | 2026-07-23 | Wave 5 U1 Gate A — fulfillment operator UI module placement + branch/dependency strategy (docs-only, draft PR into `mvp/program-integration`) | Gate A planning for U1 (the fulfillment operator experience) resolves two architecture points against the accepted governance: **(module placement)** U1 UI lives **inside `shopify_connector_fulfillment`**, not a separate `_ui` addon — grounded in PD-2 (final-mvp-module-and-dependency-architecture §1: "no separate UI module; views live in owning modules"), DEC-016(A) (single shared surface; domain modules contribute, never fork), and the RA-011/012/013 rejections (monolith / micro-module / core-abstraction duplication); PD-7 keeps fulfillment on standard Odoo views (Owl only for dashboard/setup/matching/diff). A mode-switch confirmation is a small display-and-delegate `TransientModel` wizard (U0 cancel/mutation-wizard precedent), not UI-owned business logic. **(branch/dependency)** U1 implementation should branch from the integration tip **after** PR #189 merges (Option A), not stack on the Wave 4 branch — U1 cannot install/runtime-test without Wave 4 code, review independence + clean history are DEC-040 priorities, and #189 may still receive one consolidated correction. | **GATE-A PLANNING DECISION — DOCS-ONLY; control-room `REVISE` (PR #194 comment `5056513213`) applied via one consolidated correction 2026-07-23; `DOCS CORRECTED — AWAITING FRESH INDEPENDENT GATE-A REVIEW`. Not self-accepted, not ready-marked, not merged.** | Reuses AR number **079** because AR-078 is reserved by the unmerged Wave 4 PR #189 (renumbered there during its base reconciliation) and would collide on merge. No `addons/**` change; every referenced model/field/action/group XML id verified at exact Wave 4 head `2d9cff02dd5459f4ec7afee33c84fec5d00b0b8a`. **Correction (2026-07-23, comment `5056513213`):** D-P0-2 resolved **SEC-2-first** (binding; **no** parallel four-internal-group path; customer-facing UI **visibility** = the two SEC-2 roles Connector User/Connector Administrator, server authorization = the four internal groups); D-P0-1 Option A accepted; D-P1-1..D-P1-6 accepted (module in `shopify_connector_fulfillment`; mode-switch wizard frozen **display-and-delegate only**; browser/render evidence **required before U1 merge**); the Odoo **package-import allowlist** corrected (addon root `__init__.py` imports `wizards`; `models/__init__.py` must **not** import the sibling wizards package); a **Gate-A prerequisite/status table** added (package `README.md` §4); product docs `fulfillment-operating-modes.md`/`shopify-fulfillment-status-model.md` annotated for superseded vocabulary (still **Proposed**). **D-P0-3 remains the one open P0** (load-bearing Proposed product/UX contracts need independent acceptance; Wave-5 gates G5-1…G5-9 all unchecked). **Status-layer synthesis reset (2026-07-23, control-room ruling `5058042330`; independent review `5057796514`):** a confirmed material-P2 in UX/IA §8 (the code fields `display_status_*` were mislabelled A5 `FulfillmentEventStatus` when they are A7 `FulfillmentDisplayStatus`, and a phantom A2 `FulfillmentOrderStatus` badge was asserted with no backing field) was re-derived from the exact Wave 4 source and reset into one **canonical status-source & badge matrix** (`u1-backend-ui-contract-inventory.md` §12): A7 = `display_status_*` (display-only, never a carrier milestone); A5 only via `delivered_inconsistency` + parsed `tracking_snapshot`; **A2 DEFERRED — no backing read seam, no badge**; A4 = `fulfillment_status_*`; layers never merged; acceptance A22 verifies per-layer correctness; the six independent-review Tier-3 items were normalized in the same docs-only commit. `STATUS-LAYER SYNTHESIS RESET COMPLETE — AWAITING FRESH INDEPENDENT REVIEW`; no `addons/**` change; not self-accepted/ready-marked/merged. Full package: `docs/07-implementation-plan/wave-5-u1-gate-a/**`. |
