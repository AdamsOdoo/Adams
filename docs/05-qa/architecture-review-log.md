...
| AR-050 | 2026-07-15 | Task LC-1 — accepted DEC-030 lifecycle implementation | Implement the already-accepted disable-first plus supported-domain-uninstall posture: permanent historic job type, set-once original type, audited cancellation of non-terminal rows, terminal retyping without log loss, domain selection-removal callables, additive upgrade backfill, and fail-closed historic dispatch | **Implementation follows accepted DEC-030; runtime review pending** | This is the smallest implementation of the locked lifecycle design and keeps domain-owned bindings subject to the documented export/re-derivation posture rather than moving them into core | Exact-head Odoo.sh fresh install, inherited-base upgrade, domain uninstall-after-use, reinstall/re-derivation, full suite, and zero-residue evidence on PR #172 | Physical uninstall intentionally loses domain binding/mapping tables; rollback must not drop the additive column or delete audit/business data | Record runtime in `task-lc1-validation-results.md`; Claude reviews only at the complete Wave 1 boundary | **Implemented on draft PR #172; exact-head runtime pending** |
| AR-051 | 2026-07-15 | Task JOB-ACTIONS — generic core manual retry/cancel doors | Implement accepted D-JA-1 as a pure additive job-model extension with exact state and role gates, mandatory cancellation reason, one audit row, and no bypass; reserve only the two write-site elevations for SEC-1 | **Implementation follows accepted DEC-034 extraction; runtime review pending** | Keeps generic job controls in core without importing Wave 2 Area 6 scan/domain-trigger scope and gives SEC-1 concrete sanctioned action doors | Exact-head Odoo.sh focused role/state/audit suite plus full inherited core/product/sale suites on PR #172 | Incorrect group or state gates could bypass recovery policy; SEC-1 must add only the reserved elevation while preserving behavior | Record runtime in `task-job-actions-validation-results.md`; re-run after SEC-1; Claude reviews only at complete Wave 1 boundary | **Implemented on draft PR #172; exact-head runtime pending** |
| AR-052 | 2026-07-15 | Task SEC-1 — protected job/binding mutations and PII controls | Implement accepted D-SEC1-1..7 on the existing Wave 1 substrate, including all three product-owner completeness/runtime rulings: protect `original_job_type`; reuse the lifecycle maintenance audit carrier; validate current/target companies against `env.company`; and preserve the accepted CORE-R2 committed-state recovery transitions | **Runtime regression found; narrow product-owner correction implemented; exact-head rerun pending** | Closes the RPC/ORM mutation and PII exposure surfaces server-side without ACL duplication, a new audit table, store ownership, context flags, dispatcher redesign, or replay-policy changes | Build `34968318` at `62b2645` plus corrected-head Odoo.sh four-role/security, lifecycle, importer, retention, full-suite, leak/residue and genuine SRR-03 concurrency evidence on PR #172 | Build 34968318 exposed five missing recovery edges after rollback; correction `2b6d9d8` adds only those edges, while incorrect future sudo inventory, audit atomicity, or transition widening remains a bypass/correctness risk | Re-run exact corrected head; keep SRR-03 OPEN until all independent-connection stability proof passes; Claude reviews only at complete Wave 1 boundary | **Correction implemented at `2b6d9d8`; seven Python parses and edge/sudo/no-replay guards green; no corrected runtime pass; PR #172 remains draft** |
