...
| AR-004 | 2026-06-30 | Module boundaries (addon family) | **Proposed for ChatGPT review** (2026-07-02, AR-004 + AR-006 Decision Preparation) — layered domain-aligned addon family (`shopify_connector_core`/`product`/`sale`/`inventory`/`fulfillment` for Phase 1), strict dependency DAG, cross-cutting substrate concentrated in `core` | **Proposed for ChatGPT review** (2026-07-02 — see note below) | Odoo docs favour link modules; exact boundaries are now proposed against the accepted AR-002/003/005 baseline | See [`../03-architecture/ar004-module-boundary-decision-brief.md`](../03-architecture/ar004-module-boundary-decision-brief.md) for options considered and evidence | One-giant-module (A-MOD-1) and over-fragmentation (A-MOD-2) both evaluated and avoided; dependency coupling | RB-14 → **proposed** [`../04-decisions/DEC-008-module-boundary-strategy.md`](../04-decisions/DEC-008-module-boundary-strategy.md) (Status: Proposed for ChatGPT review) | **Proposed for ChatGPT review** |
...
| AR-006 | 2026-06-30 | Error handling & retries | **Proposed for ChatGPT review** (2026-07-02, AR-004 + AR-006 Decision Preparation) — classified retry policy (auto-retry only safe/transient classes), job-source/state/error-class taxonomy, layered idempotency (platform + connector-designed keys), recovery-first logs/audit | **Proposed for ChatGPT review** (2026-07-02 — see note below) | `ir.cron` auto-deactivates after repeated failures; Shopify returns 429/throttle and requires `@idempotent` on some mutations (2026-04) | See [`../03-architecture/ar006-error-retry-idempotency-decision-brief.md`](../03-architecture/ar006-error-retry-idempotency-decision-brief.md) for taxonomy tables and evidence | Missing retry → lost syncs; naive retry → duplicates / rate-limit storms — both evaluated and avoided via error-class taxonomy | RB-14 → **proposed** [`../04-decisions/DEC-009-error-retry-idempotency-strategy.md`](../04-decisions/DEC-009-error-retry-idempotency-strategy.md) (Status: Proposed for ChatGPT review) | **Proposed for ChatGPT review** |
