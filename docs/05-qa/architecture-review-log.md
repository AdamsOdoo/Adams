...
| AR-013 | 2026-07-03 | Master Blueprint UI/UX Screen Design Blueprint (Part D) | **Proposed via [`DEC-016`](../04-decisions/DEC-016-master-blueprint-ui-ux-screen-design.md)** — Master Blueprint Part D ([`../03-architecture/master-blueprint-ui-ux-screen-design.md`](../03-architecture/master-blueprint-ui-ux-screen-design.md)): a screen-level design blueprint converting the ten accepted DEC-012 operator flows and the Part A/B/C substrate/domain contracts into fourteen operator-facing surfaces (S1–S14) under the single-shared-surface rule (one dashboard/sync-center/error-center; DEC-008 §K rule 2 / RA-013, no per-domain clones); app-entry/navigation & information architecture; the accepted 11-step setup wizard; a domain-segmented store-settings surface over the seven Part A §B config concepts + feature flags + gateway→journal placeholder; the nine-card dashboard; the sync center over the fixed 6 sources / 10 states / 16 error classes with state-conditional retry/verify/cancel; the nine-element error center + manual-review queue; a matching/duplicate-prevention center; product (5 preview states, draft-first, destructive-write guard), customer (email-only key, no-PII fallback), order (no dedicated screen — MBQ-26; total-check guard; evidence-refresh-only), inventory (location mapping, guarded first-push, quantity-source explanation) and fulfillment (FulfillmentOrder-based, tracking write-back, location-mismatch review) screens; a conceptual four-role permissions-visibility model (no CSV/group IDs); a shared states-&-microcopy catalog and cross-screen-consistency rules; and a 12-point premium UI/UX acceptance checklist. **Proposes to partially resolve MBQ-53 only** (screen-design layer proposed complete; exact copy MBQ-22, identifiers MBQ-01/02/03/44, and surface-split MBQ-45 remain open); **decides no other open question** — MBQ-33/34/41 shown as recommendations, MBQ-60/61/62/63 shown as residuals, none decided; **adds no new error class, job source, or guard, and invents no exact Odoo/connector identifier** | **Proposed for ChatGPT review** | Synthesizes only already-accepted DEC-003 through DEC-015 content plus the accepted product/UX inputs (`ux-operator-flow.md`, `setup-ux-principles.md` P1–P12, `product-vision.md` premium quality bar); no new Tier-1 platform-fact claim introduced; no web research performed; all platform identifiers referenced are existing, previously-cited facts carried from Part A/B/C | ChatGPT review of the Part D blueprint document and DEC-016 against DEC-003–015, AR-002–012, RA-001–023, and the open-questions register — confirming the screen blueprint decides nothing beyond the proposed partial resolution of MBQ-53, invents no identifier/state/class/source, and preserves every guard | A screen spec being read as authorizing code or fixing an Odoo identifier (mitigated: proposed-names-only discipline; MBQ-01/02/03/44 routed; no-code framing repeated); a recommendation on an open row (MBQ-33/34/41/45) being read as decided (mitigated: each labelled `[Recommendation]`, decision owner preserved, screen designed to *accommodate* not decide); inventing a state/class/job source outside the fixed registries (mitigated: §14 draws only from the accepted enums; MBQ-62's fabricated seventh source explicitly forbidden; no 17th class); over-claiming MBQ-53 as fully resolved (mitigated: proposed *partially* resolved, residuals carved out) | RB-14 → proposed [`../04-decisions/DEC-016-master-blueprint-ui-ux-screen-design.md`](../04-decisions/DEC-016-master-blueprint-ui-ux-screen-design.md) (Proposed for ChatGPT review, 2026-07-03); next → ChatGPT review, then Master Blueprint Part E (implementation-planning bridge) or the implementation gate per ChatGPT | **Proposed** |
...
_**Master Blueprint Sprint D (2026-07-03) — AR-013 added, Proposed for
ChatGPT review.** Prepared on the base commit
`b6199f78064ae4e1934bccee630a14b3d7eef438` (PR #74 / DEC-015 merge into
`Shopify-connector`). AR-013 is **not** a new platform-fact/architecture-option
question in the AR-002…AR-008 sense — it is the **UI/UX Screen Design
Blueprint (Part D)** synthesis of the already-accepted DEC-003 through
DEC-015 decisions and the accepted product/UX inputs (`ux-operator-flow.md`,
`setup-ux-principles.md`, `product-vision.md`), proposed via
[`DEC-016`](../04-decisions/DEC-016-master-blueprint-ui-ux-screen-design.md).
It proposes to **partially resolve MBQ-53 only**; it decides no other open
question (MBQ-33/34/41 are shown as recommendations, MBQ-60/61/62/63 as
residuals, none decided), adds no error class / job source / guard, invents no
exact Odoo/connector identifier, and authorizes no implementation. **AR-013's
table row is `Proposed for ChatGPT review` / `Proposed`; it is NOT accepted,
and neither DEC-016 nor Part D is accepted. Part E remains not started;
implementation remains blocked.** This note will be superseded by an
acceptance patch if and when ChatGPT accepts DEC-016._
