...
| AR-011 | 2026-07-03 | Master Blueprint Product, Customer, and Sale/Order Domain Blueprint | **Proposed via [`DEC-014`](../04-decisions/DEC-014-master-blueprint-product-customer-sale.md)**, revised 2026-07-03 (PR #72 review) — Master Blueprint Part B: product-template/product-variant binding under `shopify_connector_product`; customer/order binding under `shopify_connector_sale`; product import/export/update flow (variant-mutation-strategy direction now citing both `productVariantsBulkCreate` and `productVariantsBulkUpdate` official docs, draft/publish mechanism via `Product.status`+`publishablePublish`, media/price handling per DEC-007 §2/§3, corrected §A.4 wording so an update job requires an explicit operator action, never an ordinary Odoo write alone); customer import/matching (default-customer-fallback direction, proposed email-only match-key recommendation); order import (whole-order-hold rule for unmatched product lines, two-path customer-resolution rule, total-check guard definition routed to the existing `financial total mismatch` error class, proposed order-import operator-touchpoint recommendation, gateway→journal mapping concept, generalized order-scope wording in the error-class table); the **proposed automated import create/bind policy** (pre-create duplicate check + six-condition auto-create gate, replacing the withdrawn "retrospective visibility satisfies preview" reading); cross-domain sequencing; consolidated error/retry-class mapping (no new class added to the fixed 16-class registry); open-questions register updates for MBQ-23–31 plus new rows MBQ-55–59 (MBQ-59 added in the revision) | **Proposed for ChatGPT review** (not yet accepted) | Synthesizes already-accepted DEC-003/006/007/012/013 content; three small, targeted official-doc checks performed (Shopify `productSet`/`productVariantsBulkCreate`/`productVariantsBulkUpdate`/`Product.status`/`publishablePublish` reference pages, accessed 2026-07-03; Odoo 19 accounting/taxes documentation, accessed 2026-07-03, inconclusive) — no broad research performed, per the sprint's "targeted checks only" instruction | ChatGPT/Fable review of the Part B blueprint document against DEC-003/006/007/012/013 and RA-001–023 — ChatGPT requested revision before Fable review (this row); Fable review not yet performed | Blueprint detail being read as code-level commitment (mitigated: proposed-names-only discipline continued from Part A, MBQ-01/02/55 unresolved); **the original draft read retrospective sync-center/dashboard visibility as satisfying the "no blind create"/preview requirement for confident automated product/customer creates — ChatGPT flagged this as weakening DEC-003/006's guard (audit is not preview); mitigated in this revision by replacing that reading with an explicit pre-create duplicate check plus a six-condition auto-create gate (§A.2/§A.9/§B.2/§B.9), tracked as new open question MBQ-59, labelled `[Blueprint proposal, pending DEC-014]` throughout — interactive/batch create-bind/write is unaffected and still requires a blocking preview**; §A.4 read as implying autonomous Odoo-write-triggered Shopify pushes (mitigated: corrected to require an explicit operator action or a later accepted controlled trigger); an unverified `read_all_orders` scope claim in the error-class table (mitigated: generalized to "missing required order read scope / protected-customer-data approval," already covered by MBQ-06/MBQ-09); MBQ-26/MBQ-31 recommendations being mistaken for already-decided outcomes (mitigated: both explicitly labelled "recommendation to ChatGPT" throughout, both name their register decision owner as ChatGPT (Sprint B)) | RB-14 → **proposed** [`../04-decisions/DEC-014-master-blueprint-product-customer-sale.md`](../04-decisions/DEC-014-master-blueprint-product-customer-sale.md) (Proposed for ChatGPT review, 2026-07-03; revised 2026-07-03 per PR #72 review); next, pending acceptance → Master Blueprint Sprint C (inventory/fulfillment domain blueprint), not started | **Proposed** |
