...
| AR-040 | 2026-07-10 | Task 011 customer import/matching implementation — draft PR opened, not yet reviewed | After PR #144 merged into `Shopify-connector` (merge commit `8b364aa360cb596dd584bbc8345b790cc7ad20ed`) and ChatGPT performed the explicit customer-domain gate-opening act (GitHub comment ID `4934249603`, PR #144) naming this exact prompt, this session verified the base placeholder against the actual current tip (no drift) and implements the `shopify_connector_sale` addon per the final implementation prompt (branch `claude/task-011-shopify-connector-solrrp`, opened as draft): one concrete binding model (`shopify.connector.customer.binding`), declaring explicit `_name`/`_inherit` per the accepted MBQ-55 schema; the inert `customer_fallback_partner_id` store-settings field (D5, Posture A — defined, zero consumption, confirmed by source-level grep and a byte-identical-outcome behavioral test); a read-only `shopify.connector.customer.importer` `AbstractModel` implementing the final prompt §8.1 match-key sequence (existing binding → recall-safe email match, with candidate discovery via the always-safe `[('email', '!=', False)]` baseline plus mandatory Python-side `email_normalize()` comparison on both sides, applied identically to the active and archived-inclusive searches — the critical D1 rule this task's own instructions singled out — → exactly-one-active-candidate bind guarded against `binding_conflict` → archived-match check routed to `duplicate_risk` → confident no-match create, gated → ambiguous/missing-email never create, routed to `blocked_manual_review` via the existing, unmodified `JobHandlerError`/`_route_failure()` mechanism, with the exact §8.2 JSON candidate-evidence payload carried in the existing `job.log.technical_detail` field); the three required extension seams (`job_type` `selection_add` for `customer_import_sync`, a `_domain_flag_for_job_type()` override mapping it to `sale_domain_enabled`, a `_get_handlers()` override), all declared inside `shopify_connector_customer_importer.py` only — zero edits to any `shopify_connector_core` or `shopify_connector_product` file (confirmed by `git diff --stat`); `ir.model.access.csv` only, reusing the four existing groups, no new group/XML; four required test files covering every case named in final prompt §10, including the recall-safety wrapped/display-name/mixed-case email tests and their duplicate-prevention proofs. Four narrow in-task decisions were made and are recorded in [`../05-qa/task-011-customer-import-validation-results.md`](./task-011-customer-import-validation-results.md) §C: (1) `res_model`/`res_id` targeting not implemented by the dispatcher handler, mirroring Task 010's own exact precedent; (2) the always-safe `[('email', '!=', False)]` baseline chosen over the permitted `ilike`-prefilter alternative for candidate discovery; (3) partner `name` mapped from the incoming `displayName`; (4) the final prompt's own "informational job-log line" for an unresolved country/state is not implemented, since `_apply_import(store, payload)` mirrors Task 010's established signature convention with no `job` parameter to append one through — both actual behavioral guarantees (empty field, no failure) are met without it | **Not yet reviewed — draft PR opened, awaiting ChatGPT's first review pass.** No live Odoo/PostgreSQL runtime was reachable in this session (no `odoo` package installed, no Odoo.sh/CI access) — every check is static only (`py_compile`, `pyflakes`, a proactive local `docutils` RST scan of every docstring/manifest field with zero warnings found, source-level greps for zero-mutation/zero-bypass/zero-forbidden-model/zero-fallback-field-read, and a regex check that the query string carries only the pinned non-deprecated field list), full detail in the validation-results doc §D; the four new test files are written and statically valid but **not execution-proven**. Per final prompt §12, a live Odoo.sh branch-database run is mandatory before merge and has not been obtained | This is an implementation-closure-style entry recording what was built, mirroring AR-036's Task-010-closure pattern, for a **draft, not-yet-reviewed** PR | No live Odoo/PostgreSQL runtime was reachable in this session; static validation only, full detail in the validation-results doc §D; ChatGPT's own review of the PR diff and of the four in-task decisions above is required, followed by the mandatory live Odoo.sh run before merge | Runtime execution proof for the four new test files remains outstanding; the four in-task decisions in this row could be found in need of revision on ChatGPT's review; VAL-B2, MBQ-05 (both branches), MBQ-55's order portion, TD-002, the fulfillment API model, Lite/Full packaging, and the multi-server concurrency proof requirement (SRR-03/04/09) all remain exactly as open as before this task — none is touched, resolved, or narrowed; no Task 012/013/014, Task 015, UI, webhook, OAuth, or Lite/Full packaging work is authorized or started by this task | ChatGPT reviews the draft PR against final prompt §3–§14 and this row's own evidence, then arranges the mandatory live Odoo.sh run before any merge; the PR must not be marked ready for review or merged without that review — the customer-domain implementation gate closes the moment this PR was opened as draft (`customer-domain-gate-criteria-proposal.md` §4; final prompt §14), so no further customer-domain work may start regardless of this PR's own outcome | **Proposed for ChatGPT review** |
