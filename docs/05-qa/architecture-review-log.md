...
| AR-044 | 2026-07-12 | Task CORE-R2 — disconnect quiescence & in-flight job contract (design only; draft PR into `Shopify-connector`) | After PR #153 merged (accepted evidence, `4950408383`; SRR-03 kept OPEN, DEF-PB-1 confirmed) and ChatGPT's CORE-R2 **design** gate act (PR #153 comment `4950413650`, docs-only, one session, verified base `fcbbb0b3fe3db9cba354a8a1c08e91036b70ec1f`), this session designs the DEF-PB-1/SRR-03 remediation and prepares a future implementation packet — **no code**. Recommends **Option E (hybrid)**: explicit `disconnecting` store state + monotonic `disconnect_generation` token (Option A), a cooperative **fresh-transaction pre-side-effect checkpoint** that defeats the REPEATABLE READ snapshot-blindness of checkpoint-3 (Option C), and store-scoped coordination via a **transaction-scoped PostgreSQL advisory lock** (`pg_advisory_xact_lock`) inside a **two-phase disconnect** (fast intent-commit → dispatcher-run quiesce → credential-clear/finalize) with a bounded **timeout + operator escalation** (Option B). Rejects Option C-only, force-`FOR UPDATE`-cancel, global lock, session advisory lock, in-method `cr.commit()`, and per-job-commit-alone (Option D deferred to PERF-1). Source-verified: REPEATABLE READ (`odoo/sql_db.py:373`), `retrying()` (`odoo/service/model.py:160`), `try_lock_for_update`/`lock_for_update` non-blocking `FOR UPDATE SKIP LOCKED` (`odoo/orm/models.py:5564`/`5592`), cron single-commit boundary (`odoo/addons/base/models/ir_cron.py:671`) + `_commit_progress` (`:846`). Deliverables: [`../03-architecture/disconnect-quiescence-remediation-analysis.md`](../03-architecture/disconnect-quiescence-remediation-analysis.md) (26 sections) and [`../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md`](../07-implementation-plan/task-core-r2-disconnect-quiescence-packet.md); SRR-03/04/09 wording, master-plan CORE-R2 dependency, and research-handoff top entry updated. | **Proposed for ChatGPT review — design only; no gate opened.** The analysis §26 enumerates six calls (D-CR2-A…F): the target contract + invariants, Option E vs alternatives, the coordination primitive, the new state/fields/`job_type` + handler seam timing, critical-path placement, and confirmation SRR-03 stays OPEN. Accepting the PR without named exceptions accepts these as proposals; the CORE-R2 **implementation** gate is a **separate, later** explicit act on the packet. | Gives the DEF-PB-1/SRR-03 remediation a written, source-grounded contract and a narrow future packet so, once gated, implementation is fast and defensible — and so no live-Shopify-mutation task ships on top of an unremediated disconnect race | The runtime evidence (PR #153 §10/§14 + `scenario-06-real-timeline.md`); the Odoo 19 / PostgreSQL 16 source citations in the analysis; **live Odoo.sh + genuine two-server proof are required by the packet before SRR-03 can move off OPEN** — none claimed by this design session | Residual races stated honestly (RR-1 check→call window; RR-2 already-issued call; RR-3 timeout escalation; RR-4 multi-host; RR-5 advisory-lock hygiene; RR-6 SRR-06 concurrency-fix-needs-live-proof). No addon/test/code changed; six-file docs-only scope; SRR-03 OPEN, SRR-04/09 REDUCED-not-closed unchanged | ChatGPT reviews the draft PR, makes the six §26 calls, and — if accepted — later performs a **separate** CORE-R2 implementation-gate act (naming the then-current base SHA and re-freezing the packet's allowed-file list) in a new session; no addon code is authorized by this review | **Proposed for ChatGPT review — design/packet only; implementation gate CLOSED** |
