...
| AR-043 | 2026-07-11 | Task CORE-R1 — capability-aware readiness correction: implementation (draft PR into `Shopify-connector`) | After PR #148 merged into `Shopify-connector` (merge commit / verified base SHA `2bdb07cf33045a696311c420e577cbbb09cdfb38`) and ChatGPT performed the explicit CORE-R1 gate-opening act (GitHub comment ID `4948225529`, "OPEN for one implementation session only"), this session implements Task CORE-R1 exactly per [`../07-implementation-plan/task-core-r1-readiness-correction-packet.md`](../07-implementation-plan/task-core-r1-readiness-correction-packet.md) (D-R1-1..5), touching only the packet's exhaustive allowlist: **D-R1-1** — `_check_cron_queue_health` becomes a real, capability-aware check (drain cron exists + active, read via the ONE new named read-only `sudo()` helper `_drain_cron_active_state`; plus no queued job stalled > `READINESS_QUEUE_STALL_MINUTES=60` using the exact discriminator `state='queued' AND NOT started_at`, `create_date`-aged; named fail reasons; no Area-6 scan-cron requirement; re-queued-with-history boundary honored); **D-R1-2** — `_check_mapped_location` not-applicable pass when `inventory_domain_enabled` is False, else fail-closed `not_proven`, reading only the same core settings flag (no inventory-model dependency); **D-R1-3** — `_check_webhook_hmac` not-applicable pass with the packet's exact reason string; **D-R1-5** — `action_test_connection` writes `api_health_state='normal'` on full non-fall-forward success only (the `degraded` fall-forward path untouched); **D-R1-4** — new `tests/test_readiness_slot_closure.py` (16-case suite) proves via real behavior that an eligible Lite store aggregates readiness `pass` and reaches `connected` through `action_activate()`, and that a genuine essential failure still blocks activation; plus the one-line `tests/__init__.py` import. Validation record: [`./task-core-r1-validation-results.md`](./task-core-r1-validation-results.md) | **Draft PR opened for ChatGPT review** — the CORE-R1 gate closes automatically on draft-PR-open; nothing accepted/merged; **one flagged blocker surfaced, not silently resolved**: the D-R1-1 sudo raises the module-directory `.sudo()` count from two to three, so the two pre-existing "exactly two sudo sites" AST guards (`test_job_log_system_append.py::test_source_level_two_sudo_sites_total`, `test_credential_service.py::test_source_level_sanctioned_sudo_sites_guard`) go stale/red — both are OUTSIDE the packet's exhaustive allowlist and were deliberately left untouched (mirroring the Task 003 precedent), pending explicit ChatGPT authorization for the trivial 2→3 update | Implements the accepted packet's readiness correction so a store can finally reach `connected` (the red-team BLOCKER in `mvp-planning-completion-audit.md` §7.1 item 1), unblocking dev-store validation for Task 010B and every later live validation | Local static validation only (no Odoo runtime in this environment): `python3 -m py_compile` on all changed Python files (clean); standalone AST replication of every source-level guard (the three new guards pass; `_check_*`-mutation guard stays green; the two count-guards confirmed 2→3). **Live Odoo.sh run pending** — runs on the platform build the draft PR triggers; must be quoted verbatim before merge (OP-43), and will show exactly the two flagged count-guards failing until the authorized 2→3 update lands | The DoD is internally unsatisfiable as written for D-R1-1 (DoD "only allowed files changed" vs "all pre-existing core tests remain green"): the mandated third sudo site cannot coexist with the two forbidden count-guards' "exactly two" assertion. Resolution deferred to ChatGPT (expand allowlist for the 2→3 guard update, or accept the flagged-stale posture, or amend the packet). The sudo is isolated in a named helper so only those two guards — not the `_check_*`-mutation guard — are affected; blast radius minimized | ChatGPT reviews the draft PR against the packet (D-R1-1..5) and the validation record, rules on the flagged 2→3 sudo-guard reconciliation, then (with the Odoo.sh run green) performs the final merge review; no Task 010B/011B/012/LC-1/Area-6/SEC-1/UI/webhook/PERF-1 work is authorized or started by this task | **Proposed for ChatGPT review** |
