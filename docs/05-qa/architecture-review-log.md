...
| AR-019 | 2026-07-05 | Core naming and schema implementation-planning pass | **Proposed via [`../07-implementation-plan/core-naming-schema-planning.md`](../07-implementation-plan/core-naming-schema-planning.md)** — the documentation-only naming/core-schema implementation-planning artifact the accepted implementation gate readiness audit (AR-018) named as the next session, prepared after PR #84 merged into `Shopify-connector` at merge commit `4bf692dceec4190705f522bc2d32851af4c79e37`. Proposes exact Odoo model names for a first core-only slice (`shopify.connector.store`, `.store.settings`, `.store.credential`, `.location`, `.binding.mixin` (abstract), `.job`, `.job.log`) and their field schemas; a store-scoped `store.settings` model as the feature-flag/settings shape; a **job+log split** with error/manual-review fields folded onto the job model (no separate error or idempotency-guard model); the operation-level idempotency key schema and a query-time serialization guard, both owned by the job model; retry-count ceilings and a backoff schedule by error-class family; four proposed group XML IDs (`group_shopify_connector_admin`/`_operator`/`_reviewer`/`_auditor`) plus planned (not created) `ir.model.access.csv` row shapes; and the `odoo_event` job-source/`trigger_origin` field mechanics (model: `shopify.connector.job`; field: `trigger_origin`; values `inventory_stock_change`/`fulfillment_picking_validation`; required only when `job_source = odoo_event`). For **MBQ-04**, explicitly adopts **Option A (slice-1 descope)** — no official Odoo encryption-at-rest evidence was reviewed this session, so a `store.credential` model is proposed for lifecycle metadata only, with no field, type, or mechanism proposed for the actual secret/token value; real credential persistence remains open. **This document does not authorize implementation, does not open the implementation gate, and does not create any implementation task or code, module, view, controller, security file, manifest, test, or CI file.** Draft MBQ register-impact wording (§14 of the document) proposes **MBQ-01, MBQ-02, MBQ-07, MBQ-16, MBQ-19, MBQ-20, MBQ-21, and the MBQ-45/MBQ-62 residuals as "Proposed resolved pending ChatGPT acceptance"**; **MBQ-04 and MBQ-44 as "Proposed partially resolved pending ChatGPT acceptance"** (MBQ-04 by descope only, not resolved for real credential persistence; MBQ-44 for planned CSV row shapes only, no CSV file created). **No MBQ row in `master-blueprint-open-questions.md` is edited by this proposal itself** — the draft wording is applied only by a future acceptance patch, if and when ChatGPT accepts. | Pending ChatGPT review | Converts the already-accepted Part A core-substrate blueprint (DEC-013) — which deliberately left every model/field name a "proposed direction, not a committed identifier" — into the exact names/schema the accepted audit found blocking even a core-only first slice; no new architecture decision is introduced, only naming/schema detail inside boundaries DEC-003 through DEC-020 already fixed | ChatGPT review/acceptance of this planning document; no external Shopify/Odoo research was performed or is required for this internal naming/schema synthesis | A proposed schema being mistaken for an accepted one before ChatGPT review (mitigated: Status section and every MBQ-impact line state "pending ChatGPT acceptance" throughout); the job+log split, the generic `enqueue_decisions` JSON field, and the pre-MBQ-04 `store.credential` model being read as settled rather than flagged judgment calls (mitigated: §16 of the document names all three explicitly as candidates for "accept with changes"); this proposal being mistaken for a gate-opening act (mitigated: Status section and §16 both state explicitly that no gate criterion is satisfied by this document and that opening the gate is not recommended) | If accepted: apply the draft §14 register-impact wording to `master-blueprint-open-questions.md` for MBQ-01/02/04/07/16/19/20/21/44/45(residual)/62(residual), record the acceptance in this log and in `../04-decisions/README.md`, and update `research-handoff.md`; not implementation — the next session after acceptance would still require the separate MBQ rows this pass does not touch (MBQ-03/05/09/14/23–43/46/48–61/63–65) and, ultimately, a separate, explicit gate-opening act. If revised or rejected: rework per ChatGPT's specific feedback before any further session builds on this schema | Proposed for ChatGPT review — NOT YET ACCEPTED |
