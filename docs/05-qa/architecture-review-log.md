...
| AR-027 | 2026-07-07 | Task 003 decision closure | Close the four Task 003-specific decision points AR-024 left open, via [`../07-implementation-plan/task-003-decision-closure.md`](../07-implementation-plan/task-003-decision-closure.md) (job-type value: **recommended** `core_test_connection`, added directly to the base selection, amending AR-019's fixed-at-two-values statement to three; `SHOP_INACTIVE`/402/423/403-fraudulent error-class mapping: **recommended** `shopify_permission_scope_auth` for all four — the underlying HTTP-status/error-code facts are already cited in `credential-connection-api-client-planning.md`, but the mapping choice and several named behavioral shapes remain flagged `[Requires external validation before implementation]` — with `credential_state` flipped to `invalid` only for a genuine token-invalid signal, not a shop-account-state condition; job-log write path: **recommended** a single sanctioned internal `sudo()`-wrapped system-append method, no ACL widening, no security-file change; per-run `payload_hash` nonce: **recommended** a UUID4 per target-less job, no schema change, also surfacing a pre-existing latent idempotency-collision defect in the already-merged `core_readiness_check` job type), plus a companion review checklist ([`./task-003-pre-implementation-review-checklist.md`](./task-003-pre-implementation-review-checklist.md)) and a dated amendment note on [`../07-implementation-plan/task-003-api-client-test-connection-proposed.md`](../07-implementation-plan/task-003-api-client-test-connection-proposed.md). **No final Task 003 implementation prompt and no gate-opening proposal are included** — unlike the AR-025 round, this package is decisions-only, deferring the final prompt/gate pairing to a later, separate session. Session constraint: no external network access; every platform statement reuses an already-cited repo document or is explicitly labelled as requiring future external validation — no new official-source research was performed or claimed. | **Proposed for ChatGPT review — not yet accepted** | AR-024 accepted the credential/API-client foundation plan but left these four Task-003-specific decision points open; AR-025 closed only the three Task-002-specific points and explicitly deferred these four | Direct inspection of `shopify_connector_job.py`/`shopify_connector_job_log.py`/`ir.model.access.csv` (confirmed current schema/ACL state, no Task 003 code exists yet); reused citations from `credential-connection-api-client-planning.md` (accessed 2026-07-06) and `DEC-009`; `rejected-approaches-log.md` (RA-001–RA-023) checked — no conflict | Recommending a mapping that later official verification contradicts (mitigated: the mapping choice and named behavioral shapes are labelled `[Requires external validation before implementation]`, not asserted as fact); the `core_readiness_check` collision defect being lost (mitigated: recorded in the decision-closure document, this row, the QA checklist, and the handoff); a future session mistaking this package for gate authorization (mitigated: Status sections in all three documents state no gate is opened and no final prompt exists) | ChatGPT review of the four recommendations; if accepted, a separate future session prepares the final Task 003 `CLAUDE.md` §9 prompt and a narrow gate-opening proposal (mirroring AR-025 → AR-026) — **not performed by this entry** | **Proposed** |
