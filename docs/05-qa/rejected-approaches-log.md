...
| RA-024 | 2026-07-07 | mapping `SHOP_INACTIVE`/HTTP 402 (frozen shop)/423 (locked shop)/403 (fraudulent-store) test-connection failures to the `shopify_user_errors_validation` error class (Task 003 error-class mapping decision) | `shopify_user_errors_validation` is a GraphQL **mutation**-`userErrors`-shaped concept; the test-connection operation is a structurally read-only query with no mutation and no `userErrors` surface to populate, making this class a poor structural fit regardless of retry semantics — both candidate classes share identical DEC-009 retry treatment ("no automatic retry, manual fix then retry"), so the query-vs-mutation structural mismatch is the deciding factor | Evaluated in `../07-implementation-plan/task-003-decision-closure.md` Decision 2, alongside the accepted alternative (`shopify_permission_scope_auth`); the four HTTP-status/error-code facts themselves are cited in `../03-architecture/credential-connection-api-client-planning.md:257-273` (shopify.dev, accessed 2026-07-06) | A future generic job-framework revision adding a dedicated read-query-failure error class distinct from both `shopify_permission_scope_auth` and `shopify_user_errors_validation` — no such class exists in the accepted fixed-16 registry today; none anticipated | [`../07-implementation-plan/task-003-decision-closure.md`](../07-implementation-plan/task-003-decision-closure.md) (Status: Accepted by ChatGPT, 2026-07-07, via AR-027) |
