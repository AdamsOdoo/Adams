...
RA-006 added, explicitly marked PROPOSED.** Unlike RA-001 (added only **after** ChatGPT accepted
DEC-003), RA-002–RA-006 are added **alongside** their DEC-004/005/006 proposals, per this sprint's
explicit instruction to log rejected/deferred approaches "if the proposed decisions explicitly
reject them." Each row is tagged **PROPOSED** and its "Related decision record" cites the DEC
file's own `Status: Proposed for ChatGPT review` — **none of these five is a final rejection**;
if ChatGPT does not accept the corresponding DEC file, the row must be revisited (not silently
treated as settled) rather than staying in this log as if finalized. This preserves the log's
core rule (`CLAUDE.md` §10: an approach is rejected only after evaluation and review) while
complying with this sprint's request to surface the candidate rejections for that same review.
No approach outside the five tied to DEC-004/005/006 was evaluated to rejection this sprint;
DEC-003's non-MVP deferrals are not re-logged here (see the Sprint G note above)._

_**PR #60 minor revision (2026-07-02, ChatGPT + Fable review — ACCEPT WITH MINOR CHANGES) —
RA-007 added, same PROPOSED framing; governance clarifier added.** Fable flagged a dangling
pointer: DEC-005's rejected-options table cited this log for AR-003 Option 4 (external worker)
with no corresponding row. Added **RA-007** (external worker as the Phase 1 sync-orchestration
substrate), tagged **PROPOSED**, same pattern as RA-002–RA-006. **Governance clarifier (applies
to RA-002 through RA-007):** for RA-002 through RA-007, the `CLAUDE.md` §10 do-not-re-propose bar
attaches **only if and when ChatGPT accepts the linked DEC record**; until then these rows are
**candidate rejections under review, not binding final rejections**. No new approach was
evaluated to rejection beyond RA-007; DEC-004/005/006 remain `Proposed for ChatGPT review`, not
accepted; no implementation authorized._
