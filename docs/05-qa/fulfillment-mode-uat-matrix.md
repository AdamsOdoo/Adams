...
| UAT-FM-3.3 | Rollback | Switch Mode 2→1 while a Mode 2 evaluation is in flight | Always allowed; future auto-application stops; evidence records, applied reconciliations, bindings, audit untouched; in-flight evaluations cancelled back to `review` — no state corruption (TD-003: the code value is `review`; `under_review` was never a real selection value — see [`connector-vocabulary-reconciliation.md`](../06-prompts/connector-vocabulary-reconciliation.md)) |
