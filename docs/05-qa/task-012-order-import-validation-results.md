...
**CAMPAIGN 4 EXACT-HEAD RUNTIME ACCEPTED — GREEN — READY FOR FINAL WAVE 2 MERGE REVIEW (2026-07-18, build `35100725`, control-room ruling [`5012365516`](https://github.com/AdamsOdoo/Adams/pull/176#issuecomment-5012365516)).** Campaign 4 is **GREEN** and its runtime evidence is **accepted by the control room**. **Runtime-tested SHA:** `63607dd87a8bfc253ee60ed00e0d761ee62c8776` (the repository-wide country-consistent tax-fixture correction completing `test_order_totals_guard.py._map_tax()`, control-room-authorized in [`5011632937`](https://github.com/AdamsOdoo/Adams/pull/176#issuecomment-5011632937)). **Accepted documentation-only evidence commit** (exactly one commit after the tested SHA; changes only the new Campaign-4 evidence file, this file, and the tracker): `3cb259d40ca3a04a0aed5781942dfff792e7ede3`. Do not confuse the two: the evidence commit itself was never runtime-tested. Environment: Odoo.sh build `35100725`, database `adamsmen-sol-wave-2-order-import-35100725`, Odoo **19.0**, PostgreSQL **16.14**. **The authoritative clean/full fresh install with tests enabled is GREEN:** `0 failed, 0 error(s), 0 skipped of 728 tests` (core `414` / product `202` / sale `232`, all `0/0`). **All three Campaign-3 `TestOrderTotalsGuard` tax-fixture errors are CLOSED** (finding #5 now fully closed in both `_tax()` and `_map_tax()`). The complete **86/86** Wave 2 authored method inventory executed with no silent exclusion, skip or removal. **All eleven original findings PASS.** Genuine independent-connection concurrency (`TestOrderDiscoveryConcurrencyGenuine`) passed **3/3**. ACL, protected-field, SRR-03, residue, resource and credential/PII/redaction checks are all **green**. **No production code changed, no test changed, and no Shopify mutation occurred during the runtime session.** Isolated baseline-upgrade/lifecycle databases (Environments B/C) remain **deferred release-readiness evidence under the previously accepted policy** (ruling [`5010851668`](https://github.com/AdamsOdoo/Adams/pull/176#issuecomment-5010851668)); read-only dev-store evidence remains **deferred to Wave 6**; neither is a Wave 2 blocker. Full Campaign-4 evidence: [`task-012-campaign-4-exact-head-runtime-evidence.md`](task-012-campaign-4-exact-head-runtime-evidence.md); see also "Campaign 4 (exact head `63607dd`) — GREEN" near the end of this document. **Recommendation: READY FOR FINAL WAVE 2 MERGE REVIEW.** PR #176 remains **open, draft and unmerged**; this closure session performed no merge, no ready-marking, no Odoo rerun, and no Shopify mutation; SRR-03 remains CLOSED; **Wave 3 remains unstarted.** This entry supersedes the Campaign-3 entry immediately below (retained verbatim as historical evidence, together with all earlier campaigns).

**CORRECTED-HEAD RUNTIME CAMPAIGN 3 EXECUTED — CORRECTION REQUIRED (2026-07-18, build `35095228`, under revised control-room ruling [`5010851668`](https://github.com/AdamsOdoo/Adams/pull/176#issuecomment-5010851668), HISTORICAL — SUPERSEDED BY CAMPAIGN 4 ABOVE, WHICH CLOSED THE `test_order_totals_guard.py` DEFECT AND RECORDED A GREEN CLEAN/FULL FRESH INSTALL).** The sole authorized runtime candidate `2525447cee2d8a3371b1f4e669f61bcd50b20162` (the documentation head that reconciles fixture correction `6f32e4c8a2e6eac44bfb32e2cca0ea2bea3b1ea4`) was independently runtime-validated on authenticated Odoo.sh build `35095228` (DB `adamsmen-sol-wave-2-order-import-35095228`, Odoo 19.0, PostgreSQL 16.14). **The clean/full fresh install with tests enabled is NOT green:** the build's own install-with-tests (`install.log`) recorded `0 failed, 3 error(s) of 728 tests` — all three in `TestOrderTotalsGuard` (`test_order_and_source_tax_fingerprints_must_reconcile`; and the parametrised `test_tax_excluded_and_tax_included_orders_use_mapped_engine_taxes` sub-cases `included=False` and `included=True`), each an `account_tax.tax_group_id` (and `country_id`) NOT-NULL violation raised by the test's own `_map_tax()` helper (`shopify_connector_sale/tests/test_order_totals_guard.py:34-45`), which creates `account.tax` without an explicit country-consistent tax group. **This is the identical country-consistent-tax-fixture defect class that correction `6f32e4c` closed in `test_order_tax_resolution.py` but did NOT apply to the sibling `test_order_totals_guard.py`** (that commit changed only `test_order_tax_resolution.py`, +47/-3). `TestOrderTaxResolution` now passes at fresh install — finding #5 is closed there — but finding #5 (Odoo 19 country-consistent tax fixture) is **NOT fully closed**: it is OPEN in `test_order_totals_guard.py`. Under the revised ruling, isolated baseline-upgrade (B) and uninstall/reinstall lifecycle (C) databases are **no longer Wave 2 blockers** and are recorded as **deferred release-readiness evidence — NOT ENVIRONMENT BLOCKED**; the Wave 2 acceptance gate is now the complete authenticated Odoo.sh clean/full matrix, which is not green. Genuine independent-connection concurrency passed **3/3**; residue, security/redaction and registry checks are clean; the warm `-u` reruns surfaced only base-`account` `res_partner.autopost_bills` NOT-NULL artifacts that are **absent from the authoritative fresh install** and are not connector-attributable. **Recommendation: CORRECTION REQUIRED** — Sol test-fixture scope: apply the `6f32e4c` country-consistent tax + tax-group pattern to `test_order_totals_guard.py._map_tax` (and audit every remaining fixture that creates `account.tax` without an explicit `country_id`/`tax_group_id`). No production code and no test was changed by this audit. Runtime-tested SHA `2525447cee2d8a3371b1f4e669f61bcd50b20162`; documentation-only evidence commit recorded in "Corrected-head runtime validation campaign 3 — 2026-07-18 (build `35095228`)" below and in the session handoff. PR #176 remains open, draft and unmerged; SRR-03 remains CLOSED; Wave 3 remains unstarted.
