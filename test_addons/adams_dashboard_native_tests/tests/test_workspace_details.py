...
class TestDashboardWorkspaceDetails(AccountTestInvoicingCommon):
    ...
    def test_crm_pagination_and_filter_shrink(self):
        ...
        compact = self.dashboard.get_workspace_details(self.options, 'crm', 24, 4)
        self.assertEqual(compact['page_size'], 4)
        self.assertEqual(len(compact['rows']), 3)
        self.assertEqual(compact['total'], 27)
        self.assertFalse(compact['has_more'])
        leads[1:].active = False
        shrunk = self.dashboard.get_workspace_details(self.options, 'crm', 25)
        compact_shrunk = self.dashboard.get_workspace_details(self.options, 'crm', 24, 4)
        self.assertEqual(compact_shrunk['offset'], 0)
        self.assertEqual(compact_shrunk['total'], 1)
