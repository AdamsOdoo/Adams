...
@tagged('shopify_connector_pro', 'post_install', '-at_install')
class TestManagerDashboardAggregator(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # Seed refund and cart data in setUpClass so _read_group with sudo()
        # always sees them (avoids flush-timing issues with per-method savepoints).
        order_binding_a = cls.OrderBinding.search(
            [('backend_id', '=', cls.backend_a.id)], limit=1,
        )
        cls.Refund.create({
            'backend_id': cls.backend_a.id,
            'shopify_id': 'r-1',
            'refund_amount': 50.0,
            'currency_code': 'USD',
    ...
    def test_refunds_and_carts_isolation(self):
        ...
        # Refund and cart data seeded in setUpClass on backend A.
        data = self.Dashboard.get_data(backend_ids=[self.backend_a.id], period='mtd')
