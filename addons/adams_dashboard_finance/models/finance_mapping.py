...
METRICS = [
    ...
    ('standard_forecast', 'short-term cash forecast'),
...
class FinanceMapping(models.Model):
    ...
    denominator_expression_id = fields.Many2one('account.report.expression', ondelete='restrict',
        help='denominator result used only to detect an undefined ratio; the engine calculates the percentage.')
    ...
    definition_note = fields.Text(required=True, help='Explain the chosen definition, variant, currency and reporting policy.')
    ...
    def _check_definition(self):
        for mapping in self:
            ...
            if mapping.metric == 'standard_forecast' and mapping.expression_id != self.env.ref(
                ...
                raise ValidationError(_('Select the Executive Summary short-term cash forecast expression.'))
            if mapping.budget_id and (mapping.metric not in BUDGET_KEYS or not mapping.report_id.filter_budgets
                ...
                raise ValidationError(_('Select a same-company budget for a supported profitability report.'))
            if mapping.metric in RATIO_KEYS and (
                ...
                raise ValidationError(_('Select the denominator expression for this ratio.'))
            ...
            if not columns or figure_types != {expected}:
                raise ValidationError(_('Choose a report column with the correct monetary or percentage unit.'))
            if mapping.cash_detail_report_id or mapping.cash_detail_expression_id:
                ...
                if (mapping.metric != 'cash' or not detail
                    ...
                    raise ValidationError(_('Cash detail requires the General Ledger balance expression.'))
            if mapping.cash_flow_report_id and (mapping.metric != 'cash' or
                ...
                raise ValidationError(_('Select the Cash Flow Statement for the cash mapping.'))
            if mapping.partner_ledger_report_id and (mapping.metric not in {'receivables', 'payables'} or
                ...
                raise ValidationError(_('Select the Partner Ledger for a receivable or payable mapping.'))
            ...
            for expression in pending.line_ids.expression_ids:
                for reference in re.findall(r'cross_report\(([^)]+)\)', expression.subformula or ''):
                    ...
                    if not dependency or dependency._name != 'account.report':
                        raise ValidationError(_('A dependent report could not be resolved.'))
